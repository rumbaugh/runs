...
for source in sources:
    ...
    if source == '0414+573':
        ...
        #tdates = np.delete(tdates,gdel)
    FILE = open('/home/rumbaugh/EVLA/light_curves/VLA_lc_aipsredux_NC_4.5.14_%s.dat'%source,'w')
    for date in tdates:
        if date[1] == '.':
        ...
        if date not in tdates[gdel]:
            FILE.write('%f'%daytot)
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_NC_4.5.14.%s.fixpos.%s.mod'%(source,date),dtype='string',comments='!')
...
for source in np.append(CSOs,'1035+564'):
    ...
    for date in tdates:
        ...
        if date not in tdates[gdel]:
            FILE.write('%f'%daytot)
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_NC.%s.fixpos.%s.mod'%(source,date),dtype='string',comments='!')
            fluxstr = cr[0]
            FILE.write(' ' + fluxstr[:len(fluxstr)-1])
            FILE.write('\n')
        else:
