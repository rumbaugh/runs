...
#while ((i < 1) & (endloop < 0.5)):
    ...
    tarfile = '%s../plotants.%sSB%s_%s.%s.%s.11.ps'%(cur_dir,EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i])
    plotants(vis=vis,figfile=tarfile)
    time.sleep(20)
        #while ((ispw < 2) & (endloop < 0.5)):
        #    print 'spw = %i'%ispw
        #    plotms(vis=vis,xaxis='chan',yaxis='amp',field='%i'%ifield,spw='%i'%ispw,plotfile='%s../%sSB%s_%s.%s.%s.11.chanvsamp.field_%i,spw_%i.plot.ps'%(cur_dir,EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i],ifield,ispw))
        #    gotcon = 0
