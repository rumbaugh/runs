...
crmqSN=np.loadtxt('/home/rumbaugh/milliquas_lightcurve_entries_y3a1_SN.tab',dtype={'names':('MQrownum','cid','mjd','ra','dec','mag','magerr','mag_auto','mag_auto_err','band','flags',),'formats':('i8','i8','f8','f8','f8','f8','f8','f8','f8','|S12','i8')},skiprows=1)
...
for cid,MQrn,SPrn,SDSSNAME,imi in zip(crmim['COADD_OBJECTS_ID'],crmim['MQ_ROWNUM'],crmim['SP_ROWNUM'],crmim['SDSS_NAME'],np.arange(len(crmim))):
    ...
    if SDSSNAME!='-1':
        ...
        db_outcr['DatabaseID'],db_outcr['Survey'],db_outcr['SurveyCoaddID'],db_outcr['SurveyObjectID'],db_outcr['RA'],db_outcr['DEC'],db_outcr['MJD'],db_outcr['TAG'],db_outcr['BAND'],db_outcr['MAGTYPE'],db_outcr['MAG'],db_outcr['MAGERR'],db_outcr['FLAG']=DBID,'DES',cid,np.array(crybh['OBJECT_ID'])[gBHy],yra,ydec,mjd,'NONE',bands,'PSF',mag,magerr,flags
        outcr=np.append(outcr,db_outcr)
        ...
        if len(gBHs)>0:
            ...
            print "No SDSS data for %s"%SDSSNAME
    else:
