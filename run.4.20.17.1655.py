...
crdb=crdb[gdr]
crrd=np.loadtxt('/home/rumbaugh/radecname_forSDSScutouts.4.20.17.csv',dtype={'names':('ra','dec','dbid'),'formats':('f8','f8','|S24')},delimiter=',')
...
for i in range(0,len(dbids)):
    grd=np.where(dbids[i]==crrd['dbid'])[0][0]
