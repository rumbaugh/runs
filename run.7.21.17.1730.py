...
plt.figure(1)
plt.clf()
plt.hist(df.sigma.values,color='k',alpha=0.5,label='All')
plt.hist(df[df.field.values=='S1'].sigma.values,color='r',alpha=0.5,label='S1')
plt.hist(df[df.field.values=='S2'].sigma.values,color='b',alpha=0.5,label='S2')
plt.xlabel(r'$\sigma**2$')
plt.ylabel(r'$\tau$ (days)')
plt.legend()
