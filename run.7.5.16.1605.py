...
for field in ['cl1324']:
    ...
    for i in range(0,len(RFU)):
        ...
        FILE.write('%6s %9.5f %9.5f %9.6f %2f %16s %9.5f %9.5f %9.6f %9.6f %2i %4s %9.5f %9.5f %i %6s %9.5f %9.5f %i %f %f\n'%(cr['ID'][i],cr['a'][i],cr['dec'][i],crpz['z_peak'][i],flag[i],ID_spec,ra_spec,dec_spec,z_spec_tmp,cr['z_spec'][i],Q_tmp,ID_xray,rax,decx,nm,ID_xray_adam,rax_adam,decx_adam,nm_adam,mRFU[i],mRFB[i]))
    FILE.close()
