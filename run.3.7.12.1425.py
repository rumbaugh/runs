...
for i in range(0,len(names)):
    ...
    gs = np.where((sigX_fulltemp >= 3) & (fluxX_fulltemp > 0.1E-13))
    gs = gs[0]
...
for i in range(0,len(offax_dens)):
    ...
    #offax_dens[i] = len(gt[0])/7.0
    offax_dens_err[i] = m.sqrt(len(gt[0]))/7.0/(m.pi*((prev+step)**2-prev**2))
    #offax_dens_err[i] = m.sqrt(len(gt[0]))/7.0
    prev += step
...
for i in range(0,len(offax_dens2)):
    ...
    #offax_dens2[i] = len(gt[0])
    #offax_dens2_err[i] = m.sqrt(len(gt[0]))
    prev += step
...
pylab.errorbar(x1,offax_dens,yerr=offax_dens_err,color='red',fmt='ro',lw=1,capsize=3,mew=1,label='_nolegend_')
pylab.errorbar(x2,offax_dens2,yerr=offax_dens2_err,marker='d',fmt='ro',color='blue',lw=1,capsize=3,mew=1,ms=8)
pylab.savefig('/home/rumbaugh/offaxis_analysis.3.7.12.png')
pylab.close('all')
pylab.hist(allfluxes,bins=20,range=[0,0.2E-13])
pylab.savefig('/home/rumbaugh/fluxtest1.png')
pylab.close('all')
pylab.hist(fluxX_fulltemp,bins=20,range=[0,0.2E-13])
