...
for target,it in zip(targets,np.arange(0,len(targets))):
    if target=='cl1324':
        cr=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_lum_limit_alt.dat'%('cl1324_north','cl1324_north','cl1324_north'))
    else:
        cr=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_lum_limit_alt.dat'%(target,target,target))
    plt.figure(1)
    plt.clf()
    ...
    plt.scatter(cr[:,1],np.sqrt(cr[:,2]/np.pi))
    plt.xlabel('Off axis angle')
    plt.ylabel('A90')
    plt.title(target)
    plt.savefig('/home/rumbaugh/Chandra/plots/X-ray_completeness.OAAvsR90.%s.%s.png'%(target,date))
    ...
    plt.tick_params(which='major',length=8,width=2,labelsize=14)
    plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
    plt.scatter(cr[:,4],cr[:,5])
    plt.xlabel('PhysX')
    plt.ylabel('PhysY')
    plt.title(target)
    plt.savefig('/home/rumbaugh/Chandra/plots/X-ray_completeness.PhysXvsY.%s.%s.png'%(target,date))
