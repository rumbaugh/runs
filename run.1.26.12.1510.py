...
for i in range(0,len(names)-1):
   ...
   for j in range(0,len(gclus)):
      ...
      avgvelt = C*((1.0+zavg)**2-1)/((1.0+zavg)**2+1)
      ...
      #veldist = 0.1*tempsum-C*C*np.average(zerr[gq[gclus[nnarr[j,0:10]]]])**2/((1+zavgt)**2)
      veldist = np.std(relvelst)
      delta[j] = (m.sqrt(11.0)/veldists[i])*m.sqrt((np.average(velst)-np.average(vels))**2+(veldist-veldists[i])**2)
   Del = np.sum(delta)
