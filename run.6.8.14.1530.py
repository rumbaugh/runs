...
#find time delays
    ...
    mintime = -1*maxtime
    tau_init,mu_init = -50.,np.mean(Bflux)/np.mean(Cflux)
    ndim,nwalkers = 2,10
    if lens == '1938':
        maxtimestep = 60.
    else:
    ...
    sampler = emcee.EnsembleSampler(nwalkers,ndim,delaylnprob,args=[Bflux,Cflux,Berr,Cerr,ltime,mu_init,mintime,maxtime,False,True,True,'boxcar',15.,0.5,None])
