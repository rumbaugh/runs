...
cov = np.array([[1.0,0.0],[0.0,0.01]])

for i in range(0,1):
    ndim,nwalkers = 2,4
    p0 = np.zeros((nwalkers,ndim))
    p0[:,0],p0[:,1] = np.zeros(nwalkers)+np.random.normal(scale=0.1,size=nwalkers),np.ones(nwalkers)*mu_init+np.random.normal(scale=0.01,size=nwalkers)
    if i != 0: sampler.reset()
    sampler = emcee.MHSampler(cov,ndim,delaylnprob,args=[Aflux,Bflux,Aerr,Berr,ltime])
    #pos,prob,state=sampler.run_mcmc(p0,runs)
    p0 = np.array([-23.,mu_init])
    ...
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/testcurve.MH_output_hist_tau.BA_delay.%s.ps'%(date))
    py.close()
    py.plot(sampler.flatchain[:,1])
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/testcurve.MH_output_chain_plot_mu.BA_delay.%s.ps'%(date))
    py.close()
    py.plot(sampler.flatchain[:,0])
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/testcurve.MH_output_chain_plot_tau.BA_delay.%s.ps'%(date))
    py.close()
    for j in range(0,nwalkers):
        py.plot(sampler.chain[j,:,0])
        py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/testcurve.MH_output_all_chain_plot_tau.BA_delay.%s.ps'%(date))
    py.close()
    for k in range(1,2):
        for j in range(0,nwalkers):
            py.plot(sampler.chain[j,:,k])
            py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/testcurve.MH_output_all_chain_plot_mu%i.BA_delay.%s.ps'%(k,date))
        py.close()
    py.scatter(sampler.flatchain[:,0],sampler.flatlnprobability)
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/testcurve.MH_output_tau-prob_scatter.BA_delay.%s.ps'%(date))
    py.close()
