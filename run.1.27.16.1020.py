...
for field,z in zip(targets[9:],zlist[9:]):
    ...
    for band in ['soft','hard','full']:
        ...
        err=np.copy(hdu[0].data)
        if writeerr:hdu.writeto('%s/%s_%s_nops.smoothed_err.z_%.2f.beta_0.67.rc_180kpc.img'%(curdir,field,band,z),clobber=True)
        hdu.close()
        hdu=py.open(smthfile)
        hdu[0].data[err!=0]/=err[err!=0]
        hdu.writeto('%s/%s_%s_nops.smoothed_sigma.z_%.2f.beta_0.67.rc_180kpc.img'%(curdir,field,band,z),clobber=True)
