...
#find time delays
BA_delay_d2,BAmud2,BAdispd2,BANeff = chi_squared_min_delays(Aflux,Bflux,ltime,ltime,Aerr,Berr,t_grid,maxtimestep,timestep,minmu,maxmu,mustep,smooth_param=np.array([10.]),output=3,use_overlap_mean=True,ALAG=ALAG)
t1 = time.time()
...
print "\n\nB-A:\n delay: %4.1f days  mu: %6.4f   chi^2: %f\n:"%(BA_delay_d2,BAmud2,BAdispd2)
if not justone: print "\n\nB-C:\n delay: %4.1f days  mu: %6.4f   chi^2: %f\n:"%(BC_delay_d2,BCmud2,BCdispd2)
if not justone: print "\n\nB-D:\n delay: %4.1f days  mu: %6.4f   chi^2: %f\n:"%(BD_delay_d2,BDmud2,BDdispd2)
t3 = time.time()
print "All Done. Elapsed Time: %f seconds"%(t3-st1)
FILE = open("output.chi_squared_delays.1.25.13.dat",'w')
FILE.write("B-A:\n delay: %4.1f days  mu: %6.4f   chi^2: %f\n:"%(BA_delay_d2,BAmud2,BAdispd2))
if not justone: FILE.write("B-C:\n delay: %4.1f days  mu: %6.4f   chi^2: %f\n:"%(BC_delay_d2,BCmud2,BCdispd2))
if not justone: FILE.write("B-D:\n delay: %4.1f days  mu: %6.4f   chi^2: %f\n:"%(BD_delay_d2,BDmud2,BDdispd2))
FILE.write('Total Elapsed Time: %f seconds'%(t3-st1))
FILE.close()
