...
bgcnts = np.array([626,626,572,482,988,988,614,1460,1460])
...
for i in range(0,len(names)):
    ...
    #cr2 = read_file(xfile + '500-2000.nops.fits')
    load_data(xfile + '500-2000.nops.fits')
    emreg = '/home/rumbaugh/ChandraData/%s/master/embg.%s.2.14.12.reg'%(strucs[i],names[i])
    set_coord('physical')
    bgcnts2 = calc_data_sum2d(reg=emreg)
    #pvals2 = copy_piximgvals(cr2)
    #pvals = pvals1 + pvals2
    bgcnts = bgcnts1 + bgcnts2
    pvshape = np.shape(pvals)
    cp1 = time.time()
    tp0 = P_0(pvals,imcensX[i],imcensY[i],srchdist[i])-bgcnts*srchdist[i]**2/((0.492*0.492)*regsizes[i])
    ...
    tp0v = m.sqrt((P_0_var(pvals,imcensX[i],imcensY[i],srchdist[i]))**2+bgcnts*(srchdist[i]/(0.492))**4/(regsizes[i]**2))
