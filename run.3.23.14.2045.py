...
FILE=open('/home/rumbaugh/runs/run.3.23.14.2045.sh','w')
FILE.write("#!/bin/csh\n")
...
for date,aipsdate in zip(dates,aips_dates):
    ...
    for source in tsources:
        ...
        outfile = '/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_3.23.14.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
    ...
    for source in np.append(tCSOs,'1035+564'):
        ...
        FILE.write(fit_string)
        outfile = '/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
        FILE.write(end_string)
FILE.close()
