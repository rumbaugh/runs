...
outcr=np.zeros((len(cr_peaks),),dtype={'names':('field','clus','Rmax','xcen','ycen','P3','P4','P3LB','P3UB','P4LB','P4UB'),'formats':('|S24','|S24','f8','f8','f8','f8','f8','f8','f8','f8','f8')})
...
for i in range(0,len(cr_peaks)):
    ...
    data=hdu[0].data
    SF_arr=data[int(np.round(ycen0))-wid:int(np.round(ycen0))+wid+1,int(np.round(xcen0))-wid:int(np.round(xcen0))+wid+1]*0.5
    ...
    outcr['Rmax'][i]=R_Mpc
np.savetxt('/home/rumbaugh/Chandra/ORELSE.power_ratios.halved_SF.dat',outcr,fmt='%12s %12s %6.1f %10.5f %10.5f %E %E %E %E %E %E',header='field cluster Rmax ImageX ImageY P3 P4 P3LB P3UB P4LB P4UB')
