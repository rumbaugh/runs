...
FILE=open('/home/rumbaugh/runs/run.4.5.14.1530.sh','w')
FILE.write("#!/bin/csh\n")
...
for date,aipsdate in zip(dates,aips_dates):
    ...
    for source in tsources:
        ...
        uvfile = '/mnt/data2/rumbaugh/AIPS/FITS/AF377_NC_%s_20%s'%(source,aipsdate)
        FILE.write('obs %s\n'%uvfile)
        ...
        outfile = '/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_NC_4.5.14.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
    ...
    for source in np.append(tCSOs,'1035+564'):
        ...
        FILE.write(fit_string)
        outfile = '/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_NC.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
        FILE.write(end_string)
FILE.close()
