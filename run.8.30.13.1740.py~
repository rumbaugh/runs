...
for lens in lens_dict:
    ...
    for img in range(0,nimg):
        ...
        if lens == '0414':
            g = np.where(((S/np.mean(S[g])/CSOnorm<1.051)&(S/np.mean(S[g])/CSOnorm>0.95)&(np.arange(len(S))!=34)&(S>0)&(CSO1S>0)&(CSO2S>0)))[0]
            CSOnorm = 0.5*(CSO1S[g]/np.mean(CSO1S[g]) + CSO2S[g]/np.mean(CSO2S[g]))
        elif lens == '0712':
            g = np.where(((S/np.mean(S[g])/CSOnorm<1.085)&(S/np.mean(S[g])/CSOnorm>0.905)&(np.arange(len(S))!=34)&(S>0)&(CSO1S>0)&(CSO2S>0)))[0]
            CSOnorm = 0.5*(CSO1S[g]/np.mean(CSO1S[g]) + CSO2S[g]/np.mean(CSO2S[g]))
        elif lens == '1030':
            g = np.where(((S/np.mean(S[g])/CSOnorm>0.835)&(np.arange(len(S))!=34)&(S>0)&(CSO1S>0)&(CSO2S>0)))[0]
            CSOnorm = 0.5*(CSO1S[g]/np.mean(CSO1S[g]) + CSO2S[g]/np.mean(CSO2S[g]))
        elif lens == '1127':
            g = np.where(((S/np.mean(S[g])/CSOnorm<1.04)&(np.arange(len(S))!=34)&(S>0)&(CSO1S>0)&(CSO2S>0)))[0]
            CSOnorm = 0.5*(CSO1S[g]/np.mean(CSO1S[g]) + CSO2S[g]/np.mean(CSO2S[g]))
        elif lens == '1152':
            g = np.where(((S/np.mean(S[g])/CSOnorm>0.94)&(S/np.mean(S[g])/CSOnorm<1.05)&(np.arange(len(S))!=34)&(S>0)&(CSO1S>0)&(CSO2S>0)))[0]
            CSOnorm = 0.5*(CSO1S[g]/np.mean(CSO1S[g]) + CSO2S[g]/np.mean(CSO2S[g]))
        print np.mean(CSOnorm)
    ...
    plt.ylabel('Flux')
    plt.xlim(1810,2090)
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s.lc_plot.8.29.13.png'%lens)
    plt.clf()
    plt.rc('axes',linewidth=2)
    plt.fontsize = 14
    plt.tick_params(which='major',length=8,width=2,labelsize=14)
    ...
    plt.xlim(1810,2090)
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s.lc_norm_plot.8.29.13.png'%lens)
lens = '0712'
cr = np.loadtxt('/home/rumbaugh/EVLA/light_curves/%s_g.edt'%(lens))
nimg = lens_dict[lens]['images']
plt.figure(1)
...
for img in range(1,nimg):
    ...
    if img == 1: 
        SA = cr[:,0]
        g = np.where(((np.arange(len(S))!=34)&(S>0)&(SA>0)&(CSO1S>0)&(CSO2S>0)))[0]
        CSOnorm = 0.5*(CSO1S/np.mean(CSO1S[g]) + CSO2S/np.mean(CSO2S[g]))
        g = np.where(((S/np.mean(S[g])/CSOnorm<1.085)&(S/np.mean(S[g])/CSOnorm>0.905)&(np.arange(len(S))!=34)&(S>0)&(SA>0)&(CSO1S>0)&(CSO2S>0)))[0]
        S = 0.5*(S+SA)
    else:
        g = np.where(((np.arange(len(S))!=34)&(S>0)&(CSO1S>0)&(CSO2S>0)))[0]
    #CSOnorm = 0.5*(CSO1S[g]/np.mean(CSO1S[g]) + CSO2S[g]/np.mean(CSO2S[g]))
...
plt.ylabel('Normalized Flux',fontsize=14)
plt.xlim(1810,2090)
plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s.lc_norm_plot_comb_A+B.8.29.13.png'%lens)
