...
for rung in range(0,5):
    for i in range(0,9):
        ...
        cr = np.loadtxt('/mnt/data2/rumbaugh/TDC/tdc1sample_for_test/rung%i/emcee/%s.emcee_output_full_chain.12.19.13.dat'%(rung,pair))
        tau_sort = np.sort(cr[500:,0])
        FILE.write('%25s %6.2f %6.2f %6.2f %3i %3i\n'%(pair,np.median(tau_sort),tau_sort[int(0.16*len(tau_sort))],tau_sort[int(0.84*len(tau_sort))],ref_dict['rung'][rung][pair]['range'][0],ref_dict['rung'][rung][pair]['range'][1]))
FILE.close()
