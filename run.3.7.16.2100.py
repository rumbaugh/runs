...
for field in targets:
    ...
    for band in ['soft','hard','full']:
        hduimg=py.open('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s_img'%(field,field,field,band))
        tmpdimg=hduimg[0].data
        numadd0,numadd1=binwid-np.shape(tmpdimg)[0]%binwid,binwid-np.shape(tmpdimg)[1]%binwid
        dimg=np.append(tmpdimg,np.zeros((numadd,np.shape(tmpdimg)[0])),axis=0)
        dimg=np.append(dimg,np.zeros((np.shape(dimg)[0],numadd)),axis=1)
