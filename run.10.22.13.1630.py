...
FILE = open('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/chisq_results_Aconfig.10.22.13.1715.dat','w')
...
for lens in lens_dict:
    ...
    for img in range(0,nimg):
        ...
        if lens != '0712': 
            chisq_arr[img],prob_arr[img] = calc_chi_sqrd(S[g][days[g]<100]/CSOnorm[days[g]<100],Serr[g][days[g]<100]/CSOnorm[days[g]<100],calcprob=True,trials=trials)
    ...
    #    plt.legend(testlegs,('A','B'),loc=2)
            imgsarr = ['A','B']
        plt.title('Normalized Lightcurve for Lens %s Image %s'%(lens_dict[lens]['name'],imgsarr[img]))
        plt.xlabel('Days',fontsize=14)
        plt.ylabel('Normalized Flux',fontsize=14)
        plt.xlim(1810-cr[0][0],2090-cr[0][0])
        plt.plot(days[g],np.mean(S[g]/CSOnorm)*np.ones(len(g)))
        if lens != '0712': plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s_%s.lc_norm_plot.10.22.13.png'%(lens,imgsarr[img]))
    for img in range(0,4):
        if lens != '0712': FILE.write("%2s %6.2f %6.4f "%(imgarr[img],chisq_arr[img],prob_arr[img]))
lens = '0712'
cr = np.loadtxt('/home/rumbaugh/EVLA/light_curves/%s_g.edt'%(lens))
nimg = lens_dict[lens]['images']
testlegs = []
...
for img in range(1,nimg):
    ...
    Serr /= Smean
    imgarr = ['A+B','C','D','NA']
    chisq_arr[img-1],prob_arr[img-1] = calc_chi_sqrd(S[g][days[g]<100]/CSOnorm[days[g]<100],Serr[g][days[g]<100]/CSOnorm[days[g]<100],calcprob=True,trials=trials)  
    ...
    #plt.legend(testlegs,('A+B','C','D'),loc=2)
    plt.title('Normalized Lightcurve for Lens %s Image %s'%(lens_dict[lens]['name'],['A+B','C','D'][img-1]))
    plt.xlabel('Days',fontsize=14)
    plt.ylabel('Normalized Flux',fontsize=14)
    plt.xlim(1810-cr[0][0],2090-cr[0][0])
    plt.plot(days[g],np.mean(S[g]/CSOnorm)*np.ones(len(g)))
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s_%s.lc_norm_plot_comb.10.22.13.png'%(lens,['A+B','C','D'][img-1]))
for img in range(0,4):
    FILE.write("%2s %6.2f %6.4f "%(imgarr[img],chisq_arr[img],prob_arr[img]))
FILE.close()
