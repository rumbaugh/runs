...
ldate = '8.18.14'
for lens in ['B0712']:
    cr = np.loadtxt('/home/rumbaugh/EVLA/light_curves/Dispersions/%s_jointfit.emcee_output_full_chain_%s_bs.2Xerr.%s.dat'%(lens,kernel,ldate))
    tau,mu = cr[:,0],cr[:,1]
    #plt.figure(1)
    plt.clf()
    ...
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s_jointfit.chisq_conplot.%s.2Xerr.triangle_mod_plot.%s.png'%(lens,kernel,date))
