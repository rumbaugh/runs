...
for i,pair in zip(np.arange(len(level_dict.keys())),level_dict.keys()):
    ...
    stime,F = cr[:,0],cr[:,1]
    disparr,muarr,tarr = calc_disp_delay(F,F,stime,stime,errstd*np.ones(len(stime)),errstd*np.ones(len(stime)),100,0.237,1,1,None,'D_2',output=2,disparray=True,outfile='/mnt/data2/rumbaugh/Fermi/output/test/Dispersion.testlightcurve_%i.tau_%.2f.mu_%.3f.%s.dat'%(pair,tau,mu,ldate))
    plt.figure(1)
    plt.clf()
