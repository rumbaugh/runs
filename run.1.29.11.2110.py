...
Rcore = np.array([15.07,16.99])
names = np.array(['3572','4807'])

FILE=open("/scratch/rumbaugh/ciaotesting/analysis/lens.analysis.Amp.1.29.11.dat","w")
for i in range(0,len(names)):
    load_data('/scratch/rumbaugh/ciaotesting/%s/acis%s.img.500-2000.S07.nops.norand.fits'%(names[i],names[i]))
    set_coord("physical")
    ostemp = os.system('dmstat /scratch/rumbaugh/ciaotesting/%s/conv.beta.1.29.11.fits verbose=0'%(names[i]))
...
Rcore[0] = 15.44
for i in range(0,1):
    load_data('/scratch/rumbaugh/ciaotesting/merged/423+424.test.fits')
    set_coord("physical")
    ostemp = os.system('dmstat /scratch/rumbaugh/ciaotesting/merged/conv.beta.1.29.11.423+424.fits verbose=0')
    ostemp = os.system('pget dmstat out_max_loc | tee dmstat.temp.txt')
    crcen = read_file("dmstat.temp.txt")
    centemp = get_colvals(crcen,'col1')
    centemp = centemp[0]
    ...
    tInt = IntegrateSurBrSimp(0, 15, a, 0 )
    Amp = DataCen/(m.pi*(15*15)*tInt)
    FILE.write('%12s %5.2f %9.7f %9.7f %9.7f\n'%('423+424',Rcore[i],bkg,Amp,Amp/bkg)) 
FILE.close()
