...
st = time.time()
prevtime = st
FILE = open('/local3/rumbaugh/EVLA/scripts/Automap/run_calfluxonPC.sh','w')
FILE.write("#!/bin/csh\n\nset c = 'false'\n")
...
#for i in range(0,len(datearr)):
    ...
    FILE.write("set a = %s%s.%s.uvfits\nset b = /local3/rumbaugh/EVLA/data/11A-138/difmap_results/%s.%s\ndifmap << EOF\n@/local3/rumbaugh/EVLA/scripts/Automap/calflux.scr $a,$b,$c\nquit\nEOF\n"%(cur_dir,outbase,fieldname,outbase,fieldname))
