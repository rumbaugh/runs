...
for i in range(0,7): 
    ...
    for band in ['soft','hard','full']:
        ...
        dum=os.system('cp %s bkup_%s'%(sfile,sfile))
        dum=os.system('cp %s bkup_%s'%(rfile,rfile))
        if not((band=='soft')&(strname=='3181+4987')):check_wavdetect_regs(ffile,sfile,rfile,False)
