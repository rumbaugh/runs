...
for i in range(0,len(names)):
    ...
    if skipMC == 0:
        Del,P,delta = DStest(RA[g[gRGP[gRGPtemp]]],Dec[g[gRGP[gRGPtemp]]],z[g[gRGP[gRGPtemp]]],vel_in=vels[gRGPtemp])
        if writeMC == 1:
    ...
    #print 'c'
    gBCG = np.where((BCGzs[i] + 0.00004 > z[g[gRGP[gRGPtemp]]]) & (BCGzs[i] - 0.00004 < z[g[gRGP[gRGPtemp]]]))
    gBCG = gBCG[0]
    if len(gBCG) > 1: gBCG = gBCG[0]
    BCGvel = vels[gRGPtemp[gBCG]]
    print '%s:\n sig = %f using %i gals\n Del = %f - P = %f\n'%(names[i],sig,len(gRGPtemp),Del,P)
    print biweight_loc(sz[g[gRGP[gRGPtemp]]])
    ...
    for j in range(0,len(delta)):
        ...
        if ((z[g[gRGP[gRGPtemp[j]]]] > avg_z-zsig) & (z[g[gRGP[gRGPtemp[j]]]] < avg_z+zsig)): 
            tx = pylab.cos(pylab.linspace(0,2*m.pi,50)).tolist()
            ty = pylab.sin(pylab.linspace(0,2*m.pi,50)).tolist()
            xy1 = zip(tx,ty)
            pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='red')
        elif ((z[g[gRGP[gRGPtemp[j]]]] < avg_z-zsig)):
            #pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=circsize,facecolors=html_teal,edgecolor='black')
            ...
            xy4 = zip(tx,ty)
            pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors=html_teal)
            pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=0.25*circsize,marker=(xy2,0),facecolors='none',edgecolors=html_teal)
            pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=0.25*circsize,marker=(xy3,0),facecolors='none',edgecolors=html_teal)
            pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=0.25*circsize,marker=(xy4,0),facecolors='none',edgecolors=html_teal)
        else:
            tx = pylab.cos(pylab.linspace(0,2*m.pi,50)).tolist()
            ty = pylab.sin(pylab.linspace(0,2*m.pi,50)).tolist()
            xy1 = zip(tx,ty)
            pylab.scatter(RA[g[gRGP[gRGPtemp[j]]]],Dec[g[gRGP[gRGPtemp[j]]]],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='blue',linestyle='dotted')
    pylab.savefig('/home/rumbaugh/DSplot.%s.3.16.12.png'%(names[i]))
    pylab.close('all')
    #print 'd'
    #nep5281newcnt = 0
    ...
    #print 'e'
    if ccnt != 6:
        gred = np.where((srB[g[gRGP[gRGPtemp]]]-siB[g[gRGP[gRGPtemp]]] > rsb[ccnt]-rsm[ccnt]*siB[g[gRGP[gRGPtemp]]]-rsNSTD[ccnt]*rsSTD[ccnt]))
    else:
        gred = np.where((siB[g[gRGP[gRGPtemp]]]-szB[g[gRGP[gRGPtemp]]] > rsb[ccnt]-rsm[ccnt]*szB[g[gRGP[gRGPtemp]]]-rsNSTD[ccnt]*rsSTD[ccnt]))
    if ccnt != 6:
        gblu = np.where((srB[g[gRGP[gRGPtemp]]]-siB[g[gRGP[gRGPtemp]]] < rsb[ccnt]-rsm[ccnt]*siB[g[gRGP[gRGPtemp]]]-rsNSTD[ccnt]*rsSTD[ccnt]))
    else:
        gblu = np.where((siB[g[gRGP[gRGPtemp]]]-szB[g[gRGP[gRGPtemp]]] < rsb[ccnt]-rsm[ccnt]*szB[g[gRGP[gRGPtemp]]]-rsNSTD[ccnt]*rsSTD[ccnt]))
    #print 'f'
    gblu = gblu[0]
    gred = gred[0]
    ...
    if ((len(gblu) >= 10) & (len(gred) >= 10)): pylab.plot(tempgaussx,tempgaussy,color='blue')
    tempgaussy = np.zeros(3200)
    for it in range(0,3200): tempgaussy[it] = 500*(len(gred)/(redsig*m.sqrt(2*m.pi)))*m.exp(-0.5*((tempgaussx[it]-redmean)/redsig)**2)
    #print 'h'
    if ((len(gblu) >= 10) & (len(gred) >= 10)): pylab.plot(tempgaussx,tempgaussy,color='red')
    ...
    pylab.savefig('/home/rumbaugh/veldist.redblu.%s.3.23.12.png'%(names[i]))
    pylab.close('all')
    FILEidl1 = open('/home/rumbaugh/temp/idl_clusprops.%s.3.23.12.dat'%(names[i]),'w')
    if enoughBoR == 1:
        FILEidl1.write('%f %f %f %f'%(sig,blusig,redsig,BCGvel))
    else:
        FILEidl1.write('%f %f %f %f'%(sig,-1,-1,BCGvel))
    FILEidl1.close()
    FILEidl2 = open('/home/rumbaugh/temp/idl_vels.%s.3.23.12.dat'%(names[i]),'w')
    for iidl in range(0,len(gRGPtemp)):
        isblu = 0
        if ccnt == 6:
            if siB[g[gRGP[gRGPtemp[iidl]]]]-szB[g[gRGP[gRGPtemp[iidl]]]] < rsb[ccnt]-rsm[ccnt]*szB[g[gRGP[gRGPtemp[iidl]]]]-rsNSTD[ccnt]*rsSTD[ccnt]: isblu = 1
        else:
            if srB[g[gRGP[gRGPtemp[iidl]]]]-siB[g[gRGP[gRGPtemp[iidl]]]] < rsb[ccnt]-rsm[ccnt]*siB[g[gRGP[gRGPtemp[iidl]]]]-rsNSTD[ccnt]*rsSTD[ccnt]: isblu = 1
