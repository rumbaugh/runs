...
FILE=open('/home/rumbaugh/scalingrelation.offsets_sigma.5.24.12.dat','w')
...
pylab.fontsize = 18
pylab.tick_params(which='major',length=8,width=3.5,labelsize=16)
pylab.tick_params(which='minor',length=4,width=2,labelsize=16)
...
pylab.xlabel('T (keV)',fontsize=24)
pylab.ylabel('L$_x$ E(z)$^{-1}$ (10$^{44}$ erg s$^{-1}$)',fontsize=24)
...
pylab.savefig('/home/rumbaugh/LxT.plot.bol.5.24.12.png')
pylab.close('all')
...
pylab.xlabel('$\sigma_v$ (km s$^{-1}$)',fontsize=24)
pylab.ylabel('L$_x$ E(z)$^{-1}$ (10$^{44}$ erg s$^{-1}$)',fontsize=24)
for i in range(0,len(Temps)): 
    if i == 0: 
        pylab.text(sigma[i]-65,lumbol[i]+0.33,names[i],color='black',weight='extra bold')
    elif i == 5: 
        pylab.text(sigma[i]-95,lumbol[i]-0.14,names[i],color='black',weight='extra bold')
    elif i == 3:
        ...
        pylab.text(sigma[i]-160,lumbol[i]+0.05,names[i],color='black',weight='extra bold')
    elif i == 8:
...
pylab.yticks([1,5],['1','5'])
pylab.savefig('/home/rumbaugh/sigLx.plot.5.24.12.png')
pylab.close('all')
print '\nsig-Lx\n'
...
pylab.xlabel('T (keV)',fontsize=24)
pylab.ylabel('$\sigma_v$ (km s$^{-1}$)',fontsize=24)
for i in range(0,len(Temps)): 
    if i == 0: 
...
pylab.xticks([1,5],['1','5'])
pylab.savefig('/home/rumbaugh/sigT.plot.5.24.12.png')
pylab.close('all')
