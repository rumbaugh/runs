...
outfile='/home/rumbaugh/combined_match_catalog.all_sources.%s.dat'%date
FILE=open(outfile,'w')
...
for field in targets: 
    ...
    g_az=np.arange(len(all_zs))[np.abs(all_zs-0.5*(zLB+zUB))<0.5*(zUB-zLB)]
    zoom_zs=all_zs[g_az]
    zoom_ras,zoom_decs=all_ra[g_az],all_decs[g_az]
    x_zs=zcur[qcur>2.5]
    g_zs=np.arange(len(x_zs))[np.abs(x_zs-0.5*(zLB+zUB))<0.5*(zUB-zLB)]
    x_zoom_zs=x_zs[g_zs]
    ...
    plt.hist(zoom_zs,bins=80,range=(zLB,zUB),color='k')
    plt.hist(x_zoom_zs,bins=80,range=(zLB,zUB),color='r')
    plt.xlabel('Redshift')
