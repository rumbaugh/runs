...
cov = np.array([[0.25,0.0],[0.0,0.01]])

ndim,nwalkers = 2,10
ltime -= ltime[0]
p0 = np.zeros((nwalkers,ndim))
p0[:,0],p0[:,1] = np.ones(nwalkers)*36.5+np.random.normal(scale=1,size=nwalkers),np.ones(nwalkers)*1.03083+np.random.normal(scale=0.1,size=nwalkers)
sampler = emcee.MHSampler(cov,ndim,delaylnprob,args=[Cflux,Bflux,Cerr,Berr,ltime])
sampler.run_mcmc([36.5,1.03083],runs)
