...
for source in sources:
    ...
    if source == '0414+573':
        ...
        #tdates = np.delete(tdates,gdel)
    FILE = open('/home/rumbaugh/EVLA/light_curves/VLA_lc_aipsredux_UVR_0_300_5.30.14_%s.varpos.dat'%source,'w')
    for date in tdates:
        if date[1] == '.':
        ...
        if date not in tdates[gdel]:
            FILE.write('%f'%daytot)
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_UVR_0_300_5.30.14.%s.varpos.%s.mod'%(source,date),dtype='string',comments='!',usecols=(0,1,2))
            try:
                test = cr[2,2]
                num_img = 4
                ...
                print 'fit_aipsredux_UVR_0_300_5.30.14.%s.varpos.%s.mod failed'%(source,date)
