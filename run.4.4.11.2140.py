...
for i in range(0,5):
    ...
    FILE = open('/home/rumbaugh/paperstuff/input.fullRShist.%s.dat'%(names[i]),'w')
    for j in range(0,len(gsq)): FILE.write('%f %f %f\n'%(sRA[gsq[j]],sDec[gsq[j]],sz[gsq[j]]))
    FILE.close()
    FILE = open('/home/rumbaugh/paperstuff/input.fullXrayRShist.%s.dat'%(names[i]),'w')
    for j in range(0,len(gq)): FILE.write('%f %f %f\n'%(sRA[mInds[ns[gq[j]]]],sDec[mInds[ns[gq[j]]]],sz[mInds[ns[gq[j]]]]))
    FILE.close()
