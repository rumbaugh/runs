...
for i in range(0,len(names)):
    ...
    pylab.xlabel('R.A. (J2000)',fontsize=18)
    pylab.ylabel('Dec. (J2000)',fontsize=18)
    pylab.title(names[i],fontsize=18)
    pylab.xlim(boundsRAHi[i],boundsRALo[i])
    pylab.ylim(boundsDecLo[i],boundsDecHi[i])
    if names[i] == 'RXJ1821':
        #pylab.xlim(275.50004353025524,275.28195646974466)
        #pylab.ylim(68.43,68.51)
        xlocs = (np.arange(10)+2)/3.0*360.0/24/60/60*15+275.25
        xlabs = np.array(['','18$^{h}$21$^{m}$15$^{s}$','','','30$^{s}$','','','45$^{s}$','',''])
        ylocs = (np.arange(5)+1)/60.0+68+25/60.0
        ylabs = np.array(['68' + degree_symbol + "26'","27'","28'","29'","30'"])
        pylab.yticks(ylocs,ylabs,fontsize=16)
        pylab.xticks(xlocs,xlabs,fontsize=16)
    if names[i] == "RXJ1757":
        #pylab.xlim(269.45285292808654,269.22714707191341)
        #pylab.ylim(66.482,66.572)
        xlocs = (np.arange(10)-1)/3.0*360.0/24/60/60*15+269.25
        xlabs = np.array(['','17$^{h}$57$^{m}$00$^{s}$','','','15$^{s}$','','','30$^{s}$'])
        ylocs = (np.arange(5)+1)/60.0+66+29/60.0
        ylabs = np.array(['66' + degree_symbol + "30'","31'","32'","33'","34'"])
        pylab.yticks(ylocs,ylabs,fontsize=16)
        pylab.xticks(xlocs,xlabs,fontsize=16)
    if names[i] == "RXJ0910+5422":
        #pylab.xlim(137.74503696960534,137.62496303039467)
        #pylab.ylim(54.337,54.407)
        xlocs = (np.arange(6)+1)*360.0/24/60/60*5+(9+10.5/60)*360.0/24
        xlabs = np.array(['','9$^{h}$10$^{m}$40$^{s}$','','50$^{s}$','','9$^{h}$11$^{m}$00$^{s}$'])
        ylocs = (np.arange(5))/60.0+54+20/60.0
        ylabs = np.array(['54' + degree_symbol + "20'","21'","22'","23'","24'"])
        pylab.yticks(ylocs,ylabs,fontsize=16)
        pylab.xticks(xlocs,xlabs,fontsize=16)
    if names[i] == 'Cl1324+3011':
        #pylab.xlim(201.24428157607926,201.15171842392076)
        #pylab.ylim(30.16,30.240)
        xlocs = (np.arange(5))*360.0/24/60/60*5+360/24.0*(13+(24+40/60.0)/60.0)
        xlabs = np.array(['13$^{h}$24$^{m}$40$^{s}$','','50$^{s}$','','25$^{m}$00$^{s}$'])
    ...
    if names[i] == 'Cl1324+3013':
        ...
        xlocs = (np.arange(5)-1)*360.0/24/60/60*5+360/24.0*(13+(24+15/60.0)/60.0)
        xlabs = np.array(['13$^{h}$24$^{m}$10$^{s}$','','20$^{s}$','','30$^{s}$'])
        ylocs = (np.arange(5)-1)/60.0+30+12/60.0
        ylabs = np.array(['30' + degree_symbol + "11'","12'","13'","14'","15'"])
        pylab.yticks(ylocs,ylabs,fontsize=16)
        pylab.xticks(xlocs,xlabs,fontsize=16)
    ...
    if names[i] == 'Cl1604A':
        ...
        xlocs = (np.arange(5)-1)*360.0/24/60/60*5+360/24.0*(16+(4+15/60.0)/60.0)
        xlabs = np.array(['16$^{h}$04$^{m}$10$^{s}$','','20$^{s}$','','30$^{s}$'])
        ylocs = (np.arange(4)-1)/60.0+43+4/60.0
        ylabs = np.array(['43' + degree_symbol + "03'","4'","5'","6'"])
        pylab.yticks(ylocs,ylabs,fontsize=16)
        pylab.xticks(xlocs,xlabs,fontsize=16)
    if names[i] == 'Cl1604B':
        ...
        xlabs = np.array(['','16$^{h}$04$^{m}$20$^{s}$','','30$^{s}$',''])
    ...
    if names[i] == 'RXJ0910+5419':
        ...
        xlocs = (np.arange(6)-1)*360.0/24/60/60*5+360/24.0*(9+(9+55/60.0)/60.0)
        xlabs = np.array(['9$^{h}$09$^{m}$50$^{s}$','','9$^{h}$10$^{m}$00$^{s}$','','10$^{s}$',''])
        ylocs = (np.arange(5))/60.0+54+17/60.0
        ylabs = np.array(['54' + degree_symbol + "17'","18'","19'","20'","21'"])
        pylab.yticks(ylocs,ylabs,fontsize=16)
        pylab.xticks(xlocs,xlabs,fontsize=16)
    for j in range(0,len(delta)):
        circsize = 100*m.exp(1.0*delta[j])
        if ((znew[j] > avg_z-zsig) & (znew[j] < avg_z+zsig)): 
            ...
            xy1 = zip(tx,ty)
            pylab.scatter(RAnew[j],Decnew[j],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='red',lw=3)
            #pylab.scatter(RAnew[j],Decnew[j],s=secsize*0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='red')
            ...
            xy4 = zip(tx,ty)
            pylab.scatter(RAnew[j],Decnew[j],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors=html_teal,lw=3)
            pylab.scatter(RAnew[j],Decnew[j],s=0.25*circsize,marker=(xy2,0),facecolors='none',edgecolors=html_teal,lw=3)
            pylab.scatter(RAnew[j],Decnew[j],s=0.25*circsize,marker=(xy3,0),facecolors='none',edgecolors=html_teal,lw=3)
            pylab.scatter(RAnew[j],Decnew[j],s=0.25*circsize,marker=(xy4,0),facecolors='none',edgecolors=html_teal,lw=3)
        else:
            tx = pylab.cos(pylab.linspace(0,2*m.pi,50)).tolist()
            ty = pylab.sin(pylab.linspace(0,2*m.pi,50)).tolist()
            xy1 = zip(tx,ty)
            pylab.scatter(RAnew[j],Decnew[j],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='blue',linestyle='dotted',lw=4)
    pylab.scatter([centerRAs[i]],[centerDecs[i]],marker='x',edgecolors='blue',facecolors='none',s=300,lw=4)
    pylab.scatter([BCGRA[i]],[BCGDec[i]],marker='s',edgecolors=html_brwn,facecolors='none',s=300,lw=4)
    pylab.scatter([RGalPeakRA[i]],[RGalPeakDec[i]],marker='d',edgecolors=html_purp,facecolors='none',s=300,lw=4)
    pylab.savefig('/home/rumbaugh/DSplot.%s.6.27.12.png'%(names[i]))
    pylab.close('all')
    #print 'd'
    #nep5281newcnt = 0
    ...
    pylab.savefig('/home/rumbaugh/veldist.redblu.%s.4.10.12.png'%(names[i]))
    pylab.close('all')
    FILEidl1 = open('/home/rumbaugh/temp/idl_clusprops.%s.4.10.12.dat'%(names[i]),'w')
    if enoughBoR == 1:
        FILEidl1.write('%f %f %f %f'%(sig,blusig,redsig,BCGvel))
    else:
        FILEidl1.write('%f %f %f %f'%(sig,-1,-1,BCGvel))
    FILEidl1.close()
    FILEidl2 = open('/home/rumbaugh/temp/idl_vels.%s.4.10.12.dat'%(names[i]),'w')
