...
RGalPeakRA = np.array([201.20353640,201.09003360,201.20748930,(360/24.0)*(17+(57+(18.769/60))/60.0),275.3801,241.08946,241.09890,(360.0/24)*(9+(10+(4.168/60))/60.0),(360.0/24)*(9+(10+(47.686/60))/60.0)])
...
centerzs = np.array([0.76,0.69,0.691,0.69,0.82,0.89861,0.86531,1.1,1.1])
...
for i in range(0,len(names)):
    ...
    #while validBCGs < 3.4:
     #   isgood = 0
     #   si += 1
     #   gsi = FindCloseSources(RA[g[gall[grs[garg[si]]]]],Dec[g[gall[grs[garg[si]]]]],0.1,sRA[gs[galls[grss]]],sDec[gs[galls[grss]]],0)
    #    if len(gsi) > 0: 
     #       if ((sz[gs[galls[grss[gsi[0]]]]] > centerzs[i]-0.1) & (sz[gs[galls[grss[gsi[0]]]]] < centerzs[i]+0.1)): isgood = 1
...
#        pylab.ylabel("F606W-F814W")
    ...
    if (i < 4.5):
        ...
        if band == 'r-i':
            pylab.plot(np.arange(50),rsb[ccnt-1]-np.arange(50)*rsm[ccnt-1]-rsNSTD[ccnt-1]*rsSTD[ccnt-1],color='red')
            pylab.plot(np.arange(50),rsb[ccnt-1]-np.arange(50)*rsm[ccnt-1]+rsNSTD[ccnt-1]*rsSTD[ccnt-1],color='red')
        for i2 in range(0,len(gall)):
            gi2 = FindCloseSources(RA[g[gall[i2]]],Dec[g[gall[i2]]],0.1,LFCRA,LFCDec,0)
            if len(gi2) > 0:
                pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=8,color='black')
    print 'j'
...
#    print "%s - 4th Position\n(RA,Dec) = (%s,%s)\ni'/f814 magnitude: %f\n"%(names[i],rastrs[3],decstrs[3],iB[g[gall[grs[garg[validBCGIDs[3]]]]]])
    FILE = open('/home/rumbaugh/BCG.anal.%s.gal_list_%4.2fMpc.2.9.12.dat'%(names[i],aperture),'w')
    FILE2 = open('/home/rumbaugh/BCG.anal.%s.gals_%4.2fMpc.2.9.12.reg'%(names[i],aperture),'w')
    ...
    for iw in range(0,len(gblu)):
        ...
        isgood = 0
        regcol = 'blue'
        giw = FindCloseSources(RA[g[gall[gblu[iw]]]],Dec[g[gall[gblu[iw]]]],0.1,sRA[gs[galls[gblus]]],sDec[gs[galls[gblus]]],0)
        if len(giw) > 0: 
            if ((sz[gs[galls[gblus[giw[0]]]]] > centerzs[i]-0.01) & (sz[gs[galls[gblus[giw[0]]]]] < centerzs[i]+0.01)):
                ...
                if ((i != 5) & (i != 6)):
                    ...
                    gi2 = FindCloseSources(RA[g[gall[gblu[iw]]]],Dec[g[gall[gblu[iw]]]],0.1,LFCRA,LFCDec,0)
                    if len(gi2) > 0:
                        pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=12,color='red')
                ...
                if ((i != 5) & (i != 6)):
                    ...
                    gi2 = FindCloseSources(RA[g[gall[gblu[iw]]]],Dec[g[gall[gblu[iw]]]],0.1,LFCRA,LFCDec,0)
                    if len(gi2) > 0:
                        pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=12,color='green')
            else:
                isgood = 3
                regcol = 'cyan'
                if ((i != 5) & (i != 6)):
                    ...
                    gi2 = FindCloseSources(RA[g[gall[gblu[iw]]]],Dec[g[gall[gblu[iw]]]],0.1,LFCRA,LFCDec,0)
                    if len(gi2) > 0:
                        pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=12,color=html_teal)
    ...
    for iw in range(0,len(gars)):
        ...
        isgood = 0
        regcol = 'yellow'
        giw = FindCloseSources(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]],0.1,sRA[gs[galls[garss]]],sDec[gs[galls[garss]]],0)
        if len(giw) > 0: 
            if ((sz[gs[galls[garss[giw[0]]]]] > centerzs[i]-0.01) & (sz[gs[galls[garss[giw[0]]]]] < centerzs[i]+0.01)):
                ...
                if ((i != 5) & (i != 6)):
                    ...
                    gi2 = FindCloseSources(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]],0.1,LFCRA,LFCDec,0)
                    if len(gi2) > 0:
                        pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=12,color='red')
        ...
        if ((top4ar < 3.4) & (isgood < 2.2) & (isgood > 0.1)):
            if ((i == 5) | (i == 6)):
                ...
                if len(gi2) > 0:
                    if isgood > 0.1:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f,%6.4f}\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]]-2.5/60/60.0,rB[g[gall[gars[iw]]]],iB[g[gall[gars[iw]]]],zB[gi2[0]],sz[gs[galls[garss[giw[0]]]]]))
                    else:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f}\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]]-2.5/60/60.0,rB[g[gall[gars[iw]]]],iB[g[gall[gars[iw]]]],zB[gi2[0]]))
                else:
                    if isgood > 0.1:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%6.4f}\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]]-2.5/60/60.0,rB[g[gall[gars[iw]]]],iB[g[gall[gars[iw]]]],sz[gs[galls[garss[giw[0]]]]]))
                    else:
                ...
                if isgood > 0.1:
                    FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f,%6.4f}\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]]-2.5/60/60.0,rB[g[gall[gars[iw]]]],iB[g[gall[gars[iw]]]],zB[g[gall[gars[iw]]]],sz[gs[galls[garss[giw[0]]]]]))
                else:
                    FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f}\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]]-2.5/60/60.0,rB[g[gall[gars[iw]]]],iB[g[gall[gars[iw]]]],zB[g[gall[gars[iw]]]]))
            top4ar += 1
        if ((isgood == 1) | (isgood == 2)): 
            FILE2.write('point(%f,%f) # point=diamond color=%s\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]],regcol))
        else:
            FILE2.write('point(%f,%f) # point=x color=%s\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]],regcol))
        if isgood == 1: FILE2.write('point(%f,%f) # point=x color=%s\n'%(RA[g[gall[gars[iw]]]],Dec[g[gall[gars[iw]]]],regcol))
    top10 = 0
    top4 = 0
    ...
    for iw in range(0,len(garg)):
        ...
        isgood = 0
        regcol = 'green'
        giw = FindCloseSources(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],0.1,sRA[gs[galls[grss]]],sDec[gs[galls[grss]]],0)
        if len(giw) > 0: 
            if ((sz[gs[galls[grss[giw[0]]]]] > centerzs[i]-0.01) & (sz[gs[galls[grss[giw[0]]]]] < centerzs[i]+0.01)):
                ...
                if ((i != 5) & (i != 6)):
                    ...
                    gi2 = FindCloseSources(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],0.1,LFCRA,LFCDec,0)
                    if len(gi2) > 0:
                        pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=12,color='red')
        ...
        if ((top10 < 9.4) & (isgood < 2.2)):
            if ((i == 5) | (i == 6)):
                ...
                if len(gi2)> 0:
                    if isgood > 0.1:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f,%6.4f}\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]]-2.5/60/60.0,rB[g[gall[grs[garg[iw]]]]],iB[g[gall[grs[garg[iw]]]]],zB[gi2[0]],sz[gs[galls[grss[giw[0]]]]]))
                    else:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f}\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]]-2.5/60/60.0,rB[g[gall[grs[garg[iw]]]]],iB[g[gall[grs[garg[iw]]]]],zB[gi2[0]]))
                else:
                    if isgood > 0.1:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%6.4f}\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]]-2.5/60/60.0,rB[g[gall[grs[garg[iw]]]]],iB[g[gall[grs[garg[iw]]]]],sz[gs[galls[grss[giw[0]]]]]))
                    else:
                        FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f}\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]]-2.5/60/60.0,rB[g[gall[grs[garg[iw]]]]],iB[g[gall[grs[garg[iw]]]]]))
            else:
                if isgood > 0.01:
                    FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f,%6.4f}\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]]-2.5/60/60.0,rB[g[gall[grs[garg[iw]]]]],iB[g[gall[grs[garg[iw]]]]],zB[g[gall[grs[garg[iw]]]]],sz[gs[galls[grss[giw[0]]]]]))
                else:
                    FILE2.write('# text(%f,%f) color=magenta font="helvetica 10 bold" text={%4.1f,%4.1f,%4.1f}\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]]-2.5/60/60.0,rB[g[gall[grs[garg[iw]]]]],iB[g[gall[grs[garg[iw]]]]],zB[g[gall[grs[garg[iw]]]]]))
            top10 += 1
        if ((top4 < 3.4) & (isgood < 2.2)):
            ...
            if ((i != 5) & (i != 6)):
                ...
                gi2 = FindCloseSources(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],0.1,LFCRA,LFCDec,0)
                if len(gi2) > 0:
                    if top4 == 0: pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=50,marker='d',facecolors='none',color='red')
                    if top4 == 1: pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=50,marker='d',facecolors='none',color=html_brwn)
                    if top4 == 2: pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=50,marker='d',facecolors='none',color=html_teal)
                    if top4 == 3: pylab.scatter(CMDx[gi2[0]],CMDy[gi2[0]],s=50,marker='d',facecolors='none',color='blue')
                else:
                    print '\n\n%s\n%i\n\n'%(names[i],top4)
            top4 += 1
    ...
    FILE.close()
    FILE2.close()
    #pylab.savefig('/home/rumbaugh/CMD.%s.%s.2.9.12.png'%(names[i],band))
    pylab.close('all')
