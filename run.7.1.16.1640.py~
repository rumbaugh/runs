...
for field in reffile_dict.keys():
    ...
    zpeak=crpz['z_peak']+0.0041*(1+crpz['z_peak'])
    sigmaU,sigmaL=crpz['u68']-crpz['z_peak'],crpz['z_peak']-crpz['l68']
    zdummy=np.linspace(fzl,fzu,500)
    zstep=zdummy[1]-zdummy[0]
    PDFU=1./sigmaU.reshape((len(zpeak),1))/np.sqrt(2*np.pi)*np.exp(-(zdummy-zpeak.reshape((len(zpeak),1)))**2/(2*sigmaU.reshape((len(zpeak),1))**2))
    PDFL=1./sigmaL.reshape((len(zpeak),1))/np.sqrt(2*np.pi)*np.exp(-(zdummy-zpeak.reshape((len(zpeak),1)))**2/(2*sigmaL.reshape((len(zpeak),1))**2))
    zpeak_chk=zpeak.reshape((len(zpeak),1))*np.ones((len(zpeak),len(zdummy)))
    zdummy_chk=np.ones((len(zpeak),len(zdummy)))*zdummy
    chk=np.array(np.sign(zdummy_chk-zpeak_chk)+1,dtype='bool')
    PDFL[chk],PDFU[True-chk]=0,0
    P_inclus=np.sum(PDFU,axis=1)*zstep+np.sum(PDFL,axis=1)*zstep
    P_inclus[np.isnan(P_inclus)]=0
    np.savetxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s.P_inclus.dat'%(field,field,field),P_inclus)
