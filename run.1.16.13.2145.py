...
ltime = (ltime-ltime[0])
#find time delays
BA_delay_d2,BAmud2,BAdispd2 = calc_disp_delay(Aflux,Bflux,ltime,ltime,Aerr,Berr,maxtimestep,timestep,minmu,maxmu,mustep,'D_2',output=2,simplemuerr=True)
t1 = time.time()
print "33%% Done. Elapsed Time: %f seconds"%(t1-st1)
BC_delay_d2,BCmud2,BCdispd2 = calc_disp_delay(Cflux,Bflux,ltime,ltime,Cerr,Berr,maxtimestep,timestep,minmu,maxmu,mustep,'D_2',output=2,simplemuerr=True)
t2 = time.time()
print "66%% Done. Elapsed Time: %f seconds"%(t2-st1)
BD_delay_d2,BDmud2,BDdispd2 = calc_disp_delay(Dflux,Bflux,ltime,ltime,Derr,Berr,maxtimestep,timestep,minmu,maxmu,mustep,'D_2',output=2,simplemuerr=True)
if not justone:
...
FILE.write("\n\nB-A:\nD_2 delay: %4.1f days  mu: %6.4f   disp: %f\nD_4_2:"%(BA_delay_d2,BAmud2,BAdispd2))
pylab.plot(ltime,Aflux,color='blue',label='A')
pylab.plot(ltime+BA_delay_d2,Bflux*BAmud2,color='red',label='B')
pylab.legend()
pylab.savefig('/home/rumbaugh/BA_delay_fit.output.1.8.13.png')
pylab.close('all')
if not justone:
    for i in range(0,len(delta_arr)): 
...
FILE.write("\n\nB-C:\nD_2 delay: %4.1f days  mu: %6.4f   disp: %f\nD_4_2:"%(BC_delay_d2,BCmud2,BCdispd2))
pylab.plot(ltime,Cflux,color='blue',label='C')
pylab.plot(ltime+BC_delay_d2,Bflux*BCmud2,color='red',label='B')
pylab.legend()
pylab.savefig('/home/rumbaugh/BC_delay_fit.output.1.8.13.png')
pylab.close('all')
if not justone:
    for i in range(0,len(delta_arr)): 
...
FILE.write("\n\nB-D:\nD_2 delay: %4.1f days  mu: %6.4f   disp: %f\nD_4_2:"%(BD_delay_d2,BDmud2,BDdispd2))
pylab.plot(ltime,Dflux,color='blue',label='D')
pylab.plot(ltime+BD_delay_d2,Bflux*BDmud2,color='red',label='B')
pylab.legend()
pylab.savefig('/home/rumbaugh/BD_delay_fit.output.1.8.13.png')
pylab.close('all')
if not justone:
    for i in range(0,len(delta_arr)): 
