...
for source in sources:
    ...
    if source == '0414+573':
        ...
        #tdates = np.delete(tdates,gdel)
    FILE = open('/home/rumbaugh/EVLA/light_curves/VLA_lc_aipsredux_UVT_wgauss_5.20.14_%s.dat'%source,'w')
    for date in tdates:
        if date[1] == '.':
        ...
        if date not in tdates[gdel]:
            FILE.write('%f'%daytot)
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_UVT_wgauss_5.20.14.%s.fixpos.%s.mod'%(source,date),dtype='string',comments='!',usecols=(0,1,2))
