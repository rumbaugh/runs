...
device,file="/home/rumbaugh/RSoffset.hist.r-i.2.17.11.ps",/color
...
plot,[0,1],[0,1],/nodata,XTITLE='Red Sequence Offset',YTITLE='Num. of AGN',XRANGE=[-1.5,0.5],YRANGE=[0,4.1],/xstyle,/ystyle
HISTOPLOT,y_RSO,BINSIZE=0.125,MININPUT=-1.5,MAXINPUT=0.5,xstyle=1,ystyle=1,/LINE_FILL,polycolor='Indian Red',spacing=0.05,orientation=-45,locations=loc1,PROBABILITY=cum1,/oplot
HISTOPLOT,o_RSO,BINSIZE=0.125,MININPUT=-1.5,MAXINPUT=0.5,xstyle=1,ystyle=1,/LINE_FILL,polycolor='Charcoal',spacing=0.2,orientation=45,/oplot,locations=loc2,PROBABILITY=cum2

ySi = sort(y_RSO)
yS = y_RSO[ySi]
oSi = sort(o_RSO)
oS = o_RSO[oSi]
yF = dblarr(n_elements(yS))
for i=0,n_elements(yS)-1 do yF[i] = (i+1.0)/n_elements(yS)
o_F = dblarr(n_elements(oS))
for i=0,n_elements(oS)-1 do o_F[i] = (i+1.0)/n_elements(oS)

;plot,[0,1],[0,1],/nodata,XRANGE=[-1.5,0.5],YRANGE=[0,1.01]
;oplot,loc1,cum1
;oplot,loc2,cum2,linestyle=2
;oplot,yS,yF,PSYM=2
;oplot,oS,o_F,PSYM=4
