...
ltaudiff,lsigdiff=10**df.ltau-df.tau,10**df.lsig.values-df.sigma.values*np.sqrt(365)
...
plt.figure(1)
plt.clf()
plt.scatter(ltaudiffnorm,lsigdiffnorm,s=4,edgecolor=None,facecolor='r',color='r',alpha=0.1)
xlim=plt.xlim()
ylim=plt.ylim()
plt.axhline(0,color='k',lw=2,ls='dashed')
plt.axvline(0,color='k',lw=2,ls='dashed')
plt.ylabel(r'Macleod $\sigma$-CAR1 fit $\sigma$ (normalized) ')
plt.xlabel(r'Macleod $\tau$ (days)-CAR1 fit $\tau$ (days) (normalized)')
plt.savefig('/home/rumbaugh/S82_CARfit_test.6.22.17.tausig_diff_norm.png')
