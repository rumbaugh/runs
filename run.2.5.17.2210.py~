...
for i in range(0,1):
    ...
    if ((len(gred) > 9.5) & (len(gblu) > 9.8)):
        ...
        for j in range(0,ntrials):
            bluvels_t,redvels_t = np.zeros(len(gblu)),np.zeros(len(gred))
            for k in range(len(gblu)):
                trand = np.random.randint(0,len(vels))
                bluvels_t[k] = vels[trand]
            for k in range(len(gred)):
                trand = np.random.randint(0,len(vels))
                redvels_t[k] = vels[trand]
        ...
        gsbwlBS = np.where(sort_BWL_BS > np.fabs(blu_bwl-red_bwl))
        gsbwlBS = gsbwlBS[0]
