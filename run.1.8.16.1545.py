...
targets=np.array(['6932','6933+7343','9403+9840','9404+9836','10443+11999','10444+10924'])

obj_dict=dict(zip(targets,np.arange(len(targets))))
names=obj_dict.keys()
for i in range(0,len(targets)): 
    os.chdir('/home/rumbaugh/Chandra/%s'%names[i])
    cr = np.loadtxt("sources.%s.full+soft+hard.1e6.b1.1-16.wexp20.xsradecsigncnts.hdat"%names[i])
    ...
    sig,ncnts,ra,dec,ra_out,dec_out = np.zeros((np.shape(cr)[0],3)),np.zeros((np.shape(cr)[0],3)),np.zeros((np.shape(cr)[0],3)),np.zeros((np.shape(cr)[0],3)),np.zeros(np.shape(cr)[0]),np.zeros(np.shape(cr)[0])
    mask=cr[:,0]
    sig[:,0] = cr[:,7]
    sig[:,1] = cr[:,8]
    sig[:,2] = cr[:,9]
    ...
    for i in range(0,np.shape(cr)[0]):
        ...
        ra_out[i] = ra[i][g[0]]
        dec_out[i] = dec[i][g[0]]
        FILE.write('%f %f %i %f %f %f %f %f %f\n'%(ra_out[i], dec_out[i], mask[i], sig[i][0],sig[i][1],sig[i][2], ncnts[i][0], ncnts[i][1], ncnts[i][2]))
    FILE.close()
