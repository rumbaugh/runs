...
for field in targets[:-1]: 
    ...
    for j in range(0,len(crm['indX'])):
        ...
        if len(gz)==0:
            try:
                ...
                pass
        if field=='cl1324':
            gtmp=np.where((np.abs(mras[j]-all_ra)/np.cos(mdecs[j])<stol)&(np.abs(mdecs[j]-all_decs)<stol))[0]
            if len(gtmp)!=0: 
                disttmp=SphDist(mras[j],mdecs[j],all_ra[gtmp],all_decs[gtmp])
    ...
    for k in np.argsort(x_zoom_zs):
        ...
        if field=='cl0023': print 'Full - net counts: %6.1f  flux: %E\nSoft - net counts: %6.1f  flux: %E\nHard - net counts: %6.1f  flux: %E\n'%(pdata['Full_net_cts'][qcur>2.5][g_zs][k],pdata['Full_flux'][qcur>2.5][g_zs][k],pdata['Soft_net_cts'][qcur>2.5][g_zs][k],pdata['Soft_flux'][qcur>2.5][g_zs][k],pdata['Hard_net_cts'][qcur>2.5][g_zs][k],pdata['Hard_flux'][qcur>2.5][g_zs][k])
