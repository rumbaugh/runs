...
ltimemod = np.loadtxt('/local3/rumbaugh/EVLA/data/11A-138/difmap_results/B1938.LATE.obs_times.dat',dtype='float')
ltime = ltimemod[:,4].copy()
...
def make_lens_plot(infile,outfile,loc=1):
    ...
    timeticks = june10th_time+86400+86400*10*(np.arange(7)-1)
    timelabels = np.array(['6-01','6-11','6-21','7-1','7-11','7-21','7-31'])
    ax = pylab.axes()
    ...
    pylab.plot(ltime,np.ones(len(ltime))-offset,color='black',linestyle='dashed')
    pylab.xlim(4814250883-0.2*(4819371930-4814250883),4819371930)
    pylab.savefig(outfile + '.offset.png')
    pylab.close()
    return

make_lens_plot('indshift.fixpos.8.25.12','/local3/rumbaugh/EVLA/data/11A-138/difmap_results/B1938+666.fluxplot.indshift.fixpos.8.26.12',loc=8)
make_lens_plot('indshift.varpos.8.25.12','/local3/rumbaugh/EVLA/data/11A-138/difmap_results/B1938+666.fluxplot.indshift.varpos.8.26.12',loc=1)
make_lens_plot('meanshift.fixpos.8.25.12','/local3/rumbaugh/EVLA/data/11A-138/difmap_results/B1938+666.fluxplot.meanshift.fixpos.8.26.12',loc=2)
make_lens_plot('meanshift.varpos.8.25.12','/local3/rumbaugh/EVLA/data/11A-138/difmap_results/B1938+666.fluxplot.meanshift.varpos.8.26.12',loc=1)
