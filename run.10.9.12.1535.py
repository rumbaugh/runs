...
for i in range(0,1):
    ...
    for j in range(9,10):
        ...
        notice(0.3,8.0)
        set_model(1,powlaw1d.pb*xswabs.abs1)
        abs1.nh = nharr[i]
        freeze(abs1.nh)
        thaw(pb.ampl)
        set_stat('cash')
        ...
        fit(1)
        plot_fit(1)
        print_window("/home/rumbaugh/chips.plot.NEP5281_model_fit.grouped_20.bkg.10.9.12.ps",clob+)
        load_pha(2,str(namespec[i]))
        notice(0.3,8.0)
        set_model(2,xsraymond.rs*xswabs.abs1+powlaw1d.pb*xswabs.abs1)
        rs.kT = 3
        rs.redshift = rsarr[i]
        rs.Abundanc = 0.3
        rs.norm = 1.0
        ...
        projection(2)
        plot_fit(2)
        print_window("/home/rumbaugh/chips.plot.NEP5281_model_fit.grouped_20.src.10.9.12.ps",clob+)
