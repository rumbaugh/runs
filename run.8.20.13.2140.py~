...
FILE = open('/home/rumbaugh/ORELSE/ORELSE.Xray_combined_phot+matching.8.13.13.dat','w')
...
for field in fields:
    if field in ['Cl0849','Cl0910']:
        crp = np.loadtxt('/home/rumbaugh/ChandraData/photometry_files/%s.radec_sig_xerr.mod.dat'%(phot_cat_name_dict[field]))
    else:
    ...
    for i in range(0,np.shape(crp)[0]):
        ...
        if field in ['Cl0849','Cl0910']:
            FILE.write('%10s %9.5f %9.5f %5.2f %5.3E %5.3E %5.3E %6.1f %6.1f %6.1f %6.2f %6.2f %6.2f %2i %9.5f %9.5f %15s %6.3f %6.3f\n'%(field,float(cml[1]),float(cml[2]),float(cml[3]),cpl[3],0,0,0,0,0,0,0,cpl[2],int(cml[4]),float(cml[5]),float(cml[6]),cml[7],float(cml[8]),float(cml[20])))
        else:
