...
for curIDstep in steps_arr[:5]:
    ...
    MQquery='SELECT mq_rownum,coadd_objects_id from RUMBAUGH.MILLIQUAS_Y1A1_MATCH_ONLY WHERE coadd_objects_id in (%s)'%idsstr

    MDF=con.query_to_pandas(MQquery)
    ...
    MQrownum,MQcid=np.array(MDF['MQ_ROWNUM']),np.array(MDF['COADD_OBJECTS_ID'])
    ...
    for idcur,iid in zip(curIDs,np.arange(len(curIDs))):
        ...
        gmci=np.where(idcur==MQcid)[0]
        curMQ=MQrownum[gmci[0]]
        #print idcur
        #DBID=curIDstep+iid
        DBID=1000000+curMQ
