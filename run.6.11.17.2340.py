...
codedf=pd.read_csv('/home/rumbaugh/Chandra/ORELSE.test_codes.csv',index_col='cluster')
...
fields[np.in1d(fields,['cl1324_north','cl1324_south'])]='cl1324'

clusters_params_df=pd.DataFrame({'field':fields,'cluster':clusters,'temps':temps,'tubs':tubs,'tlbs':tlbs,'lums': lums[gdo],'lumerrs': lumerrs[gdo], 'Ezs':Ezs,'lumsEz': lums[gdo]/Ezs,'lumerrsEz': lumerrs[gdo]/Ezs, 'sigs':sigs,'sigerrs':sigerrs,'r0':r0,'r500':r500})
clusters_params_df=clusters_params_df.set_index(['cluster'])
clusters_params_df=pd.merge(clusters_params_df,codedf,on=['field'],left_index=True,right_index=True)
cut_params_df=clusters_params_df[(clusters_params_df.mean_SR_offset.values<1)&(clusters_params_df.mean_code.values<1)]

gcut=np.in1d(clusters_params_df.index.tolist(),cut_params_df.index.tolist(),invert=True)
...
outcr=np.zeros((len(temps),),dtype={'names':('field','cluster','mindistlit_LxT','mindistfit_LxT','mindistfit_iter_LxT','mindistlit_sigT','mindistfit_sigT','mindistfit_iter_sigT','mindistlit_Lxsig','mindistfit_Lxsig','mindistfit_iter_Lxsig'),'formats':('|S24','|S24','f8','f8','f8','f8','f8','f8','f8','f8','f8')})
...
LxTdata_iter=odr.Data(np.log(cut_params_df.temps),np.log(cut_params_df.lumsEz),wd=(0.5*(cut_params_df.tubs-cut_params_df.tlbs)/cut_params_df.temps)**-2,we=(cut_params_df.lumerrs/cut_params_df.lums)**-2)
odrLxT=odr.ODR(LxTdata,linearLxT,beta0=[2.6,np.log(0.1)])
odrLxT_iter=odr.ODR(LxTdata_iter,linearLxT,beta0=[2.6,np.log(0.1)])
LxTout=odrLxT.run()
LxTout_iter=odrLxT_iter.run()

sreg=skLR(np.log(temps),np.log(lums[gdo]/Ezs))
...
plt.scatter(temps[gcut],lumsEz[gcut],s=32,color='cyan',marker='x',zorder=20)
xlim=plt.xlim()
ylim=plt.ylim()
...
plt.loglog(xdummy,ydummy_lit,lw=2,color='cyan',ls='-.')
plt.loglog(xdummy,ydummy_lit2,lw=2,color='purple')#,ls='-.')
plt.loglog(xdummy,ydummy_fit,lw=2,color='magenta',ls='dotted')
plt.loglog(xdummy,ydummy_fit_iter,lw=2,color='green',ls='dashed')
plt.loglog(xdummy, np.e**sreg.predict(np.log(xdummy)[:,np.newaxis]), color='orange',linewidth=3)
...
plt.savefig('/home/rumbaugh/Chandra/plots/scaling_relations_sktest.Lx-T.6.11.17.png')
xdummy_tmp=np.copy(xdummy)
mindistlit,mindistfit,mindistfit_iter=calc_SR_dists(xdummy,ydummy_lit_4func,temps,lumsEz*10**(-44),terrhi,lumEzerrs*10**(-44),Xerrlo=terrlo),calc_SR_dists(xdummy,ydummy_fit,temps,lumsEz,terrhi,lumEzerrs,Xerrlo=terrlo),calc_SR_dists(xdummy,ydummy_fit_iter,temps,lumsEz,terrhi,lumEzerrs,Xerrlo=terrlo)

outcr['mindistlit_LxT'],outcr['mindistfit_LxT'],outcr['mindistfit_iter_LxT']=mindistlit,mindistfit,mindistfit_iter
...
sigTdata_iter=odr.Data(np.log(cut_params_df.temps),np.log(cut_params_df.sigs),we=(cut_params_df.sigerrs/cut_params_df.sigs)**-2,wd=(0.5*(cut_params_df.tubs-cut_params_df.tlbs)/cut_params_df.temps)**-2)
odrsigT=odr.ODR(sigTdata,linearsigT,beta0=[0.65,np.log(10**2.49)])
odrsigT_iter=odr.ODR(sigTdata_iter,linearsigT,beta0=[0.65,np.log(10**2.49)])
sigTout=odrsigT.run()
sigTout_iter=odrsigT_iter.run()

sreg=skLR(np.log(temps),np.log(sigs))
...
plt.scatter(temps[gcut],sigs[gcut],s=32,color='cyan',marker='x',zorder=20)
xlim=plt.xlim()
ylim=plt.ylim()
xdummy=np.linspace(np.max([xlim[0],0.7]),xlim[1],dummypoints)
ydummy_lit=10**2.49*xdummy**0.65
ydummy_fit=np.e**sigTout.beta[1]*xdummy**sigTout.beta[0]
ydummy_fit_iter=np.e**sigTout_iter.beta[1]*xdummy**sigTout_iter.beta[0]
...
plt.savefig('/home/rumbaugh/Chandra/plots/scaling_relations_sktest.sig-T.6.11.17.png')

mindistlit,mindistfit,mindistfit_iter=calc_SR_dists(xdummy,ydummy_lit,temps,sigs,terrhi,sigerrs,Xerrlo=terrlo),calc_SR_dists(xdummy,ydummy_fit,temps,sigs,terrhi,sigerrs,Xerrlo=terrlo),calc_SR_dists(xdummy,ydummy_fit_iter,temps,sigs,terrhi,sigerrs,Xerrlo=terrlo)

outcr['mindistlit_sigT'],outcr['mindistfit_sigT'],outcr['mindistfit_iter_sigT']=mindistlit,mindistfit,mindistfit_iter


lumXW=lums[gdo]/(1.-1./np.sqrt(1+(r500/r0)**2))
lumXWerr=lumerrs[gdo]/(1.-1./np.sqrt(1+(r500/r0)**2))

clusters_params_df['lumXW'],clusters_params_df['lumXWerr']=clusters_params_df.lums/(1.-1./np.sqrt(1+(clusters_params_df.r500/clusters_params_df.r0)**2)),clusters_params_df.lumerrs/(1.-1./np.sqrt(1+(clusters_params_df.r500/clusters_params_df.r0)**2))
cut_params_df=clusters_params_df[(clusters_params_df.mean_SR_offset.values<1)&(clusters_params_df.mean_code.values< 1)]
...
sigLxdata_iter=odr.Data(np.log(cut_params_df.sigs),np.log(cut_params_df.lumXW/cut_params_df.Ezs),wd=(cut_params_df.sigerrs/cut_params_df.sigs)**-2,we=(cut_params_df.lumXWerr/cut_params_df.lumXW)**-2)
odrsigLx=odr.ODR(sigLxdata,linearsigLx,beta0=[5.30,np.log(10**(42-12.9))])
odrsigLx_iter=odr.ODR(sigLxdata_iter,linearsigLx,beta0=[5.30,np.log(10**(42-12.9))])
sigLxout=odrsigLx.run()
sigLxout_iter=odrsigLx_iter.run()

sreg=skLR(np.log(sigs),np.log(lumXW/Ezs))
...
plt.scatter(sigs[gcut],lumXW[gcut]/Ezs[gcut],s=32,color='cyan',marker='x',zorder=20)
xlim=plt.xlim()
ylim=plt.ylim()
...
ydummy_fit_iter=np.e**sigLxout_iter.beta[1]*xdummy**sigLxout_iter.beta[0]
plt.loglog(xdummy,ydummy_lit,lw=2,color='b')
plt.loglog(xdummy,ydummy_fit,lw=2,color='magenta',ls='dotted')
plt.loglog(xdummy,ydummy_fit_iter,lw=2,color='green',ls='dashed')
plt.loglog(xdummy, np.e**sreg.predict(np.log(xdummy)[:,np.newaxis]), color='orange',linewidth=3)
...
plt.ylabel(r'$L_x\ E(z)^{-1}$ ergs s$^{-1}$')
plt.savefig('/home/rumbaugh/Chandra/plots/scaling_relations_sktest.sig-Lx.6.11.17.png')

mindistlit,mindistfit,mindistfit_iter=calc_SR_dists(xdummy,ydummy_lit_4func,sigs,lumsEz*10**(-44),sigerrs,lumEzerrs*10**(-44)),calc_SR_dists(xdummy,ydummy_fit,sigs,lumsEz,sigerrs,lumEzerrs),calc_SR_dists(xdummy,ydummy_fit_iter,sigs,lumsEz,sigerrs,lumEzerrs)

outcr['mindistlit_Lxsig'],outcr['mindistfit_Lxsig'],outcr['mindistfit_iter_Lxsig']=mindistlit,mindistfit,mindistfit_iter
outcr['field'],outcr['cluster']=fields,clusters
np.savetxt('/home/rumbaugh/Chandra/ORELSE.scaling_relation_offsets.tab',outcr,header='Field Cluster mindistlit_LxT mindistfit_LxT mindistfit_iter_LxT mindistlit_sigT mindistfit_sigT mindistfit_iter_sigT mindistlit_Lxsig mindistfit_Lxsig mindistfit_iter_Lxsig',fmt='%12s %12s %f %f %f %f %f %f %f %f %f')
