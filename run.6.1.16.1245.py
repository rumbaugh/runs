...
for i in range(0,len(crx['RA'])):
    ...
    if (((zx<spec_dict[field]['z'][1])|(zx>spec_dict[field]['z'][2]))&(zx>0.68)&(zx<1.11)):
        CKPgrouparr[0][2]=int(CKPgrouparr[0][2])+1
        if zx<0.8:
            CKPgrouparr[1][2]=int(CKPgrouparr[1][2])+1
        elif zx>0.96:
            CKPgrouparr[3][2]=int(CKPgrouparr[3][2])+1
        else:
            CKPgrouparr[2][2]=int(CKPgrouparr[2][2])+1
        gfAGN_arr[i]=True
        ...
        if ((len(gnoz)>0)&(zx>0.01)):
            ...
            if len(gCKP)>1: 
                AGN_CKP_arr[i]=1
                CKPgrouparr[0][1]=int(CKPgrouparr[0][1])+1
                if zx<0.8:
                    CKPgrouparr[1][1]=int(CKPgrouparr[1][1])+1
                elif zx>0.96:
                    CKPgrouparr[3][1]=int(CKPgrouparr[3][1])+1
                else:
                    CKPgrouparr[2][1]=int(CKPgrouparr[2][1])+1
...
for field,ifld in zip(targets[gzlist],np.arange(len(gzlist))): 
    ...
    for i in range(0,len(gq)):
        zx=crs['z'][gq[i]]
        CKPgrouparr[0][4]=int(CKPgrouparr[0][4])+1
        if zx<0.8:
            CKPgrouparr[1][4]=int(CKPgrouparr[1][4])+1
        elif zx>0.96:
            CKPgrouparr[3][4]=int(CKPgrouparr[3][4])+1
        else:
            CKPgrouparr[2][4]=int(CKPgrouparr[2][4])+1
        ...
        if len(gKDC)>1: 
            tmpall_CKP_arr[i]=1
            CKPgrouparr[0][3]=int(CKPgrouparr[0][3])+1
            if zx<0.8:
                CKPgrouparr[1][3]=int(CKPgrouparr[1][3])+1
            elif zx>0.96:
                CKPgrouparr[3][3]=int(CKPgrouparr[3][3])+1
            else:
                CKPgrouparr[2][3]=int(CKPgrouparr[2][3])+1
...
for i in range(0,4): FILE.write('%12s %i %i %i %i\n'%(CKPgrouparr[i][0],CKPgrouparr[i][1],CKPgrouparr[i][2],CKPgrouparr[i][3],CKPgrouparr[i][4]))
FILE.close()
...
lA,lB,lC,lD=np.log10(crl['lum_full'][(((crl['field']=='cl1350')|(crl['field']=='rxj1221')|(crl['field']=='rxj1757')|(crl['field']=='rxj1821')|(crl['field']=='rcs0224')))]),np.log10(crl['lum_full'][(((crl['field']=='cl1324')|(crl['field']=='cl1324_north')|(crl['field']=='cl1324_south')|(crl['field']=='rxj1716')))]),np.log10(crl['lum_full'][(((crl['field']=='cl0023')|(crl['field']=='cl1604')))]),np.log10(crl['lum_full'][(((crl['field']=='rxj1053')|(crl['field']=='cl0849')|(crl['field']=='rxj0910')))])
...
plt.legend()
plt.savefig('/home/rumbaugh/Chandra/plots/lum_hist.group_comps.6.1.16.png')
...
plt.ylabel('Number of Sources')
plt.legend()
plt.savefig('/home/rumbaugh/Chandra/plots/lum_hist.field_comp.6.1.16.png')
