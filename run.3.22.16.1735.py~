...
allzdf=plt.hist(allz4hist,color='b',alpha=0.3,range=(0.6,1.3),bins=7)
plt.xlabel('Redshift')
plt.ylabel('Number of Sources')
...
MCout,KSout=MCRandFromHist(allzdf,lums4hist,allz4hist,num=len(lums4hist),ntrials=1000,doKStest=True,KScomp=np.log10(crn['lum_full'][crn['lum_full']>0]))
...
plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
a=plt.hist(np.log10(crn['lum_full'][crn['lum_full']>0]),color='r',range=(42,45),bins=8)
b=plt.hist(MCout.flatten(),color='b',alpha=0.3,range=(42,45),bins=8,weights=np.ones(np.shape(MCout)[0]*np.shape(MCout)[1])*1./np.shape(MCout)[0])
plt.xlabel('X-ray Luminosity')
plt.ylabel('Number of Sources')
plt.savefig('/home/rumbaugh/Chandra/plots/lumhist_MC.3.15.16.png')

k1,k2=KStest(np.log10(crn['lum_full'][crn['lum_full']>0]),MCout.flatten())
print k1,k2
