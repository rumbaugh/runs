...
FILE = open('paper.centers_table.3.21.12.dat','w')
for i in range(0,9):
    ...
    meanRA /= weighttot
    meanDec /= weighttot
    FILEr = open('/home/rumbaugh/paper_marks.%s.3.21.12.reg'%(names[i]),'w')
    ...
    distLW[i] = SphDist(centerRAs[i],centerDecs[i],meanRA,meanDec)*srchdist[i]
    distBCG[i] = SphDist(centerRAs[i],centerDecs[i],BCGRA[i],BCGDec[i])*srchdist[i]
    distRGP[i] = SphDist(centerRAs[i],centerDecs[i],RGalPeakRA[i],RGalPeakDec[i])*srchdist[i]
    ...
    FILEr.write('point(%f,%f) # point=circle color=cyan\n'%(BCGRA[i],BCGDec[i]))
    FILEr.write('point(%f,%f) # point=x color=blue\n'%(meanRA,meanDec))
    FILEr.write('point(%f,%f) # point=diamond color=magenta\n'%(RGalPeakRA[i],RGalPeakDec[i]))
    FILE.write('%13s & %3s %s %s & %2s %s %s & %5.3f & %3s %s %s & %2s %s %s & %5.3f & %3s %s %s & %2s %s %s & %5.3f'%(names[i],bcgrahstr,bcgramstr,bcgrasstr,bcgdecdstr,bcgdecmstr,bcgdecsstr,distBCG[i],rgrahstr,rgramstr,rgrasstr,rgdecdstr,rgdecmstr,rgdecsstr,distRGP[i],lwrahstr,lwramstr,lwrasstr,lwdecdstr,lwdecmstr,lwdecsstr,distLW[i]))
    if i < 8: FILE.write(' \\\\\n')
    FILEr.close()
FILE.close()
