...
RSOtot,lumtot=np.zeros(0),np.zeros(0)
for field,jf in zip(plottargets,np.arange(len(plottargets))):
    ...
    RSOtot,lumtot=np.append(RSOtot,RSO[gf]),np.append(lumtot,np.log10(crn['lum_full'][gAGN][gf]))
...
gr3=np.where((RSOtot<-1)&(RSOtot>-3)&(lumtot<43.3)&(lumtot>42.5))[0]
