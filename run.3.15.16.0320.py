...
psfpdf=bpdf.PdfPages('/home/rumbaugh/Chandra/plots/diffhist.supercolors_vs_rfU-B_nozcorr.3.15.16.pdf')
date='3.12.16'

adamcorrvar=-1.3975614258700002
...
for field in ['cl0023','rxj1716']:
    ...
    r,i,rerr,ierr,redshift,gCMD=r[((redshift>0.7)&(redshift<1.1))],i[((redshift>0.7)&(redshift<1.1))],rerr[((redshift>0.7)&(redshift<1.1))],ierr[((redshift>0.7)&(redshift<1.1))],redshift[((redshift>0.7)&(redshift<1.1))],gCMD[((redshift>0.7)&(redshift<1.1))]
    ...
    mRFU[RFU>0],mRFB[RFB>0]=-2.5*np.log10(RFU[RFU>0])+25-DM[gdls]-0*2.5*np.log10(1+redshift),-2.5*np.log10(RFB[RFB>0])+25-DM[gdls]-0*2.5*np.log10(1+redshift)
    ...
    ax.set_xlim(-2,2)
    fig.savefig(psfpdf,format='pdf')
    print np.median(r-mRFU),np.median(i-mRFB)
    ...
    plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
    ax=fig.add_subplot(1,1,1)
    ax.scatter(redshift,r-mRFU,color='r',s=3)
    ax.set_ylabel("Magnitude Difference")
    ax.set_xlabel("Redshift")
    ax.set_title('%s (m_blue - restframe U) vs. Redshift'%field)
    ax.set_xlim(0.69,1.11)
    #ax.set_ylim(0,1)
    fig.savefig(psfpdf,format='pdf')
    fig.clf()
    plt.rc('axes',linewidth=2)
    ...
    plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
    ax=fig.add_subplot(1,1,1)
    ax.scatter(redshift,i-mRFB,color='r',s=3)
    ax.set_ylabel("Magnitude Difference")
    ax.set_xlabel("Redshift")
    ax.set_title('%s (m_red - restframe B) vs. Redshift'%field)
    ax.set_xlim(0.69,1.11)
    #ax.set_ylim(0,1)
    fig.savefig(psfpdf,format='pdf')
