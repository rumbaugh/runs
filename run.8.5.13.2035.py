...
def delaylnprob(x,A,B,A_err,B_err,ltime,t_grid,maxtime=60,mintime=-60,t_grid_spacing=1.,smooth_param=smooth_param):
    ...
    if ((x[0] > maxtime) | (x[0] < mintime)):
        ...
        smB,smB_var = sm.boxcar(ltime+x[0],B*x[1],t_grid[gltime],smooth_param,y_var=x[1]*x[1]*B_err*B_err)
...
C_max = 78.
smC,smC_var = sm.boxcar(ltime,Cflux,t_grid_C,smooth_param,y_var=Cerr*Cerr)
...
py.savefig('/home/rumbaugh/B1938+666_files/B1938.emcee_output_2D.BC_delay.%s.boxcar.ps'%date)
py.close()
...
py.savefig('/home/rumbaugh/B1938+666_files/B1938.emcee_output_hist_tau.BC_delay.%s.boxcar.ps'%date)
py.close()
py.plot(sampler.flatchain[:,0])
py.savefig('/home/rumbaugh/B1938+666_files/B1938.emcee_output_chain_plot_tau.BC_delay.%s.boxcar.ps'%date)
py.close()
py.scatter(sampler.flatchain[:,0],sampler.flatlnprobability)
py.savefig('/home/rumbaugh/B1938+666_files/B1938.emcee_output_tau-prob_scatter.BC_delay.%s.boxcar.ps'%(date))
py.close()
