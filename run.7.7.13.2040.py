...
for mask in ['0435_slit2','0435_slit3']:
	for color in ['blue','red']:
		for image in files_dict[mask][color]['images']:
			temp_dict = copy.deepcopy(files_dict[mask])
			temp_dict[color]['images'] = np.array([image])
			if not (((color == 'blue') & (image in [250,251,256,257])) | ((color == 'red') & (image in [245,251]))): lris_reduce_longslit(indir_dict[mask],outdir_dict[mask],files_dict[mask]['prefix'],temp_dict,mask,colors=color,doCR=True)
