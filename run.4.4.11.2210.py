...
for i in range(4,5):
    ...
    if i == 0:
        ...
        leg = pylab.legend(('z < 0.68','0.68 < z < 0.725','0.725 < z < 0.765','z > 0.765'),loc=2,markerscale=0.8,labelspacing=0.3)
    ...
    if i == 2:
        ...
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color=html_teal)
        g = np.where((sz[gsq[gsz]] >= 0.845) & (sz[gsq[gsz]] < 0.8485))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=13,color=html_orng)
        leg = pylab.legend(('z < 0.832','0.832 < z < 0.841','z > 0.841'),loc=2,markerscale=0.8,labelspacing=0.3)
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if i == 3:
        ...
        g = np.where((sz[gsq[gsz]] >= 0.922) & (sz[gsq[gsz]] < 0.925))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color='purple')
        leg = pylab.legend(('z < 0.857','0.857 < z < 0.885','0.885 < z < 0.91','0.91 < z < 0.93','0.93 < z < 0.942','z > 0.942'),loc=2,markerscale=0.8,labelspacing=0.3)
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if i == 4:
        #pylab.xlim(269.725,269.125)
        g = np.where(sz[gsq] < 0.9)
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=12,color=html_purp)
        g = np.where((sz[gsq] >= 0.9) & (sz[gsq] < 1.05))
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=12,color='blue')
        g = np.where(sz[gsq] >= 1.05)
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=12,color=html_teal)
        ...
        leg = pylab.legend(('z < 0.9','0.9 < z < 1.05','z > 1.05'),loc=2,markerscale=0.8,labelspacing=0.3)
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if ((i == 1) | (i > 33.1)) : pylab.scatter(sRA[gsq[gsz]],sDec[gsq[gsz]],s=12)
    ...
    for j in range(0,len(gmsz)):
        ...
        AGNstr = str(1+j)
        pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red')  
    FILE.close()
    pylab.title('%s'%(names[i]))
    pylab.xlabel('Right Ascension (J2000)')
    pylab.ylabel('Declination (J2000)')
    savfile = '/home/rumbaugh/paperstuff/test.AGN.spatmap.color.%s.png'%(names[i])
