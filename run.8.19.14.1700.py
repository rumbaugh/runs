...
for source in ['B0712']:
    ...
    if source == 'B0712':
        ...
        tfluxA1,tfluxA2,tfluxB,tAerr1,tAerr2,tBerr,days = tfluxA1[gB],tfluxA2[gB],tfluxB[gB],tAerr1[gB],tAerr2[gB],tBerr[gB],days[gB]
        ltime = np.append(days,ltime+1000)
        mtemp = np.mean(fluxB[g])/np.mean(tfluxB[gB])
        fromEVLA = np.append(np.zeros(len(tfluxA1)),np.ones(len(fluxA)))
    ...
    tau_init,mu_init,mu2_init = 0.,np.mean(fluxA[g])/np.mean(fluxB[g]),np.mean(fluxA[g])/np.mean(fluxB[g])
    ndim,nwalkers = 3,10
    maxtime,mintime = 60.,-60.
    p0 = np.zeros((nwalkers,ndim))
    p0[:,0],p0[:,1],p0[:,2] = np.ones(nwalkers)*tau_init+np.random.normal(scale=0.1,size=nwalkers),np.ones(nwalkers)*mu_init+np.random.normal(scale=0.01,size=nwalkers),np.ones(nwalkers)*mu_init+np.random.normal(scale=0.01,size=nwalkers)
    sampler = emcee.EnsembleSampler(nwalkers,ndim,delaylnprob,args=[fluxA[g],fluxB[g],Aerr[g],Berr[g],ltime[g],mu_init,mintime,maxtime,False,True,True,kernel,10.,0.5,seasonbounds,gEVLA])
    pos,prob,state=sampler.run_mcmc(p0,100)
    sampler.reset()
    pos,prob,state=sampler.run_mcmc(pos,runs)
    if runs < 50: 
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/test.%s_jointfit.emcee_output_full_chain_%s_bs.%s.dat'%(lens,kernel,date),'w')
    else:
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/%s_jointfit_difmus.emcee_output_full_chain_%s_bs.%s.dat'%(lens,kernel,date),'w')
    for i in range(0,runs):
        for j in range(0,nwalkers):
            FILE.write('%f %f %f\n'%(sampler.chain[j,i,0],sampler.chain[j,i,1],sampler.chain[j,i,2]))
    FILE.close()
    plt.clf()
    ...
    plt.hist(sampler.chain[j,i,0],nbins=40)
    plt.xlabel('Time Delay (days)',fontsize=14)
    plt.title('%s'%lens)
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s_jointfit.tau_hist_plot.interp_bs.%s.png'%(lens,date))
