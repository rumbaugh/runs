...
cr = read_file("/home/rumbaugh/COSMOS/conv.centers.12.29.11.dat")
...
texp= 0.0
FILE = open("/home/rumbaugh/COSMOS/lens.flux.analysis.1.29.11.dat","w")
avglum = 0.0
avglumcut = 0.0
cnterrsum = 0.0
for i in range(0,len(ID)):
    ...
    if i < len(ID)-1:
        cnts_bg = calc_data_sum2d('circle(%f,%f,110)-circle(%f,%f,90)'%(cenX[i],cenY[i],cenX[i],cenY[i]))
        ncnts = cnts_inner - cnts_bg*(80*80)/(110*110-1.0*90*90)
        cestemp = (cnts_inner**2+cnts_bg**2*(80*80)/(110*110-1.0*90*90))/(exps[i]**2)
    else:
    ...
    if i == 3:
        cnts_inner = calc_data_sum2d('circle(%f,%f,60)'%(cenX[i],cenY[i]))
        cnts_bg = calc_data_sum2d('circle(4388.7144,4462.0777,60)')
        ncnts = cnts_inner - cnts_bg
    if i == 7:
        cnts_inner = calc_data_sum2d('circle(%f,%f,60)'%(cenX[i],cenY[i]))
        cnts_bg = calc_data_sum2d('circle(4469.7732,4309.7301,60)')
        ncnts = cnts_inner - cnts_bg*(60*60)/(60*60)
    if i == len(ID)-3:
        cnts_inner = calc_data_sum2d('circle(%f,%f,120)'%(cenX[i],cenY[i]))
        cnts_bg = calc_data_sum2d('circle(%f,%f,150)-circle(%f,%f,140)'%(cenX[i],cenY[i],cenX[i],cenY[i]))
        ncnts = cnts_inner - cnts_bg*(120*120)/(150*150-1.0*140*140)
    cnterrsum += (cnts_inner**2+cnts_bg**
    cntrt = ncnts/exps[i]
