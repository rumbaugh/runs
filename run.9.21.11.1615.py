...
pylab.savefig('/home/rumbaugh/paperstuff/LxT.plot.bol.9.21.11.png')
pylab.close('all')
...
pylab.savefig('/home/rumbaugh/paperstuff/sigLx.plot.9.21.11.png')
pylab.close('all')
...
pylab.ylim(300,1500)
pylab.loglog(lineX,lineY)
pylab.errorbar(Temps2, sigma2, xerr=[TerrD2,TerrU2], yerr=[sigerr2,sigerr2], fmt='ro')
pylab.xlabel('T (keV)')
pylab.ylabel('Vel. Dispersion (km/s)')
for i in range(0,len(Temps2)): 
    if i == 0: 
        pylab.text(Temps2[i]+0.2,sigma2[i]+40,names2[i],color='black',weight='extra bold')
    elif i == 2:
        pylab.text(Temps2[i]+0.1,sigma2[i]-50,names2[i],color='black',weight='extra bold')
    elif i == 3:
        pylab.text(Temps2[i]-1,sigma2[i]+10,names2[i],color='black',weight='extra bold')
    elif i == 7:
        pylab.text(Temps2[i]-0.7,sigma2[i]+10,names2[i],color='black',weight='extra bold')
    elif i == 8:
        pylab.text(Temps2[i]+0.1,sigma2[i]+13,names2[i],color='black',weight='extra bold')
    else:
        pylab.text(Temps2[i],sigma2[i],names2[i],color='black',weight='extra bold')
pylab.savefig('/home/rumbaugh/sigT.plot.9.21.11.png')
pylab.close('all')
