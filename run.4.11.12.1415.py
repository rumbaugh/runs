...
for i in range(0,len(names)):
    ...
    #print np.sort(Dec[g[gRGP[gRGPtemp]]])
    ...
    if names[i] != "RXJ1821": 
        ...
        #if names[i] == "Cl1604A":
            #for ic in range(0,len(gRGPtemp)):
                #print '%s %f %f %f %f'%(sID[gq[gRGP2[gRGPtemp[ic]]]],RA[gq[gRGP2[gRGPtemp[ic]]]],Dec[gq[gRGP2[gRGPtemp[ic]]]],z[gq[gRGP2[gRGPtemp[ic]]]],vels[gq[gRGP2[gRGPtemp[ic]]]])
    delta = np.zeros(len(gRGPtemp))
    if skipMC == 0:
        Del,P,delta = DStest(RAnew,Decnew,znew,vel_in=vels[gRGPtemp],reps=100000)
        if writeMC == 1:
    ...
    if names[i] == 'Cl1604A':
        ...
        pylab.xlim(241.152760150518022,241.03)
        #pylab.ylim(43.06,43.115)
        pylab.ylim(43.01,43.115)
    ...
    plotcircle(RAcen=RGalPeakRA[i],DECcen=RGalPeakDec[i],rad=srchdist[i]/60,lstyle='dotted')
    plotcircle(RAcen=241.08946,DECcen=43.07613,rad=srchdist[i]/60,pcolor='red')
    if names[i] == "Cl1604A": pylab.savefig('/home/rumbaugh/testDSplot.%s.4.11.12.png'%(names[i]))
    pylab.close('all')
    #print 'd'
    #nep5281newcnt = 0
