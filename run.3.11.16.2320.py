...
psfpdf=bpdf.PdfPages('/home/rumbaugh/Chandra/plots/diffhist.supercolors_vs_rfU-B.3.12.16.pdf')
date='3.12.16'
...
for field in ['cl0023','rxj1716']:
    ...
    mRFU[RFU>0],mRFB[RFB>0]=-2.5*np.log10(RFU[RFU>0]),-2.5*np.log10(RFB[RFB>0])
    ...
    ax.set_title('%s (m_red-m_blue vs rest-frame U-B)'%field)
    ax.set_xlim(-1,1)
    #ax.set_ylim(0,1)
    ...
    plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
    ax=fig.add_subplot(1,1,1)
    ax.hist(r-mRFU,color='r')#,range=(-1,1),bins=20)
    ax.set_xlabel("Magnitude Difference")
    ax.set_ylabel("Number of Sources")
    ax.set_title('%s (m_red - restframe U)'%field)
    #ax.set_xlim(-1,1)
    #ax.set_ylim(0,1)
    fig.clf()
    plt.rc('axes',linewidth=2)
    ...
    plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
    ax=fig.add_subplot(1,1,1)
    ax.hist(i-mRFB,color='r')#,range=(-1,1),bins=20)
    ax.set_xlabel("Magnitude Difference")
    ax.set_ylabel("Number of Sources")
    ax.set_title('%s (m_blue - restframe B)'%field)
    #ax.set_xlim(-1,1)
    #ax.set_ylim(0,1)
    fig.savefig(psfpdf,format='pdf')
