...
for source in ['B0712']:
    ...
    if source == 'B0712':
        ...
        Aerr,Berr = np.append(2*np.sqrt(tAerr1**2+tAerr2**2),Aerr),2*np.append(tBerr,Berr)
    ...
    if runs < 50: 
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/test.%s_jointfit.emcee_output_full_chain_interp_bs.2Xerr.%s.dat'%(lens,date),'w')
    else:
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/%s_jointfit.emcee_output_full_chain_interp_bs.2Xerr.%s.dat'%(lens,date),'w')
