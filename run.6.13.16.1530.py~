...
for target,it in zip(targets,np.arange(0,len(targets))):
    ...
    Xout,Xout_obs,outscat=Xll.calc_lum_lims(100000)
    for band,ib in zip(['soft','hard','full'],np.arange(0,3)):
        ...
        outfile='/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s.X-ray_lum_limit_scatter.%s.dat'%(target,target,target,band,date)
        np.savetxt(outfile,outscat[ib])
