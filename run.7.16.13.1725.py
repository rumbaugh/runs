...
for mask in ['0435_slit1','0435_slit2','0435_slit3']:
    for color in ['blue','red']:
        for side in ['top','bottom']:
            ...
            dataList = []
            i = 0
            if ((mask == '0435_slit3') | ((side == 'bottom') & ((mask == '0435_slit1') | ((mask == '0435_slit2') & (color == 'red'))))):
                imgtmp = [files_dict[mask][color]['images'][0]]
            else:
                imgtmp = files_dict[mask][color]['images']
            for img in imgtmp:
                ...
                if i == 0: 
                    ...
                    if ((mask == '0435_slit2') & (color == 'blue') & (side == 'bottom')): scicoadd,varcoadd = numpy.zeros((np.shape(var)[0]+int(numpy.fabs(shift)),np.shape(var)[1])),numpy.zeros((np.shape(var)[0]+int(numpy.fabs(shift)),np.shape(var)[1]))
                for row in range(0,np.shape(varcoadd)[0]):
                    if ((mask == '0435_slit2') & (color == 'blue') & (side == 'bottom')):
                        if i == 0:
                            if row < np.shape(var)[0]-100: 
                                scicoadd[row] += sci[row]
                                varcoadd[row] += 1./var[row]
                        else:
                            if (row > shift): 
                                scicoadd[row] += sci[row+shift]
                                varcoadd[row] += 1./var[row+shift]
                    elif ((i == 1) & (shift > 0)):
                        scicoadd[row] += sci[row+shift]
                        varcoadd[row] += 1./var[row+shift]
                    elif ((i == 0) & (shift < 0)):
            ...
            newfile = pyfits.PrimaryHDU(data=1./varcoadd,header=header)
            newfile.writeto(outname,clobber=True)
