...
for i in range(0,len(cr)):
    ...
    if ((cr['Nsb'][i]>=10)&(cr['Nsr'][i]>=10)):
        Tarr[i][3:]=np.array(['%.0f+/-%.0f'%(cr['sigblue'][i],cr['sberr'][i]),cr['Nsb'][i],'%.0f+/-%.0f'%(cr['sigred'][i],cr['srerr'][i]),cr['Nsr'][i]],veldiffs[i],perc[i]*100)
    else:
        Tarr[i][3:]='-'
...
ax.table(cellText=Tarr,loc='center',colLabels=('Cluster','Full Disp.','N','Blue Disp.','N','Red Disp.','N','Vel. Diff.','P'))
