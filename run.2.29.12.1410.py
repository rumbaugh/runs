...
cr3 = read_file('/home/rumbaugh/BCGpositions.2.15.12.dat')
BCGRA = copy_colvals(cr3,'col2')
BCGDec = copy_colvals(cr3,'col3')
BCGorder = np.array([1,2,1,3,3,1,1,2,2])-1
ccnt = 0
FILE = open('/home/rumbaugh/clustercenters.2.29.12.dat','w')
for i in range(0,len(names)):
    if ((i != 1) & (i != 2) & (i != 6) & (i != 8)): ccnt += 1
    if i == 5: ccnt += 1
    sfile = '/home/rumbaugh/%s'%(files[ccnt])
    pfile = '/home/rumbaugh/LFC/%s'%(pfiles[ccnt])
    FILE.write('%13s %14.10f %13.9f  %14.10f %13.9f  %14.10f %13.9f \n'%(names[i],BCGRA[i],BCGDec[i],RGalPeakRA[i],RGalPeakDec[i],centerRAs[i],centerDecs[i]))
FILE.close()
