...
slit_dict = {'1131m3': {'blue': {'top': ['1','54','80','261','449','416','484','564','663','679','948']}}, '1131m4': {'blue': {'top': ['1','91','131','197','517','600','786']}}, '1131m1_v2': {'blue': {'top': ['1','54','80','261','490','484','564','663','678','948'], 'bottom': ['176','77']}}, '1131m2_v2': {'blue': {'top': ['1','91','121','197','227','517','600','679','786'], 'bottom': ['98','426']}}, '0435m3_v2': {'blue': {'top': ['132','450','668','604','885','849','865']}}}

outdir = '/mnt/data2/rumbaugh/LRIS/2011_01/reduced/'

for mask in ['1131m3','1131m4']: 
    sides, colors = ['top'],['blue']
    if mask in ['1131m1_v2','1131m2_v2']: colors = ['blue']
    if mask in ['0435m3_v2']: colors,sides = ['blue'],['top']
    for color in colors:
        for side in sides: 
            for slit in slit_dict[mask][color][side]:
                run_swarp(outdir,mask,color,side,slitnames=[slit])
                f = raw_input('\n\nDid it work?\n\n')
                if f == 'n':
                    try:
                        run_swarp(outdir,mask,color,side,slitnames=[slit],resample='N',skipimgs=files_dict[mask][color]['exp_times'].keys())
                    except KeyError:
                        run_swarp(outdir,mask,color,side,slitnames=[slit],resample='N')
