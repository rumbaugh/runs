...
def sim_lc(lens):
    ...
    minflux,maxflux,sigmay,min_mu,max_mu,avgerr,errstd,mintau,maxtau = carma_params[lens]['minflux'],carma_params[lens]['maxflux'],carma_params[lens]['sigmay'],carma_params[lens]['min_mu'],carma_params[lens]['max_mu'],carma_params[lens]['avgerr'],0.5*np.array(carma_params[lens]['errstd']),carma_params[lens]['mintau'],carma_params[lens]['maxtau']
...
for lens in ('0712','1030'):
    ...
    for i in range(0,runs):sim_lc(lens)
    FILE.close()
