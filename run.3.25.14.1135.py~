...
for source in sources:
    ...
    if source == '0414+573':
        ...
        tdates = np.delete(tdates,gdel)
    FILE = open('/home/rumbaugh/EVLA/light_curves/VLA_lc_aipsredux_3.23.14_%s.dat'%source,'w')
    for date in dates:
        if date[1] == '.':
        ...
        FILE.write('%f'%daytot)
        if date in tdates:
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_3.23.14.%s.fixpos.%s.mod'%(source,date),dtype='string',comments='!')
            num_img = np.shape(cr)[0]
            for img in range(0,num_img):
                fluxstr = cr[img][0]
                FILE.write(' ' + fluxstr[:len(fluxstr)-1])
        else:
            for img in range(0,num_img): FILE.write(' 0')
        FILE.write('\n')
    FILE.close()
for source in np.append(CSOs,'1035+564'):
    ...
    for date in dates:
        ...
        FILE.write('%f'%daytot)
        if date in tdates:
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux.%s.fixpos.%s.mod'%(source,date),dtype='string',comments='!')
            fluxstr = cr[0]
            FILE.write(' ' + fluxstr[:len(fluxstr)-1])
        else:
            FILE.write(' 0')
