...
mintime = -1*maxtime
tau_init,mu_init = 0.,np.mean(Bflux)/np.mean(Aflux)
ndim,nwalkers = 2,10
if lens == '1938':
    maxtimestep = 60.
else:
...
sampler = emcee.EnsembleSampler(nwalkers,ndim,delaylnprob,args=[Bflux,Aflux,Berr,Aerr,ltime,mu_init,mintime,maxtime,True,True,False,'triangle',10.])
pos,prob,state=sampler.run_mcmc(p0,100)
sampler.reset()
pos,prob,state=sampler.run_mcmc(pos,runs)
FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/%s.emcee_output_full_chain_neff.%s.dat'%(lens,date),'w')
