...
for b,ib,bnum in zip(np.array(['0','100','300','600','inf']),np.arange(5),np.array([0,100,300,600,999999])):
    if b=='inf':
        ax1.plot(crbs[ib][:,1],crbs[ib][:,0],color=colarr[ib],ls=lsarr[ib],lw=2,label=r'$\Delta t=%s$'%('\infty'))
    else:
        gsense=np.where(crbs[ib][:,1]>=bnum)[0]
        ax1.plot(crbs[ib][:,1][gsense],crbs[ib][:,0][gsense],color=colarr[ib],ls=lsarr[ib],lw=2,label=r'$\Delta t=$' '%s days'%b)
...
ax1.set_yticklabels(minor_ytick_labels,minor=True)
plt.savefig('/home/rumbaugh/var_database/Y3A1/plots/MaxChangeBaselinePlot.DetFrac_comp.baselines.6.22.17.png')
