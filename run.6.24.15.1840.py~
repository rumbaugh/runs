...
print len(g),len(medtau[avg_fwidth!=0]),len(g)*1./len(medtau[avg_fwidth!=0])

catthresh=15

delT=np.abs(medtau[avg_fwidth!=0]-taus[avg_fwidth!=0])
g2=np.where(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0]<3.502)[0]
g3=np.where(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0]>3.502)[0]
print len(g2),len(g3),len(delT[g2][delT[g2]>catthresh]),len(delT[g3][delT[g3]>catthresh])
...
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/Timebombs_accuracy_vs_flarewidths.%s.png'%date)
figure(25)
clf()
scatter(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0],np.abs(wt_tau[avg_fwidth!=0]-taus[avg_fwidth!=0]),s=4)
g=np.where(wt_tau[avg_fwidth!=0]<3.502*avg_fwidth[avg_fwidth!=0])[0]
if len(g)>0:scatter(taus[avg_fwidth!=0][g]/avg_fwidth[avg_fwidth!=0][g],np.abs(wt_tau[avg_fwidth!=0][g]-taus[avg_fwidth!=0][g]),s=4,color='red')
print len(g),len(wt_tau[avg_fwidth!=0]),len(g)*1./len(wt_tau[avg_fwidth!=0])
delT=np.abs(wt_tau[avg_fwidth!=0]-taus[avg_fwidth!=0])
g2=np.where(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0]<3.502)[0]
g3=np.where(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0]>3.502)[0]
print len(g2),len(g3),len(delT[g2][delT[g2]>catthresh]),len(delT[g3][delT[g3]>catthresh])
axvline(3.502,color='cyan',ls='dashed',lw=2)
xlabel('Normalized True Delay (Flare widths)')
ylabel('Absolute Deviation from True Delay (days)')
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/Timebombs_accuracy_vs_flarewidths.weight_cen.%s.png'%date)
figure(29)
clf()
scatter(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0],wt_tau[avg_fwidth!=0]-taus[avg_fwidth!=0],s=4)
g=np.where(wt_tau[avg_fwidth!=0]<3.502*avg_fwidth[avg_fwidth!=0])[0]
if len(g)>0:scatter(taus[avg_fwidth!=0][g]/avg_fwidth[avg_fwidth!=0][g],wt_tau[avg_fwidth!=0][g]-taus[avg_fwidth!=0][g],s=4,color='red')
axvline(3.502,color='cyan',ls='dashed',lw=2)
xlabel('Normalized True Delay (Flare widths)')
ylabel('Deviation from True Delay (days)')
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/Timebombs_bias_vs_flarewidths.weight_cen.%s.png'%date)
