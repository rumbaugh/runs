...
for i in range(0,len(stds)):
    ...
    print '%5s Mean: %9.7f STD: %9.7f Sigma: %6.2f Threshhold: %6.2f Accepted: %s'%(int(IDs[i]),means[i],stds[i],metric[i],accept)
    FILE.write('%5s %9.7f %9.7f %6.2f %6.2f %s'%(int(IDs[i]),means[i],stds[i],metric[i],accept))
FILE.close()
