...
crMQP2=np.loadtxt('/home/rumbaugh/milliquas_y1a1_match_pass2.csv',dtype={'names':('MQ_ROWNUM','RA','DEC','HPIX','CID'),'formats':('i8','f8','f8','i8','i8')},skiprows=1,delimiter=',')
...
for iDB,DBID in zip(np.arange(len(crdb['DBID'])),crdb['DBID']):
    ...
    if ((crdb['sdr7id'][iDB]!=0)&(crdb['sdr7id'][iDB]>-50000)):
        ...
        if crdb['thingid'][iDB]<=0: 
            ...
            s82[iDB]=crsp['s82flag'][gsp]
        if crdb['CID'][iDB]<=0:
            redshifts[iDB]=crsp['redshift'][gsp]
        raPOSS[iDB],decPOSS[iDB]=crsp['ra'][gsp],crsp['dec'][gsp]
