...
def delaylnprob(x,A,B,A_err,B_err,ltime,maxtime=500,mintime=None,t_grid_spacing=0.5,smooth_param=10.):
...
for i in range(0,1):
    ...
    p0[:,0],p0[:,1],p0[:,2],p0[:,3] = np.zeros(nwalkers)+np.random.normal(scale=0.1,size=nwalkers),np.ones(nwalkers)*mu1_init+np.random.normal(scale=0.01,size=nwalkers),np.ones(nwalkers)*mu2_init+np.random.normal(scale=0.01,size=nwalkers),np.ones(nwalkers)*mu3_init+np.random.normal(scale=0.01,size=nwalkers)
    ...
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_hist_tau.BA_delay.%s.ps'%(date))
    py.close()
    py.plot(sampler.flatchain[:,1])
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_chain_plot_mu1.BA_delay.%s.ps'%(date))
    py.close()
    py.plot(sampler.flatchain[:,2])
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_chain_plot_mu2.BA_delay.%s.ps'%(date))
    py.close()
    py.plot(sampler.flatchain[:,3])
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_chain_plot_mu3.BA_delay.%s.ps'%(date))
    py.close()
    py.plot(sampler.flatchain[:,0])
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_chain_plot_tau.BA_delay.%s.ps'%(date))
    py.close()
    for j in range(0,nwalkers):
        py.plot(sampler.chain[j,:,0])
        py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_all_chain_plot_tau.BA_delay.%s.ps'%(date))
    py.close()
    for k in range(1,4):
        for j in range(0,nwalkers):
            py.plot(sampler.chain[j,:,k])
            py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_all_chain_plot_mu%i.BA_delay.%s.ps'%(k,date))
        py.close()
    py.scatter(sampler.flatchain[:,0],sampler.flatlnprobability)
    py.savefig('/home/rumbaugh/time_delay_files/test/emcee_test/B1608.emcee_output_tau-prob_scatter.BA_delay.%s.ps'%(date))
    py.close()
