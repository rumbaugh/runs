...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    if i == 0:
        ...
        pylab.xlim(201.875,200.49)
        xlocs = np.array([201.75,201.625,201.5,201.375,201.25,201.125,201.0,200.875,200.75,200.625])
        xlabs = np.array(['27$^{m}$00$^{s}$','30$^{s}$','26$^{m}$00$^{s}$','30$^{s}$','25$^{m}$00$^{s}$','30$^{s}$','24$^{m}$00$^{s}$','30$^{s}$','23$^{m}$00$^{s}$','13$^{h}$22$^{m}$30$^{s}$'])
    ...
    if i == 4:
        #RXJ1757
        pylab.xlim(269.9375,268.90625)
        pylab.ylim(66 + 21.0/60, 66+41.0/60)
        ...
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs) 
        g = np.where((sz[gsq] > 0.455) & (sz[gsq] < 0.475))
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=10,color=html_teal)
        g = np.where((sz[gsq] > 0.65) & (sz[gsq] <= 0.675))
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=10,color=html_green)
        g = np.where((sz[gsq] > 0.71) & (sz[gsq] <= 0.74))
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=10,color=html_purp)
        g = np.where((sz[gsq] > 0.94) & (sz[gsq] <= 0.97))
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=10,color=html_brwn2)
        g = np.where((sz[gsq] > 0.98) & (sz[gsq] <= 1.005))
        pylab.scatter(sRA[gsq[g]],sDec[gsq[g]],s=10,color=html_orng)
        leg = pylab.legend(('0.455 < z < 0.475','0.65 < z < 0.675','0.71 < z < 0.74','0.94 < z < 0.97','0.98 < z < 1.005'),loc=3,markerscale=0.8,labelspacing=0.3)
        icoff = len(cnam)-1
        for ic in range(0,1):
            plotcircle(RAcen=clusRA[ic+icoff],DECcen = clusDec[ic+icoff],rad=crads[ic+icoff])
            #pylab.text(clusRA[ic+icoff],clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
    if ((i == 1) | (i > 3.1)) : pylab.scatter(sRA[gsq[gsz]],sDec[gsq[gsz]],s=8)
    ...
    savfile = '/home/rumbaugh/paperstuff/AGN.spatmap.color.%s.4.21.11.png'%(names[i])
