...
for i,pair in zip(np.arange(len(iter_dict.keys())),iter_dict.keys()):
    ...
    crd = np.loadtxt('/mnt/data2/rumbaugh/Fermi/output/test/Dispersion.testlightcurve_%i.tau_%.2f.mu_%.3f.D_4_2.delta_%.1f.%s.dat'%(pair,tau,mu,delta,'7.30.14'))
    tau_d,mu_d,dtmp = crd[:,0],crd[:,1],crd[:,2]
    len_mu = len(tau_d[tau_d==tau_d[0]])
    len_tau = len(mu_d[mu_d==mu_d[0]])
    Z = np.reshape(dtmp,(len_tau,len_mu))
    Ztau = np.reshape(tau_d,(len_tau,len_mu))[:,0]
    Zmu = np.reshape(mu_d,(len_tau,len_mu))[0]
    #V = np.exp(np.arange(ncontours)*1./ncontours*(np.max(np.log(Z))-np.min(np.log(Z)))+np.min(np.log(Z)))
    V = (np.arange(ncontours)*1./ncontours*(np.max(Z)-np.min(Z))+np.min(Z))
    plt.figure(1)
    plt.clf()
    ...
    plt.xlim(0,100)
    plt.ylim(0,1)
    plt.savefig('/mnt/data2/rumbaugh/Fermi/TimeBombs/plots/Dispersion+TimeBombs_contour_plot.mu_limit.testlightcurve_%i.tau_%.2f.mu_%.3f.%s.png'%(pair,tau,mu,date))
