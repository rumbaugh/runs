...
a = os.chdir('/home/rumbaugh/B1938+666_files/')
    ...
    for groupnum in np.sort(EVLA_obs_dict[EorL].keys()):
        ...
        for SBnum in np.sort(EVLA_obs_dict[EorL][groupnum].keys()):
            ...
            for field,fieldname in zip(lensfields_arr,lensnames):
                source = fieldname
                fitsfile = '/home/rumbaugh/B1938+666_files/residual_map.%s.%sSB%i_%s.fixpos.%s.fits'%(source,EorL,groupnum,str(SBnum),date)
                hdutmp = pf.open(fitsfile)
                datatmp = hdutmp[0].data.copy()
                try:
                    newdat = newdat+datatmp
                except NameError:
                    newdat = datatmp
                hdr = hdutmp[0].header.copy()
                hdutmp.close()
pf.PrimaryHDU(newdat,hdr).writeto('/home/rumbaugh/B1938+666_files/B1938.resmap.concat.fits')
