...
oldspecloaddict={'names':('ID','mask','slit','ra','dec','magR','magI','magZ','z','zerr','Q','oldid'),'formats':('|S16','|S16','|S8','f8','f8','f8','f8','f8','f8','f16','i8','|S16')}
...
for field in targets:
    ...
    try:
        ...
        np.savetxt('/home/rumbaugh/Chandra/speccats/cats4coadd_2.11.16/%s_spec4coadd.cat'%field,crs[g],fmt='%16s %12s %12s %f %f  %f  %f  %f  %f  %f %i %s',header='ID mask slit ra dec magR magI magZ z zerr Q oldid')
    except:
        crs=np.loadtxt(specfile,dtype=specloaddict)
        g=np.where((crs['Q']>2.5)&(crs['z']>=spec_dict[field]['z'][1])&(crs['z']<=spec_dict[field]['z'][2]))[0]
        np.savetxt('/home/rumbaugh/Chandra/speccats/cats4coadd_2.11.16/%s_spec4coadd.cat'%field,crs[g],fmt='%16s %12s %12s %f %f  %f  %f  %f  %f  %f %i',header='ID mask slit ra dec magR magI magZ z zerr Q')
