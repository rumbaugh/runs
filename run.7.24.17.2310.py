...
execfile('/home/rumbaugh/pythonscripts/CalcVelDisp.py')
...
crCMD=np.loadtxt('/home/rumbaugh/Chandra/ORELSE.cluster_members.6.26.17.dat',dtype={'names':('field','cluster','ra','dec','z','mag','col','rso'),'formats':('|S12','|S12','f8','f8','f8','f8','f8','f8')})
...
crBCG=pd.read_csv('/home/rumbaugh/Chandra/MMCG_spec_1.5Rvir.07.20.17.cat',delim_whitespace=True)
...
for clus,ic in zip(crBCG['id_cluster'].values,np.arange(len(crBCG))):
    ...
    BCGvel[ic]=(crBCG.z_spec.iloc[ic]-SE.biweight_loc(zs))*c/(1.0+crBCG.z_spec.iloc[ic])
outdf=pd.DataFrame({'field':crBCG['field'],'cluster':crBCG['id_cluster'],'BCGvel':BCGvel})
outdf=outdf[['field','cluster','BCGvel']]
outdf.to_csv('/home/rumbaugh/Chandra/ORELSE.MMCG_vels.csv',index=False)
