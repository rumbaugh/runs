...
outdf=pd.DataFrame({x: np.zeros(len(data)) for x in ['RMS','RMSnorm','numepoch','numepoch_tot']})
outdf['DataID']=np.arange(len(data))
...
outdf.to_csv('/home/rumbaugh/SN_fields.S1.AUTO_RMS.csv',index=False)
