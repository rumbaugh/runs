...
#find time delays
    ...
    sampler = emcee.EnsembleSampler(nwalkers,ndim,delaylnprob,args=[Bflux[g],Aflux[g],1.25*Berr[g],1.25*Aerr[g],ltime[g],mu_init,mintime,maxtime,False,True,True,'interp',10.,0.5,None])
    print maxtime,mintime
    pos,prob,state=sampler.run_mcmc(p0,100)
    sampler.reset()
    pos,prob,state=sampler.run_mcmc(pos,runs)
    if runs < 50: 
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/test.%s.emcee_output_full_chain_interp_bs.1.25Xerr.%s.dat'%(lens,date),'w')
    else:
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/%s.emcee_output_full_chain_interp_bs.1.25Xerr.%s.dat'%(lens,date),'w')
