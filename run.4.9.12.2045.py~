...
for i in range(0,9):
    ...
    if distBCG[i] < 100: dBCGstr = '\ %s'%(dBCGstr)
    if distRGP[i] < 100: dRGPstr = '\ %s'%(dRGPstr)
    if distLW[i] < 100: dLWstr = '\ %s'%(dLWstr)
    FILE.write('%13s & %3s %s %s & %2s %s %s & %5.3f & %3s %s %s & %2s %s %s & %5.3f & %3s %s %s & %2s %s %s & %5.3f'%(names[i],bcgrahstr,bcgramstr,bcgrasstr,bcgdecdstr,bcgdecmstr,bcgdecsstr,dBCGstr,rgrahstr,rgramstr,rgrasstr,rgdecdstr,rgdecmstr,rgdecsstr,dRGPstr,lwrahstr,lwramstr,lwrasstr,lwdecdstr,lwdecmstr,lwdecsstr,dLWstr))
    if i < 8: FILE.write(' \\\\\n')
    FILEr.close()
FILE.close()
