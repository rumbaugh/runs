...
#        sI = get_colvals(crs,'col18')
    ...
    savname = '/home/rumbaugh/paperstuff/CMD.%s.completeness.rad_%3.1f.5.7.11.png'%(names[i],cradmult)
    ...
    print 'Observed RS/NRS cluster members: %4i,%4i\n'%(int(len(gszRS)),int(len(gszNRS0)+len(gszNRS1)+len(gszNRS2)))
    print 'Observed all RS/NRS: %4i,%4i\n'%(int(len(gsallRS)),int(len(gsallNRS0)+len(gsallNRS1)+len(gsallNRS2)))
    ...
    print '(Tot. AGN,Tot. good Xray sources): (%f,%f)\n'%(len(gmsz),len(gmsq))
