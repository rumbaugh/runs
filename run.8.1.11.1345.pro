...
device,file="/home/rumbaugh/colVr.plot_all+clust.8.1.11.ps",/color
...
plot,[0,1],[0,1],/nodata,TITLE="AGN Color-Magnitude Diagram - " + names[i],XTITLE=xstr,YTITLE=ystr,PSYM=3,THICK=2,SYMSIZE=0.1,XRANGE=[19,25],YRANGE=[-0.0,1.6],xstyle=1,ystyle=1,CHARSIZE=1.3,CHARTHICK=4,XTHICK=6,YTHICK=6 &$
...
if n_elements(gq) eq 1 then oplot,tempi,tempr-tempi,PSYM=6,SYMSIZE=0.6,THICK=16,color=160 &$
if n_elements(gq) gt 1 then oplot,iB[g[gq]],rB[g[gq]]-iB[g[gq]],PSYM=6,SYMSIZE=0.6,THICK=16,color=160 &$
...
rsfit = 3.182-0.063*f814
rsoffset = f606-f814-rsfit
plot,[0,1],[0,1],/nodata,TITLE="AGN Color-Magnitude Diagram - 1604 - ACS only",XTITLE="F814W Magnitude",YTITLE="F606W-F814W",PSYM=3,XRANGE=[20.4,25.5],YRANGE=[0.2,2.2],xstyle=1,ystyle=1,CHARSIZE=1.3,CHARTHICK=4,XTHICK=6,YTHICK=6
h = where((F606W lt 90) and (F606W gt 0.1) and (F814W gt 0.1) and (F814W lt 90))
hh = where((f606 gt 0.1) and (f606 lt 90) and (f814 gt 0.1) and (f814 lt 90))
...
rsfit = 3.175-0.063*xl
rsoffset = F606W-F814W-rsfit
oplot,F814W[h[g[gq]]],F606W[h[g[gq]]]-F814W[h[g[gq]]],color=160,PSYM=6,SYMSIZE=0.6,THICK=16
yl = -2*0.0907+rsfit
yu = 2*0.0907+rsfit
oplot,xl,yl,linestyle=3,thick=5,color=30
oplot,xl,yu,linestyle=3,thick=5,color=30
