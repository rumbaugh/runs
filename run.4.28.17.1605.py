...
for DBID,idb in zip(crdb['DatabaseID'],np.arange(len(crdb))):
    ...
    if np.shape(cr)==():
        if len(ggood)<1:
            ...
            if len(gmc)>0:
                if len(gbmac)>0:
                    ...
                    maclen=len(cr)
                    mjd,mag,magerr,bands,survey,flags=np.array([cr['MJD']]),np.array([cr['MAG']]),np.array([cr['MAGERR']]),np.array([cr['BAND']]),np.full(len(cr),'SDSS'),np.array([cr['FLAG']])
            ...
            mjd,mag,magerr,bands,survey,flags=np.array([cr['MJD']]),np.array([cr['MAG']]),np.array([cr['MAGERR']]),np.array([cr['BAND']]),np.array([cr['Survey']]),np.array([cr['FLAG']])
            gb=np.where(bands=='g')[0]
            if len(gb)==0:
                mjd,mag,magerr,bands,survey,flags=np.zeros(0),np.zeros(0),np.zeros(0),np.zeros(0,dtype='|S4'),np.zeros(0,dtype='|S4'),np.zeros(0,dtype='i8')
                mydblen=0
            else:
                mydblen=1
        ...
        gorig=gorig[gb]
        mydblen=len(gb)    
        mjd,mag,magerr,bands,survey,flags=cr['MJD'],cr['MAG'],cr['MAGERR'],cr['BAND'],cr['Survey'],cr['FLAG']
        if len(gmc)>0:
    ...
    if len(gb)>0:
        ...
        if len(gsig)>0:
            ...
            mjd_st,mjd_end,surv_st,surv_end,flag_st,flag_end=mjd[gst],mjd[gend],survey[gst],survey[gend],flags[gst],flags[gend]
            if mjd_st>mjd_end:
                maxdiff=-maxdiff
            maxdrop[idb]=maxdiff
...
outcr['RA'],outcr['DEC'],outcr['Redshift'],outcr['MJD_lo'],outcr['g_lo'],outcr['sig_lo'],outcr['flag_lo'],outcr['MJD_hi'],outcr['g_hi'],outcr['sig_hi'],outcr['flag_hi'],outcr['RA_DES'],outcr['DEC_DES'],outcr['DBID'],outcr['y3a1_mag_auto_g']=radr7,decdr7,redshifts,mjd_max[:,0],g_max[:,0],sig_max[:,0],flags_max[:,0],mjd_max[:,1],g_max[:,1],sig_max[:,1],flags_max[:,1],ra,dec,crdb['DatabaseID'],y3a1mag
np.savetxt('/home/rumbaugh/var_database/Y3A1/DR7_full_magdiffs_wDBID.4.28.17.tab',outcr,header='RA DEC Redshift MJD_lo g_lo sig_lo flag_lo MJD_hi g_hi sig_hi flag_hi RA_DES DEC_DES DBID y3a1_mag_auto_g',fmt='%f %f %f %f %f %f %i %f %f %f %i %f %f %24s %f')
