...
crl = read_file('/home/rumbaugh/paperstuff/clus.xlums.4.25.11.dat')
...
pylab.errorbar(sigma, lums, xerr=[sigerr,sigerr],yerr=[lumes,lumes],fmt='ro')
pylab.xlabel('Velocity Dispersion (km/s)')
pylab.ylabel('Full-band Luminosity (1e44 erg/s)')
for i in range(0,len(Temps)): 
    if i == 0: 
        pylab.text(sigma[i]+0.2,lums[i]+0.0040,names[i],color='black',weight='extra bold')
    elif i == 2:
        pylab.text(sigma[i]+0.1,lums[i]-0.005,names[i],color='black',weight='extra bold')
    elif i == 4:
        pylab.text(sigma[i]+20,lums[i]+0.02,names[i],color='black',weight='extra bold')
    elif i == 6:
        pylab.text(sigma[i]-150,lums[i]+0.022,names[i],color='black',weight='extra bold')
    else:
        pylab.text(sigma[i],lums[i],names[i],color='black',weight='extra bold')
pylab.savefig('/home/rumbaugh/paperstuff/sigLx.plot.4.25.11.png')
pylab.close('all')
