...
for field in rkeys[((rkeys!='rcs0224')&(rkeys!='rxj1757')&(rkeys!='rxj1821'))]:
    ...
    elif field in ['rxj0910','rcs0224','cl1324']:
    ...
    if ((field=='rcs0224')|(field=='rxj0910')|(field=='cl1324')|(field=='rxj1757')|(field=='rxj1821')):
    ...
    gRSo,gGVo,gBCo=np.where(-rso[gknown]>=-1)[0],np.where((-rso[gknown]<-1)&(-rso[gknown]>=-3))[0],np.where(-rso[gknown]<-3)[0]
    ...
    print 'RS: %.3f  GV: %.3f  BC: %.3f'%(1./(1+len(P_inclus[gknown][gRSo])/np.sum(P_inclus[gnew0][gnew][gRS])),1./(1+len(P_inclus[gknown][gGVo])/np.sum(P_inclus[gnew0][gnew][gGV])),1./(1+len(P_inclus[gknown][gBCo])/np.sum(P_inclus[gnew0][gnew][gBC])))
