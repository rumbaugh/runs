...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    if i == 0:
        ...
        g = np.where((sz[gsq[gsz]] > 0.735) & (sz[gsq[gsz]] <= 0.745))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color=html_brwn)
        g = np.where((sz[gsq[gsz]] > 0.745) & (sz[gsq[gsz]] <= 0.765))
        ...
        plotcircle(RAcen=201.25,DECcen = 30.5,rad=5/60.0)
    ...
    if i == 1:
        ...
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
        plotcircle(RAcen=275.25,DECcen = 68.5,rad=5/60.0)
    if i == 2:
        ...
        plotcircle(RAcen=6,DECcen = 4.25,rad=5/60.0)
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if i == 3:
        ...
        plotcircle(RAcen=241.31,DECcen=43.25,rad=5/60.0)
        F = pylab.gcf()
    ...
    if i == 4:
        ...
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
        plotcircle(RAcen=269.4,DECcen = 66.5,rad=5/60.0)
    ...
    savfile = '/home/rumbaugh/paperstuff/test.AGN.spatmap.color.%s.4.19.11.png'%(names[i])
    if i == 0: pylab.savefig(savfile)
    pylab.close('all')
#FILEA.close()
