...
plt.figure(1)
plt.clf()
plt.scatter(taub,sigb,s=1)
plt.scatter(taumc,sigmc,s=1)
xlim=plt.xlim()
ylim=plt.ylim()
plt.plot([xlim[0],xlim[1]],[ylim[0],ylim[1]],lw=2,ls='dashed',color='k')
plt.scatter(taub,sigb,s=30,color='b')
plt.errorbar(taumc,sigmc,xerr=[taumclerr,taumcherr],yerr=[sigmclerr,sigmcherr],color='r',fmt='ro',lw=2,capsize=3,mew=1)
