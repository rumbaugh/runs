...
for i in range(0,4):
    cnt += 1
    cntfile = "/home/rumbaugh/ChandraData/" + names[i] + "/master/" + names2[cnt] + ".img.500-2000.nops.fits"
    convfile = "conv.beta.12.15.10.fits"
    if cnt == 3: convfile = "conv.beta.12.15.10_2.fits"
    if FC != 0:
        print 'FC != 0'
        ostemp = os.system("dmstat /scratch/rumbaugh/ciaotesting/" + names[i] + "/master/" + convfile + " verbose=0")
    ...
    #load_data("/scratch/rumbaugh/ciaotesting/" + names[i] + "/master/" + !!convfile)
    load_data(cntfile)
    set_coord("physical")
    for r in range(0,bins):
        counts[cnt,r] = calc_data_sum2d('circle(%s,%f)'%(str(cen),annuli[r]))
        #print counts[cnt,r]
xcen,ycen = 4636.0706,3657.7822
for r in range(0,bins):
