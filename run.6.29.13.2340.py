...
for source in sources:
    FILE = open('/home/rumbaugh/EVLA/lightcurves/VLA_lc_2001_%s.dat'%source,'w')
    for date in dates:
        if date[1] != '.':
        ...
        if date not in ['3.06.01','3.16.01','4.10.01']:
            FILE.write(daytot)
            cr = np.loadtxt('/mnt/data2/VLA/AF377/difmap_results/fit.%s.fixpos.%s.mod'%(source,date),dtype='string',comments='!')
            num_img = np.shape(cr)[0]
            for img in range(0,num_ing):
                fluxstr = cr[img][0]
                FILE.write(' ' + float(fluxstr[:len(fluxstr)-1]))
            FILE.write('\n')
    FILE.close()
for source in CSOs:
    FILE = open('/home/rumbaugh/EVLA/lightcurves/VLA_lc_2001_%s.dat'%source,'w')
