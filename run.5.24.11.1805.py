...
crl = read_file('/home/rumbaugh/paperstuff/clus.lums.soft.5.24.11.dat')
lums = get_colvals(crl,'col2')/10.0
lumes = get_colvals(crl,'col3')/10.0

crl = read_file('/home/rumbaugh/paperstuff/clus.lums.bol.5.24.11.dat')
...
for i in range(0,len(Temps)): 
    if i == 0: 
        ...
        pylab.text(Temps[i]+0.1,lumbol[i]-1.1,names[i],color='black',weight='extra bold')
    elif i == 3:
        pylab.text(Temps[i]-1.5,lumbol[i]-1.1,names[i],color='black',weight='extra bold')
    elif i == 4:
        aa = 1
    else:
