...
ymax = [20,40,10,10,20]
device,file="/home/rumbaugh/paperstuff/clus.RSwid.10.5.11.ps",/color
...
if i lt 4 then readcol,"/home/rumbaugh/LFC/FINAL.matched." + names2[i] + ".specnXray.nov2010.rumbaugh.cat",LFCid,mask,slit,raopt,decopt,rB,iB,zB,z,zerr,q,oldid,idX,raX,decX,errX,Rel,Sig,/silent,format='A,A,I,D,D,D,D,D,D,D,I,A,I,D,D,D,D,D' &$
if i ge 4 then readcol,"/home/rumbaugh/LFC/FINAL.matched.1604.specnXray.nov2010.rumbaugh.cat",LFCid,mask,slit,raopt,decopt,rB,iB,zB,z,zerr,q,oldid,mskACA,raACA,decACA,idACA,F606W,F814W,idX,raX,decX,errX,Rel,Sig,/silent,format='A,A,I,D,D,D,D,D,D,D,I,A,A,D,D,A,D,D,I,D,D,D,D,D' &$
...
if i lt 4 then readcol,files[i],newid,mask,slit,raLFC,decLFC,rprime,iprime,zprime,rs,rserr,sQ,oldid,/silent,format="A,A,A,D,D,D,D,D,D,D,I,A" &$
if i ge 4 then readcol,"/home/rumbaugh/LFC/FINAL.spectra.sc1604.onlysemifinal.wcompletenessmasks.nov2010.cat",newid,mask,slit,raLFC,decLFC,rprime,iprime,zprime,rs,rserr,sQ,oldid,pflags,raACS,decACS,idACS,f606,f814,/silent,format="A,A,A,D,D,D,D,D,D,D,I,A,A,D,D,A,D,D" &$
...
readcol,"/home/rumbaugh/cl1604_spectro_sed_fit_info.dat",sedID,sedmask,sedslit,sedRA,sedDec,sedz,sediid,Ebv,Ebverr,tau,metal,age,mass,SMerr,chi2,ignore,context,/silent,format='A,A,A,D,D,D,I,D,D,D,F,F,F,F,D,F,I'
;context > 63 is good, context 15 is ok, or is it 55?

effwav606 = 584.3/1000
effwav814 = 826.9/1000
;effective wavelengths for WIFPC2 in nm
k814 = 2.659*(-2.156+1.509/effwav814-0.198/effwav814/effwav814+0.011/effwav814/effwav814/effwav814)+4.05
k606 = 2.659*(-2.156+1.509/effwav606-0.198/effwav606/effwav606+0.011/effwav606/effwav606/effwav606)+4.05

readcol,"/home/rumbaugh/LFC/FINAL.spectra.sc1604.onlysemifinal.wcompletenessmasks.nov2010.cat",newid,mask,slit,raLFC,decLFC,rprime,iprime,zprime,rs,rserr,Q,oldid,pflags,raACS,decACS,idACS,f606,f814,/silent,format="A,A,A,D,D,D,D,D,D,D,I,A,A,D,D,A,D,D"
g = where((0.96 ge rs) and (rs ge 0.84) and (Q ge 2.3))
rsfit = 3.182-0.063*f814
rsoffset = f606-f814-rsfit
...
giACS = INDGEN(n_elements(g))
for l=0,n_elements(g)-1 do begin &$
gtemp = where((sedID eq newid[g[l]]) and (context gt 62)) &$
giACS[l] = gtemp[0] &$
   endfor
gebv = where(giACS gt -0.1)

print,mean(Ebv[giACS[gebv]])
print,sqrt(total((Ebv[giACS[gebv]]-mean(Ebv[giACS[gebv]]))*(Ebv[giACS[gebv]]-mean(Ebv[giACS[gebv]])))/n_elements(gebv))

savf814 = f814[g[gebv]]
savcol = f606[g[gebv]]-f814[g[gebv]]
Cf606 = f606[g[gebv]]-2.5*0.4*k606*Ebv[giACS[gebv]]
Cf814 = f814[g[gebv]]-2.5*0.4*k814*Ebv[giACS[gebv]]
cRS = Cf606-Cf814-(3.182-0.063*Cf814)
savrsfit = rsoffset[g[gebv]]
...
for i=0,3 do begin &$
readcol,rfiles[i],LFCid,mask,slit,raopt,decopt,rB,iB,zB,z,zerr,q,oldid,idX,raX,decX,errX,nm,Rel,Sig,/silent,format='A,A,I,D,D,D,D,D,D,D,I,A,I,D,D,D,D,D' &$
...
readcol,rfiles[i],LFCid,mask,slit,raopt,decopt,rB,iB,zB,z,zerr,q,oldid,mskACA,raACA,decACA,idACA,F606W,F814W,idX,raX,decX,errX,nm,Rel,Sig,/silent,format='A,A,I,D,D,D,D,D,D,D,I,A,A,D,D,A,D,D,I,D,D,D,D,D'
h = where((F606W lt 90) and (F606W gt 0.1) and (F814W gt 0.1) and (F814W lt 90))
g = where((q[h] gt 2.2) and (z[h] le 0.96) and (z[h] ge 0.84) and ((z[h] lt 0.9) or (z[h] ge 0.901)))
rsfit = 3.182-0.063*F814W
rsoffset = (F606W-F814W-rsfit)
y_RSO2 = rsoffset[h[g]]

gIDACS = INDGEN(n_elements(g))
for l=0,n_elements(g)-1 do begin &$
gtemp = where((sedID eq LFCid[h[g[l]]]) and (context gt 62)) &$
gIDACS[l] = gtemp[0] &$
   endfor
gsed = where(gIDACS gt -0.1)

print,mean(Ebv[gIDACS[gsed]])
print,sqrt(total((Ebv[gIDACS[gsed]]-mean(Ebv[gIDACS[gsed]]))*(Ebv[gIDACS[gsed]]-mean(Ebv[gIDACS[gsed]])))/n_elements(gsed))


CF606W = F606W[h[g[gsed]]]-2.5*0.4*k606*Ebv[gIDACS[gsed]]
CF814W = F814W[h[g[gsed]]]-2.5*0.4*k814*Ebv[gIDACS[gsed]]
CRSO = CF606W-CF814W-(3.182-0.063*CF814W)
plot,[0,1],[0,1],/nodata,XTITLE="Extincted RS offset",YTITLE="Corrected RS offset",XRANGE=[-1.375,0.5],YRANGE=[-2,0.5],/xstyle,/ystyle,XTHICK=4.5,YTHICK=4,CHARTHICK=4,CHARSIZE=1.3
oplot,Y_RSO2[gsed],CRSO,PSYM=6,color=255
oplot,savrsfit,cRS,PSYM=4

plot,[0,1],[0,1],/nodata,XTITLE="Extincted RS offset",YTITLE="Change in RS offset",XRANGE=[-1.375,0.5],YRANGE=[-1,1],/xstyle,/ystyle,XTHICK=4.5,YTHICK=4,CHARTHICK=4,CHARSIZE=1.3
oplot,Y_RSO2[gsed],CRSO-Y_RSO2[gsed],PSYM=6,color=255
oplot,savrsfit,cRS-savrsfit,PSYM=4
oplot,loc1,hist1*9,PSYM=10,color=60,THICK=3
tempx = [-3*0.063,-3*0.063,-9*0.063,-9*0.063]
tempy = [-100,100,100,-100]
oplot,tempx,tempy,linestyle=1

plot,[0,1],[0,1],/nodata,YTITLE="Num. of Galaxies",XTITLE="Change in RS offset",XRANGE=[-1.0,0.1],YRANGE=[0,105],/xstyle,/ystyle,XTHICK=4.5,YTHICK=4,CHARTHICK=4,CHARSIZE=1.3
ggv = where((savrsfit gt -9*0.0603) and (savrsfit lt -3*0.0603))
Histoplot,cRS[ggv]-savrsfit[ggv],BINSIZE=0.15,MININPUT=-0.825,THICK=6,/LINE_FILL,datacolorname='Navy Blue',POLYCOLOR='Navy Blue',orientation=45,/oplot
Histoplot,cRS-savrsfit,BINSIZE=0.15,MININPUT=-0.825,THICK=6,/oplot

plot,[0,1],[0,1],/nodata,YTITLE='F606W-F814W, Extincted',XTITLE='F814W, Extincted',XRANGE=[19,25],YRANGE=[-0.5,2.5],/xstyle,/ystyle,XTHICK=4.5,YTHICK=4,CHARTHICK=4,CHARSIZE=1.3
oplot,savf814,savcol,PSYM=4,SYMSIZE=0.5
oplot,F814W[h[g[gsed]]],F606W[h[g[gsed]]]-F814W[h[g[gsed]]],PSYM=6,color=255

plot,[0,1],[0,1],/nodata,YTITLE='F606W-F814W, Corrected',XTITLE='F814W, Corrected',XRANGE=[19,25],YRANGE=[-0.5,2.5],/xstyle,/ystyle,XTHICK=4.5,YTHICK=4,CHARTHICK=4,CHARSIZE=1.3
oplot,Cf814,Cf606-Cf814,PSYM=4,SYMSIZE=0.5
oplot,CF814W,CF606W-CF814W,PSYM=6,color=255
