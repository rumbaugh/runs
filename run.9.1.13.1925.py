...
for img in range(0,2):
    ...
    if img == 1: 
        ltime += 45.5
        S,Serr = S*mu/np.mean(fluxdict[1]),Serr*mu/np.mean(fluxdict[1])
    else:
        S,Serr = S/np.mean(fluxdict[1]),Serr/np.mean(fluxdict[1])
...
#plt.ylim(0.7,1.3)
print 'b'
plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/B1938.lc_delay_plot.9.1.13.png')
#plt.close('all')
