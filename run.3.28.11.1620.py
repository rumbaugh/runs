...
for i in range(0,5):
    ...
    if i == 0:
        pylab.xlim(201.35,200.95)
        xlocs = np.array([201.25,201.125,201.0])
        xlabs = np.array(['26$^{m}$00$^{s}$','30$^{s}$','13$^{h}$25$^{m}$00$^{s}$'])
        ylocs = np.array([30 + 10.0/60,30 + 20.0/60,30 + 30.0/60,30 + 40.0/60,30 + 50.0/60,31.0,31 + 10/60.0])
        ylabels = np.array(['30' + degree_symbol + "10'","20'","30'","40'","50'",'31' + degree_symbol + "00'","10'"])
    ...
    savfile = '/home/rumbaugh/paperstuff/AGN.spatmap.color.%s.png'%(names[i])
