...
#psfpdf=bpdf.PdfPages('/home/rumbaugh/Chandra/plots/Lum_lim.5.3.16.pdf')
...
for field in targets:
    crSlim=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_flatSlim.5.3.16.dat'%(field,field,field))
    ...
    g0=np.where((crSlim[:,0]>=0)|(crSlim[:,1]>=0)|(crSlim[:,2]>=0))[0]
    calc_dict={'z': spec_dict[field]['z'][0], 'nh': nh_dict[field], 'bands':{ 'names': np.array(['soft','hard','full']), 'kev': np.array([[0.5,2.0],[2.0,7.0],[0.5,7.0]])}, 'flux':crSlim[g0]}
    lums=np.zeros(np.shape(crSlim))
    lums[g0]=calc_Xray_lums(calc_dict,loadonce=True)
    np.savetxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_lim_lums.5.3.16.dat'%(field,field,field),lums,fmt='%E %E %E')
