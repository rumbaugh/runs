    ...
    for source in sources_dict[EorL]:
        ...
        for groupnum in EVLA_obs_dict[EorL]:
            for SBnum in EVLA_obs_dict[EorL][groupnum]:
                ...
                try:
                    cr = np.loadtxt('/mnt/data2/rumbaugh/EVLA/11A-138/difmap_results/fit.%s.%sSB%i_%s.fixpos.%s.mod'%(source,EorL,groupnum,str(SBnum),EVLA_date_dict[EorL][groupnum][SBnum]),dtype='string',comments='!')
