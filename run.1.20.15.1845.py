...
def sim_lc(lens):
    ...
    days,fA,fB,Aerr,Berr=days[:len(days)-10],fA[:len(days)-10],fB[:len(days)-10],Aerr[:len(days)-10],Berr[:len(days)-10]
    ...
    FILE = open('%s/../generated_lc.%s.%s.dat'%(proot,lens,date),'w')
    FILE.write('#time  intr. fluxA  intr. fluxB  fluxA  fluxB  fluxA err.  fluxB err.\n')
    for i in range(0,len(eflux_t1)):
        FILE.write('%7.2f %7.3f  %7.3f %7.3f  %7.3f  %7.3f %7.3f\n'%(days_t[i],flux_t[i],flux_t2[i],eflux_t1[i],eflux_t2[i],flux_err1[i],flux_err2[i]))
    FILE.close()
