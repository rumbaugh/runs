...
for mask in ['0435m2']:
	...
	outdir = '%s/0435m2_revised/test'%outdir_dict[mask]
	os.system('mkdir -p %s'%outdir)
	lris_reduce_multislit(indir_dict[mask],outdir,files_dict[mask]['prefix'],tdict,mask,colors='blue',sides=['top','bottom'],doCR=False)
	lris_reduce_multislit(indir_dict[mask],outdir,files_dict[mask]['prefix'],tdict,mask,colors='red',sides=['top','bottom'],doCR=False)
