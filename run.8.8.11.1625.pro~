...
cnttar3 = 0 &$
cntgood3 = 0 &$
openw,1,'/home/rumbaugh/matched.' + names[i] + '.Xray_opt_spec.bothmatches_serendips.8.8.11.cat' &$
...
for j=0,n_elements(XID)-1 do begin &$
   ...
   if ((gid2[0] gt -0.1) and (ixm eq 1))  then optID[j] = optID2[j] &$
   if ((gid2[0] gt -0.1) and (ixm eq 1))  then ra_opt[j] = ra_opt2[j] &$
   if ((gid2[0] gt -0.1) and (ixm eq 1))  then dec_opt[j] = dec_opt2[j] &$
   if ((gid2[0] gt -0.1) and (ixm eq 1))  then prob[j] = prob2[j] &$
   if ((gid2[0] gt -0.1) and (ixm eq 1))  then gid = gid2 &$
   ...
   if ((i eq 1) and (nm[j] gt 0.1) and (gid[im] lt -0.1) and (gpid[0] lt -0.1)) then printf,1,XID[j],RAX[j],DecX[j],poserr[j],sncnts[xpID[j]],hncnts[xpID[j]],fncnts[xpID[j]],sflux[xpID[j]],hflux[xpID[j]],fflux[xpID[j]],ssig[xpID[j]],hsig[xpID[j]],fsig[xpID[j]],nm[j],optID[j],'N/A','N/A',ra_opt[j],dec_opt[j],'0','-1','-1','99.','99.','99.',p606[gpidACS],p814[gpidACS],prob[j],probnone[j],inACS[j],format='(1(I," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",I," ",A," ",A," ",A," ",G," ",G," ",I," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," ",G," "," ",I," "))' &$
