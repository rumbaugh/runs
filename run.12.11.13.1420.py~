...
FILE = open('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/chisq_results.12.9.13.dat','w')
...
for lens in lens_dict:
    ...
    for img in range(0,4):
        if lens != '0712': FILE.write("%3s %7.2f %7.3f %6.4f "%(imgarr[img],chisq_arr[img],chisq_arr[img]/len(g),prob_arr[img]))
lens = '0712'
cr = np.loadtxt('/home/rumbaugh/EVLA/light_curves/%s_g.edt'%(lens))
nimg = lens_dict[lens]['images']
testlegs = []
...
for img in range(1,nimg):
    ...
    Serr /= Smean
    imgarr = ['A+B','C','D','NA']
    chisq_arr[img-1],prob_arr[img-1] = calc_chi_sqrd(S[g]/CSOnorm,Serr[g]/CSOnorm,calcprob=True)  
...
for img in range(0,4):
    FILE.write("%3s %7.2f %7.3f %6.4f "%(imgarr[img],chisq_arr[img],chisq_arr[img]/len(g),prob_arr[img]))
FILE.close()
