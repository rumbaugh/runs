...
tp0s,tp2s,tp4s,tp3s,tp02s,tp22s,tp42s,tp32s,tp0vs,tp2vs,tp4vs,tp3vs,tp02vs,tp22vs,tp42vs,tp32vs = np.append(tp0s,np.zeros(3)),np.append(tp2s,np.zeros(3)),np.append(tp3s,np.zeros(3)),np.append(tp4s,np.zeros(3)),np.append(tp02s,np.zeros(3)),np.append(tp22s,np.zeros(3)),np.append(tp32s,np.zeros(3)),np.append(tp42s,np.zeros(3)),np.append(tp0vs,np.zeros(3)),np.append(tp2vs,np.zeros(3)),np.append(tp3vs,np.zeros(3)),np.append(tp4vs,np.zeros(3)),np.append(tp02vs,np.zeros(3)),np.append(tp22vs,np.zeros(3)),np.append(tp32vs,np.zeros(3)),np.append(tp42vs,np.zeros(3))
FILEN = open('/home/rumbaugh/temp.powrat_2.2.15.12.dat','w')

for i in range(0,len(names)):
    if i < len(names)-2.1:
        xfile = '/scratch/rumbaugh/ciaotesting/%s/master/acis%s.img.'%(strucs[i],chandraIDs[i])
        if i == 3: xfile = '/scratch/rumbaugh/ciaotesting/%s/master/RXJ1757.img.'%(strucs[i])
        if ((i == 5) | (i == 6)): xfile = '/scratch/rumbaugh/ciaotesting/Cl1604/6932/acis%s.img.'%(chandraIDs[i])
    else:
        xfile = '/home/rumbaugh/ChandraData/0910/master/acis%s.img.'%(chandraIDs[i])
    if i > 5:
        cr = read_file(xfile + '500-2000.nops.fits')
    #load_data(xfile + '500-2000.nops.fits')
    #emreg = '/home/rumbaugh/ChandraData/%s/master/embg.%s.2.14.12.reg'%(strucs[i],names[i])
    #set_coord('physical')
    #bgcnts1 = calc_data_sum2d(reg=emreg)
        pvals1 = copy_piximgvals(cr)
        cr = read_file(xfile + '2000-8000.nops.fits')
    #load_data(xfile + '2000-8000.nops.fits')
    #emreg = '/home/rumbaugh/ChandraData/%s/master/embg.%s.2.14.12.reg'%(strucs[i],names[i])
    #set_coord('physical')
    #bgcnts2 = calc_data_sum2d(reg=emreg)
        pvals2 = copy_piximgvals(cr)
        pvals = pvals1 + pvals2
        pvals1,pvals2 = 0,0
    #bgcnts = bgcnts1 + bgcnts2
        pvshape = np.shape(pvals)
        cp1 = time.time()
        tp0 = P_0(pvals,imcensX[i],imcensY[i],srchdist[i])-bgcnts[i]*srchdist[i]**2/((0.492*0.492)*regsizes[i])
        ...
        tp02 = P_0(pvals,imcensX[i],imcensY[i],srchdist[i]*0.5)-bgcnts[i]*(srchdist[i]*0.5)**2/((0.492*0.492)*regsizes[i])
        ...
        tp0v = m.sqrt((P_0_var(pvals,imcensX[i],imcensY[i],srchdist[i]))**2+bgcnts[i]*(srchdist[i]/(0.492))**4/(regsizes[i]**2))
        ...
        tp02v = m.sqrt((P_0_var(pvals,imcensX[i],imcensY[i],srchdist[i]*0.5))**2+bgcnts[i]*(srchdist[i]*0.5/(0.492))**4/(regsizes[i]**2))
        ...
        FILEN.write('%E %E %E %E %E %E %E %E %E %E %E %E %E %E %E %E\n'%(tp0,tp2,tp3,tp4,tp02,tp22,tp32,tp42,tp0v,tp2v,tp3v,tp4v,tp02v,tp22v,tp32v,tp42v))
        pvals = 0
