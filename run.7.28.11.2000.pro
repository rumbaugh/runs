...
set_plot,'PS'
loadct,'13'
device,file="/home/rumbaugh/Lum+RSplots.7.28.11.ps",/color
totalslum = []
totalhlum = []
yslum = []
yhlum = []
...
for i=0,4 do begin &$
   ...
   if gic[0] gt -0.1 then totalslum = [totalslum,slum[ors[gic]]] &$
   if gic[0] gt -0.1 then totalhlum = [totalhlum,hlum[ors[gic]]] &$
   if gic[0] gt -0.1 then rsslum = [rsslum,slum[ors[gic]]] &$
   if gic[0] gt -0.1 then rshlum = [rshlum,hlum[ors[gic]]] &$
