...
cr=np.loadtxt('/home/rumbaugh/Chandra/AGN_CKP_stat.dat',dtype={'names':('group','num_AGN_CKP','num_AGN','num_all','AGN_CKP_frac','all_CKP_frac'),'formats':('|S24','f8','f8','f8','f8','f8')})
...
for i in range(0,len(cr['group'])):
    a,allerr,b,agnerr,c,agnfracerr=CKPtest(cr['all_CKP_frac'][i],cr['num_all'][i],cr['num_AGN'][i])
    FILE.write('%18s %i %2i %3i %.4f %.4f %.4f\n'%(cr['group'][i],cr['num_AGN_CKP'][i],cr['num_AGN'][i],cr['num_all'][i],agnerr,agnfracerr,allerr))
FILE.close()
