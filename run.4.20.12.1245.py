...
FILE4 = open('/home/rumbaugh/veldisp.results_wo_rounding.4.11.12.dat','w')
for i in range(0,len(names)):
    FILE5 = open('/home/rumbaugh/veldisp_calc_data.%s.dat'%(names[i]),'w')
    ...
    sig,sige = biweight_scale(velstemp[gRGPtemp],ConfInv=1,ConfInvTrials=1000000)
    for im in range(0,len(gRGPtemp)):
        gidt = np.where(sID[g[gRGP[gRGPtemp]]] == sID[g[gRGP[gRGPtemp[im]]]])
        if len(gidt[0]) > 1: print sID[g[gRGP[gRGPtemp[gidt[0]]]]]
    #print sID[g[gRGP[gRGPtemp]]]
    print '%s sig = %f +/- %f using %i gals\nrad: %f'%(names[i],sig,sige,len(gRGPtemp),srchdist[i])
    if names[i] == 'RXJ1821':
        for ifl in range(0,len(gRGPtemp)): FILE5.write('%s %f %f %f %f %i\n'%(sID[g[gRGP[gRGPtemp[ifl]]]],sRA[g[gRGP[gRGPtemp[ifl]]]],sDec[g[gRGP[gRGPtemp[ifl]]]],sz[g[gRGP[gRGPtemp[ifl]]]],velstemp[gRGPtemp[ifl]],sq[g[gRGP[gRGPtemp[ifl]]]]))
    gq = np.where(sq > 2.2)
    ...
    vels2 = np.copy(velstemp2)
    pylab.hist(velstemp2[gd2],bins=30,range=[-4000,4000])
    #pylab.savefig('/home/rumbaugh/zhist.temp.%s.4.2.12.png'%(names[i]))
    pylab.close('all')

    #print 'a'
    gRGP2 = np.where((velstemp2 < 4000) & (velstemp2 > -4000) & (dists2 < srchdist[i]))
    gRGP2 = gRGP2[0]
    FILEz = open('/home/rumbaugh/rostat/rin%s.4.11.12.dat'%(names[i]),'w')
    for iz in range(0,len(gRGP2)): FILEz.write('%f\n'%(sz[gq[gRGP2[iz]]]))
    FILEz.close()
    gRGPtemp = np.arange(len(gRGP2))
    meanRGP = biweight_loc(velstemp2[gRGP2])
    #meanRGP = np.average(velstemp2[gRGP2])
    #print np.average(velstemp2[gRGP2]),biweight_loc(velstemp2[gRGP2])
    velstemp2 = velstemp2[gRGP2]
    sig = biweight_scale(velstemp2)
    prevRGP=9999
    #print 'a'
    while len(gRGPtemp) < prevRGP:
        #if names[i] == 'Cl1324+3013': print velstemp2,meanRGP,sig,sID[gq[gRGP2[gRGPtemp]]]
        prevRGP = len(gRGPtemp)
        gRGPtemp = sigclip(velstemp2,meanRGP,sig)
        velstemp2 = (z[gq[gRGP2]]-biweight_loc(sz[gq[gRGP2[gRGPtemp]]]))*(3.0*10**5)/(1+sz[gq[gRGP2]])
        #velstemp = (z[gq[gRGP2]]-np.average(sz[gq[gRGP2[gRGPtemp]]]))*(3.0*10**5)/(1+sz[gq[gRGP2]])
        meanRGP = biweight_loc(velstemp2[gRGPtemp])
        sig = biweight_scale(velstemp2[gRGPtemp])
        #if names[i] == 'Cl1324+3013': print sID[gq[gRGP2[gRGPtemp]]]
    sig,sige = biweight_scale(velstemp2[gRGPtemp],ConfInv=1,ConfInvTrials=1000000)
    if names[i] != 'RXJ1821':
        for ifl in range(0,len(gRGPtemp)): FILE5.write('%s %f %f %f %f %i\n'%(sID[gq[gRGP2[gRGPtemp[ifl]]]],sRA[gq[gRGP2[gRGPtemp[ifl]]]],sDec[gq[gRGP2[gRGPtemp[ifl]]]],sz[gq[gRGP2[gRGPtemp[ifl]]]],velstemp2[gRGPtemp[ifl]],sq[gq[gRGP2[gRGPtemp[ifl]]]]))
    for im in range(0,len(gRGPtemp)):
        gidt = np.where(sID[gq[gRGP2[gRGPtemp]]] == sID[gq[gRGP2[gRGPtemp[im]]]])
    FILEz = open('/home/rumbaugh/rostat/redshifts.%s.rostat_input.4.11.12.dat'%(names[i]),'w')
    for iz in range(0,len(gRGPtemp)):
        #if names[i] == 'Cl1324+3013': print '%s'%(sID[gq[gRGP2[gRGPtemp[iz]]]])
        FILEz.write('%f\n'%(sz[gq[gRGP2[gRGPtemp[iz]]]]))
    FILEz.close()
    print '%s newsig = %f +/- %f using %i gals\n'%(names[i],sig,sige,len(gRGPtemp))
    zub_fv = (3000.0/c+biweight_loc(sz[gq[gRGP2[gRGPtemp]]]))/(1-3000.0/c)
    zlb_fv = (-3000.0/c+biweight_loc(sz[gq[gRGP2[gRGPtemp]]]))/(1+3000.0/c)
    FILE4.write('%12s %6.1f %5.1f %3i %f %f %f %f\n'%(names[i],sig,sige,len(gRGPtemp),RGalPeakRA[i],RGalPeakDec[i],zlb_fv,zub_fv))
    FILE5.close()
FILE4.close()
