...
#psfpdf=bpdf.PdfPages('/home/rumbaugh/Chandra/plots/spatial_plots.w_PSdists.r500.3.12.16.pdf')
date='3.12.16'
...
for i in range(0,len(crx['RA'])):
    ...
    clusdist[i],veldist[i],PSdist[i],velnear[i]=tmpclusdistnoz[gPS],tmpzdist[gPS],tmpPSdist[gPS],clus_sig[gnoz][gPS]
...
for field in targets: 
    ...
    tmpallvelnear=np.ones(len(gq))*-1
    tmpallPSdist=np.ones(len(gq))*-1
    for i in range(0,len(gq)):
        zx=crs['z'][gq[i]]
        tmpzdist=np.abs(((1+zx)**2-(1+zs[g])**2)/((1+zx)**2+(1+zs[g])**2))*3.*10.**5/clus_sig[g]
        r500 = 2*clus_sig[g]/(np.sqrt(500)*Hz[g])
        tmpdistnoz=60*SphDist(crs['ra'][gq[i]],crs['dec'][gq[i]],ras[g],decs[g])*Mpc[g]/r500
        tmpPSdist=tmpzdist*tmpdistnoz
        gPS=np.argsort(tmpPSdist)[0]
        tmpallclusdist[i],tmpallveldist[i],tmpallPSdist[i],tmpallvelnear[i]=tmpdistnoz[gPS],tmpzdist[gPS],tmpPSdist[gPS],clus_sig[g][gPS]
    allclusdist=np.append(allclusdist,tmpallclusdist)
    allveldist=np.append(allveldist,tmpallveldist)
    allvelnear=np.append(allvelnear,tmpallvelnear)
...
plt.xlabel('Clustocentric Distance (R/R_500)')
...
plt.xlim(-0,10)
plt.legend()
#plt.savefig('/home/rumbaugh/Chandra/plots/phase_space_diagram_r500.%s.png'%(date))
...
plt.tick_params(which='major',length=8,width=2,labelsize=14)
plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
allclusdist[allclusdist*allveldist==0],allveldist[allclusdist*allveldist==0]=1,10**-4
ccd[ccd*veldist==0],veldist[ccd*veldist==0]=1,10**-4
plt.scatter(allvelnear,np.log10(allclusdist*allveldist),color='b',s=2)
plt.scatter(velnear,np.log10(ccd*veldist),color='r',s=6)
#plt.scatter(ccd,veldist,color='r',s=16)
for coeff in [0.1,0.4,2]:
    plt.axvline(np.log10(coeff),lw=2,ls='--',color='k')
plt.ylabel('Phase Space Metric log[(R/R_500)*(V/vel_disp)]')
plt.xlabel('Cluster Velocity Dispersion (km/s)')
plt.title('Phase Space Metric vs. Velocity Dispersion of nearest Cluster')
#plt.ylim(-1,160)
plt.xlim(0,1200)
plt.savefig('/home/rumbaugh/Chandra/plots/phase_space_vs_veldisp.%s.png'%(date))
...
for coeff in [0.1,0.4,2]:
    plt.axvline(np.log10(coeff),lw=2,ls='--',color='k')
plt.xlabel('Phase Space Metric log[(R/R_500)*(V/vel_disp)]')
plt.ylabel('Number of Sources')
plt.title('Phase Space Histogram')
#plt.ylim(-0,20)
#plt.xlim(-0,20)
#plt.savefig('/home/rumbaugh/Chandra/plots/phase_space_hist_r500.%s.png'%(date))
...
plt.scatter(allclusdist,allveldist,color='k',s=1)
plt.scatter(ccd,veldist,color='r',s=16)
plt.xlabel('Clustocentric Distance log(R/R_500)')
...
plt.ylim(ylim)
plt.legend(loc=3)
#plt.savefig('/home/rumbaugh/Chandra/plots/phase_space_diagram_logplot_r500.%s.png'%(date))
