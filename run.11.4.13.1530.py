...
for target in targets:
    insearch = 'HST.%s*raw.fits'%target
    outroot = 'ADrizOut.HST.%s.pf_%4.2f.fs_%5.3f.cte.%s'%(target,pf,fs,date)
    os.chdir(indir)
    iraf.unlearn('astrodrizzle')
    #astrodrizzle.AstroDrizzle('%s'%(insearch),output='%s'%(outroot),final_scale=fs,final_pixfrac=pf)
    matt_astrodrizzle(indir,outdir,outroot,insearch=insearch,doCTE=True,fscale=fs,pixfrac=pf,finalkernel=fk,dotest=True)
