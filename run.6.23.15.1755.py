...
for tau,it in zip(np.array([5,10,15,20]),np.array([0,1,2,3])):
    ...
    for i,pair in zip(pairs-1,pairs):
        mus=cr[:,2*it+1]
        mu=mus[i]
        crpost=reviewresults(pair,tau,mu,showresults=False)
    ...
    figure(25)
    clf()
    #scatter(taus[avg_fwidth!=0],np.abs(wt_tau[avg_fwidth!=0]-taus[avg_fwidth!=0]),s=4)
    #g=np.where(taus[avg_fwidth!=0]/avg_fwidth[avg_fwidth!=0]<3.502)[0]
    #scatter(taus[avg_fwidth!=0][g],np.abs(wt_tau[avg_fwidth!=0][g]-taus[avg_fwidth!=0][g]),s=4,color='cyan')
    ...
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/Timebombs_accuracy_vs_tau_%i.weight_cen.%s.png'%(tau,date))
    figure(29)
    clf()
    ...
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/Timebombs_bias_vs_flarewidths.tau_%i.weight_cen.%s.png'%(tau,date))
    #figure(26)
    #clf()
