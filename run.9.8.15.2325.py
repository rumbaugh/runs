...
pairs = np.delete(pairs,np.array([9,13,24,49,51,67,70,73,97,99,100])-1)
npairs=len(pairs)
cutoff=999
...
def reviewresults(pair,suffix,ldate='9.8.15',ddate='9.8.15',cbase='/home/rumbaugh/Fermi/TimeBombs/output/',showresults=False,ntrials=10000,testplot=False,errstd=np.sqrt(2)*7.5,T='ACFT',showfullsample=False):
    sampleFile='%ssample.testlightcurve_%i%s_notau.%s.dat'%(cbase,pair,suffix,ldate)
    sample_infoFile='%ssample_info.testlightcurve_%i%s_notau.%s.dat'%(cbase,pair,suffix,ldate)
    levelsFile='%slevels.testlightcurve_%i%s_notau.%s.dat'%(cbase,pair,suffix,ldate)
    DataFile='/home/rumbaugh/Fermi/data/test/testlightcurve_%i%s_notau.%s.dat'%(pair,suffix,ddate)
    weightFile='%sweights.testlightcurve_%i%s_notau.%s.dat'%(cbase,pair,suffix,ldate)
    posteriorFile='%sposterior.testlightcurve_%i%s_notau.%s.dat'%(cbase,pair,suffix,ldate)
    plotfilebase='testlightcurve_%i%s_notau.%s.png'%(pair,suffix,ldate)
    ...
    #PPMC(T,posteriorFile,DataFile,ntrials,plotfile=None,plotfile2='/home/rumbaugh/Fermi/TimeBombs/plots/PPMC_Td_vs_tau_%s_testlightcurve_%i%s_notau.%s.png'%(T,pair,date),figN=7,testplot=testplot,errstd=errstd,truetau=tau)
...
for i,pair,j in zip(pairs-1,pairs,np.arange(npairs)):
    for suffix in ['','_A','_B']:
        ...
        if pair<cutoff:
            crpost=reviewresults(pair,suffix,showresults=False)
        else:
            crpost=reviewresults(pair,suffix,showresults=True)
        #crFP = np.loadtxt('/home/rumbaugh/Fermi/data/test/testlightcurve_%i%s_notau.flare_params_truthvalues.%s.dat'%(i+1,'',ddate))
        #if suffix=='_A':
        ...
        draw()
        savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_hist.testlightcurve_%i%s_notau.AIC.%s.png'%(i+1,suffix,date))
        ...
        ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
        draw()
        savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_hist.testlightcurve_%i%s_notau.BIC.%s.png'%(i+1,suffix,date))
fullabstaus=np.abs(fulltaus)
halfabstaus=np.abs(halftaus)
dblfullabstaus=np.zeros(len(halfabstaus))
dblfullabstaus[np.arange(0,len(halfabstaus),2)],dblfullabstaus[np.arange(0,len(halfabstaus),2)+1]=fullabstaus,fullabstaus
halfdiffs=np.abs(halfabstaus[np.arange(0,len(halfabstaus),2)]-halfabstaus[np.arange(0,len(halfabstaus),2)+1])
fulldiffs=np.zeros(len(halfabstaus))
fulldiffs[np.arange(0,len(halfabstaus),2)]=np.abs(fullabstaus-halfabstaus[np.arange(0,len(halfabstaus),2)])
fulldiffs[np.arange(0,len(halfabstaus),2)+1]=np.abs(fullabstaus-halfabstaus[np.arange(0,len(halfabstaus),2)+1])

figure(28)
clf()
hist(halfdiffs,bins=45,range=(0,225))
xlabel('Difference in Posterior Time Delay Between Half Ranges(days)')
ylabel('Fraction of Posterior')
title('Difference in Half Range Time Delays, Null Test')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
...
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/halfdiffs_notau.tau_posterior.8.19.15.png')


figure(27)
clf()
hist(fulldiffs,bins=45,range=(0,225))
xlabel('Difference in Posterior Time Delay Between Full and Half Range(days)')
ylabel('Fraction of Posterior')
title('Full vs. Half Range Time Delays, Null Test')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
...
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/fulldiffs_notau.tau_posterior.8.19.15.png')



figure(27)
clf()
hist(fullabstaus,bins=45,range=(0,225))
xlabel('Posterior Time Delay (days)')
ylabel('Fraction of Posterior')
title('Full Length Time Delays, Null Test')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
def mjrFormatter(x, pos, maxperc=len(fullabstaus)):
    return "%.2f"%(x*1./maxperc)
if ymax*1./len(fullabstaus) >= 0.5:
    fracstep=0.1
elif ymax*1./len(fullabstaus) <= 0.1:
    fracstep=0.01
else:
    fracstep=0.05
maxytick=int(ymax*1./len(fullabstaus)/fracstep)+1
ytickstep=fracstep*len(fullabstaus)
yticks(np.arange(0,maxytick*ytickstep,ytickstep))
ax=gca()
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/fullabstaus_notau.tau_posterior.8.19.15.png')
...
figure(27)
clf()
scatter(halfabstaus,dblfullabstaus)
xlabel('Posterior Half Time Delay (days)')
ylabel('Posterior Full Time Delay (days)')
title('Time Delay Scatter, Null Test')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tauscatter_notau.tau_posterior.8.19.15.png')
