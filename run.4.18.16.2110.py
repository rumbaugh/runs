...
plt.hist(np.log10(PS_AGN[np.log10(crl['lum_full'][gsig])>=43.3]),color='r',bins=[-1.75,-1,np.log10(0.4),np.log10(2),1],normed=False)
plt.hist(np.log10(PS_AGN[np.log10(crl['lum_full'][gsig])<43.3]),color='b',bins=[-1.75,-1,np.log10(0.4),np.log10(2),1],normed=False,edgecolor='k',facecolor='None',lw=4)
#plt.hist(np.log10(PS_all),color='b',bins=[-1.75,-1,np.log10(0.4),np.log10(2),1],normed=True,edgecolor='k',facecolor='None',lw=4)#weights=25*np.ones(len(allclusdist))*1./len(allclusdist),)
#plt.scatter(ccd,veldist,color='r',s=16)
#for coeff in [0.1,0.4,2]:
#    plt.axvline(np.log10(coeff),lw=2,ls='--',color='k')
...
plt.title('AGN Phase Space Histogram, divided by X-ray Luminosity')
#plt.ylim(-0,20)
plt.xlim(-1.75,1)
plt.savefig('/home/rumbaugh/Chandra/plots/phase_space_hist.lum_cats.%s.png'%(date))

print KStest(np.log10(PS_AGN[np.log10(crl['lum_full'][gsig])>=43.3]),np.log10(PS_AGN[np.log10(crl['lum_full'][gsig])<43.3]))
