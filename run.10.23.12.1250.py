...
crl = read_file('/home/rumbaugh/paperstuff/clus.lums.soft.10.19.12.dat')
...
crl_nf = read_file('/home/rumbaugh/paperstuff/clus.lums.bol.10.23.12.dat')
lumbolnf = copy_colvals(crl_nf,'col1')
lumbol500nf = copy_colvals(crl_nf,'col2')
lumbolTnf = copy_colvals(crl_nf,'col3')
lumbolerrnf = copy_colvals(crl_nf,'col4')
lumbol500errnf = copy_colvals(crl_nf,'col5')
lumbolTerrnf = copy_colvals(crl_nf,'col6')
...
pylab.errorbar(Temps, lumbol500nf, xerr=[TerrD,TerrU],yerr=[lumbol500errnf,lumbol500errnf],fmt='go',lw=2,capsize=5,mew=2)
...
pylab.savefig('/home/rumbaugh/LxT.plot.bol.10.23.12.png')
pylab.close('all')
...
for i in range(0,len(names)):
    ...
    for j in range(0,len(lineX)):
        diststemp = m.sqrt(((Temps[i]-lineX[j])/Terrt)**2+((lumbol500[i]-lineY[j])/lumbol500err[i])**2)
...
pylab.errorbar(sigma, lumbolTnf, xerr=[sigerr,sigerr],yerr=[lumbolTerrnf,lumbolTerrnf],fmt='go',lw=2,capsize=5,mew=2)
...
for i in range(0,len(Temps)): 
    if i == 0: 
        ...
        pylab.text(sigma[i]-20,lumbolT[i]-0.22,names[i],color='black',fontsize=16,weight='extra bold')
    else:
        pylab.text(sigma[i]+10,lumbolT[i]+0.05,names[i],color='black',fontsize=16,weight='extra bold')
pylab.xticks([500,1000],['500','1000'])
pylab.yticks([1,5],['1','5'])
pylab.savefig('/home/rumbaugh/sigLx.plot.10.23.12.png')
pylab.close('all')
print '\nsig-Lx\n'
...
pylab.xticks([1,5],['1','5'])
pylab.savefig('/home/rumbaugh/sigT.plot.10.23.12.png')
pylab.close('all')
