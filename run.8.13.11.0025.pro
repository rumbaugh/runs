set_plot,'PS'
loadct,13
device,file='/home/rumbaugh/LumHist.allXray.8.13.11.ps',/color

maxy1 = [25,45,45,25,27]
maxy2 = [20,35,35,25,25]
maxy3 = [15,20,15,15,15]
...
if i eq 1 then gnt2 = where((nm gt 0.1) and (inACS gt 0.3)) &$
if i eq 1 then gnt3 = where((nm gt 0.1) and (inACS lt 0.3)) &$
for j=0,n_elements(gnt)-1 do begin &$
   ...
   if i eq 1 then gidl = where(LFCID eq optID[gnt[j]]) &$
   ...
   if ((i eq 1) and (nm[gnt[j]] gt 1.1)) then gidl2 = where(LFCID eq optID2[gnt[j]]) &$
   if ((gid[0] lt -0.1) and (gid2[0] gt -0.1)) then gid = gid2 &$
   if ((gidl[0] lt -0.1) and (gidl2[0] gt -0.1)) then gidl = gidl2 &$
   if ((gid[0] lt -0.1) and (gidl[0] gt -0.1)) then gid = gidl &$
   if ((gid[0] gt -0.1) and ((q[gid[0]] gt -0.3) and (q[gid[0]] lt 1.3))) then notarget[gnt[j]] = 1 &$
   if ((gid[0] gt -0.1) and ((q[gid[0]] lt -0.3) or (q[gid[0]] gt 0.3))) then notarget[gnt[j]] = -1 &$
...
sfluxesa = DBLARR(n_elements(sigmax)) &$
hfluxesa = DBLARR(n_elements(sigmax)) &$
slumsa = DBLARR(n_elements(sigmax)) &$
hlumsa = DBLARR(n_elements(sigmax)) &$
flumsa = DBLARR(n_elements(sigmax)) &$
...
for j=0,n_elements(sigmax)-1 do begin &$
   ...
   if ((i eq 1) and (DecX[j] gt (m1604*(RaX[j]-x01604)+y01604))) then time = exps[2] &$
   if ((i eq 1) and (DecX[j] gt (m1604*(RaX[j]-x01604)+y01604))) then c2fs = c2fsarr[2] &$
   if ((i eq 1) and (DecX[j] gt (m1604*(RaX[j]-x01604)+y01604))) then c2fh = c2fharr[2] &$
   if i eq 2 then c2fs = c2fsarr[3] &$
   if i eq 2 then c2fh = c2fharr[3] &$
   if i eq 2 then time = exps[3] &$
   if ((i eq 2) and (DecX[j] lt 30.6)) then c2fs = c2fsarr[4] &$
   if ((i eq 2) and (DecX[j] lt 30.6)) then c2fh = c2fharr[4] &$
   if ((i eq 2) and (DecX[j] lt 30.6)) then time = exps[4] &$
   if i ne 1 then sfluxesa[j] = sncnts[j]*c2fs/time &$
   if i ne 1 then hfluxesa[j] = hncnts[j]*c2fh/time &$
   if i eq 1 then sfluxesa[j] = sncnts[long(XID[j])]*c2fs/time &$
   if i eq 1 then hfluxesa[j] = hncnts[long(XID[j])]*c2fh/time &$
   slumsa[j] = sfluxesa[j]*fourpiDL2[i] &$ ;in units of 10^(-57)ergs/s
   hlumsa[j] = hfluxesa[j]*fourpiDL2[i] &$
   flumsa[j] = alog10(slumsa[j]+hlumsa[j])+57 &$ ;this is log luminosity
   endfor &$
...
gall2s = where(sigmax gt 2)
gall3s = where(sigmax gt 3)
gnt2s = where(sigmax[gnt] gt 2)
gnt3s = where(sigmax[gnt] gt 3)
flumsallg2st = [flums[g2s],flumst[g2st]]
flumsallg3st = [flums[g3s],flumst[g3st]]
plot,[0-1],[0-1],/nodata,XRANGE=[41.5,45],YRANGE=[0,maxy1[i]],XTITLE="Log Luminosity",YTITLE='Num. of X-ray Sources',TITLE='All X-ray Sources - ' + names[i],/xsty,/ysty,XTHICK=4,YTHICK=4,CHARTHICK=3 &$
Histoplot,flumsa[gall3s],BINSIZE=0.25,MININPUT=41.5,MAXINPUT=45.5,datacolorname='Navy Blue',/LINE_FILL,polycolor='Navy Blue',orientation=-45,spacing=0.2,THICK=2,/oplot &$
Histoplot,flumsa[gall2s],BINSIZE=0.25,MININPUT=41.5,MAXINPUT=45.5,datacolorname='Red',THICK=10,/oplot &$
plot,[0-1],[0-1],/nodata,XRANGE=[41.5,45],YRANGE=[0,maxy2[i]],XTITLE="Log Luminosity",YTITLE='Num. of X-ray Sources',TITLE='Opt. Matched X-ray Sources - ' + names[i],/xsty,/ysty,XTHICK=4,YTHICK=4,CHARTHICK=3 &$
Histoplot,flumsa[gnt[gnt3s]],BINSIZE=0.25,MININPUT=41.5,MAXINPUT=45.5,datacolorname='Navy Blue',/LINE_FILL,polycolor='Navy Blue',orientation=-45,spacing=0.2,THICK=2,/oplot &$
Histoplot,flumsa[gnt[gnt2s]],BINSIZE=0.25,MININPUT=41.5,MAXINPUT=45.5,datacolorname='Red',THICK=10,/oplot &$
plot,[0-1],[0-1],/nodata,XRANGE=[41.5,45],YRANGE=[0,maxy3[i]],XTITLE="Log Luminosity",YTITLE='Num. of X-ray Sources',TITLE='Spec. Matched X-ray Sources - ' + names[i],/xsty,/ysty,XTHICK=4,YTHICK=4,CHARTHICK=3 &$
Histoplot,flumst[g3st],BINSIZE=0.25,MININPUT=41.5,MAXINPUT=45.5,datacolorname='Navy Blue',/LINE_FILL,polycolor='Navy Blue',orientation=-45,spacing=0.2,THICK=2,/oplot &$
Histoplot,flumst[g2st],BINSIZE=0.25,MININPUT=41.5,MAXINPUT=45.5,datacolorname='Red',THICK=10,/oplot &$
