...
#for i in range(0,len(datearr)):
    ...
    fieldname = 'J2006+6424'
    fieldnum = '5'
    if SBgroupnum == '2':
        #if ((i != 13) & (i != 6) & (i != 14) & (i != 16)): execfile("/local3/rumbaugh/EVLA/scripts/export_CSOs.py")
        if False: execfile("/local3/rumbaugh/EVLA/scripts/export_CSOs.py")
    else:
        #outbase = '%sSB%s_%s.%s.%s.11.11A-138_1'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i])
        #if ((i != 13) & (i != 6) & (i != 14) & (i != 16)): execfile("/local3/rumbaugh/EVLA/scripts/export_CSOs.py")
        #outbase = '%sSB%s_%s.%s.%s.11.11A-138_2'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i])
        #fieldnum = '7'
        #if ((i != 13) & (i != 6) & (i != 14) & (i != 16)): execfile("/local3/rumbaugh/EVLA/scripts/export_CSOs.py")
        #vis = ['%sSB%s_%s.%s.%s.11.11A-138_1.%s.ms'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i],fieldname),'%sSB%s_%s.%s.%s.11.11A-138_2.%s.ms'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i],fieldname)]
        #concatvis = '%sSB%s_%s.%s.%s.11.11A-138.%s.ms'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i],fieldname)
        #concat(vis,concatvis)
        if ((i != 13) & (i != 6) & (i != 14) & (i != 16)): execfile("/local3/rumbaugh/EVLA/scripts/export_PC.py")
