...
plt.ylabel('Normalized Flux',fontsize=14)
#plt.ylim(0.7,1.3)
plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/B1938.lc_delay_plot.11.25.13.png')
#plt.close('all')
...
for img in range(0,2):
    ...
    if img == 1: 
        ltime += 45.5
        S,Serr = S*muold/np.mean(fluxdict[0]),Serr*muold/np.mean(fluxdict[0])
    else:
        S,Serr = S/np.mean(fluxdict[0]),Serr/np.mean(fluxdict[0])
    plt.plot(ltime+(img-1),S,color=color_arr[img],lw=1.5,label=imgname[img],ls=ls_arr[img])
    plt.errorbar(ltime+(img-1),S,yerr=Serr,color=color_arr[img],fmt='ro',lw=1,capsize=3,mew=1,label='_nolegend_')
...
plt.ylabel('Normalized Flux',fontsize=14)
#plt.ylim(0.7,1.3)
plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/B1938.lc_delay_plot_oldvals.11.25.13.png')
#plt.close('all')
