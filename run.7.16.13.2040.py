...
def doextract(mask,side,colors=['blue','red'],maxpeaks=3):
    ...
    for color in colors:
        try:
            osv = os.system("ln -sf %s%s_%s_%s_coadd_bgsub.fits ."%('/mnt/data2/rumbaugh/LRIS/2011_01/reduced/',mask,color,side))
            osv = os.system("ln -sf %s%s_%s_%s_coadd_bgsub.weight.fits ."%('/mnt/data2/rumbaugh/LRIS/2011_01/reduced/',mask,color,side))
