...
#    print "%s - 4th Position\n(RA,Dec) = (%s,%s)\ni'/f814 magnitude: %f\n"%(names[i],rastrs[3],decstrs[3],iB[g[gall[grs[garg[validBCGIDs[3]]]]]])
    ...
    FILE2 = open('/home/rumbaugh/BCG.anal.%s.gals_%4.2fMpc.2.6.12.reg'%(names[i],aperture),'w')
    ...
    for iw in range(0,len(garg)):
        ...
        if iw == 0: FILE2.write('point(%f,%f) # point=circle color=%s\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],'red'))
        if iw == 1: FILE2.write('point(%f,%f) # point=circle color=%s\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],'magenta'))
        if iw == 2: FILE2.write('point(%f,%f) # point=circle color=%s\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],'yellow'))
        if iw == 3: FILE2.write('point(%f,%f) # point=circle color=%s\n'%(RA[g[gall[grs[garg[iw]]]]],Dec[g[gall[grs[garg[iw]]]]],'cyan'))
