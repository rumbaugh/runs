...
tp0s,tp2s,tp4s,tp3s,tp0s2,tp2s2,tp4s2,tp3s2 = np.zeros(len(names)),np.zeros(len(names)),np.zeros(len(names)),np.zeros(len(names)),np.zeros(len(names)),np.zeros(len(names)),np.zeros(len(names)),np.zeros(len(names))
for i in range(0,len(names)):
    ...
    tp0s[i],tp2s[i],tp3s[i],tp4s[i],tp0s2[i],tp2s2[i],tp3s2[i],tp4s2[i] = tp0,tp2,tp3,tp4,tp02,tp22,tp32,tp42
    print '%s - 0.5 Mpc\nP_2/P_0: %E\nP_3/P_0: %E\nP_4/P_0: %E\n%s - 0.25 Mpc\nP_2/P_0: %E\nP_3/P_0: %E\nP_4/P_0: %E\n\n'%(names[i],tp2/tp0,tp3/tp0,tp4/tp0,names[i],tp22/tp02,tp32/tp02,tp42/tp02)
inds1 = np.arange(9)
width = 0.2
p1 = pylab.bar(inds1-width,tp2s/tp0s,width,log='True',color='blue')
p2 = pylab.bar(inds1,tp3s/tp0s,width,log='True',color='red')
p3 = pylab.bar(inds1+width,tp4s/tp0s,width,log='True',color='green')
pylab.title('Power Ratios within 0.5 Mpc')
pylab.xticks(inds1+0.5*width,('Cl1324\n+3011','Cl1324\n+3013','Cl1324\n+3059','RXJ-\n1757','RXJ-\n1821','Cl-\n1604A','Cl-\n1604B','RXJ-\n0910\n+5419','RXJ-\n0910\n+5422'),rotation='vertical')
pylab.legend((p1[0],p2[0],p3[0]),('P$_2$/P$_0$','P$_3$/P$_0$','P$_4$/P$_0$'))
pylab.savefig('/home/rumbaugh/powerratios_0.5Mpc.plot.1.30.12.png')
pylab.close('all')  
p1 = pylab.bar(inds1-width,tp2s2/tp0s2,width,log='True',color='blue')
p2 = pylab.bar(inds1,tp3s2/tp0s2,width,log='True',color='red')
p3 = pylab.bar(inds1+width,tp4s2/tp0s2,width,log='True',color='green')
pylab.title('Power Ratios within 0.25 Mpc')
pylab.xticks(inds1+0.5*width,('Cl1324\n+3011','Cl1324\n+3013','Cl1324\n+3059','RXJ-\n1757','RXJ-\n1821','Cl-\n1604A','Cl-\n1604B','RXJ-\n0910\n+5419','RXJ-\n0910\n+5422'),rotation='vertical')
pylab.legend((p1[0],p2[0],p3[0]),('P$_2$/P$_0$','P$_3$/P$_0$','P$_4$/P$_0$'))
pylab.savefig('/home/rumbaugh/powerratios_0.25Mpc.plot.1.30.12.png')
pylab.close('all')
