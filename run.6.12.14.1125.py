...
try:
    ...
    minmu = 0.9
lens = 'B0712'
for source in ['B0712','MG0414','B1938']:
    ...
    sampler = emcee.EnsembleSampler(nwalkers,ndim,delaylnprob,args=[fluxA[g],fluxB[g],Aerr[g],Berr[g],ltime[g],mu_init,mintime,maxtime,False,True,True,kernel,10.,0.5,seasonbounds])
    ...
    if runs < 50: 
        ...
        FILE = open('/home/rumbaugh/EVLA/light_curves/Dispersions/%s_jointfit.emcee_output_full_chain_%s_bs.%s.dat'%(lens,kernel,date),'w')
    ...
    plt.errorbar(ltime[g],fluxA[g]/np.mean(fluxA[g]),Aerr[g]/np.mean(fluxA[g]))
    plt.errorbar(ltime[g],fluxB[g]/np.mean(fluxB[g]),Berr[g]/np.mean(fluxB[g]))
    plt.scatter(ltime[g],fluxA[g]/np.mean(fluxA[g]))
    plt.scatter(ltime[g],fluxB[g]/np.mean(fluxB[g]))
    plt.xlabel('Time Delay (days)',fontsize=14)
    plt.ylabel('Magnification')
    plt.title('%s'%lens)
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/testplot.%s.%s.png'%(lens,date))
