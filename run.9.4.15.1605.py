...
blue_ID_list = np.array([366,367,368,369])
red_ID_list = np.array([342,343,350,351])
...
def do_redux(ID,side):
    ...
    mupoly,sigpoly=trace_spectrum(data[:,dlb:dub+1],mu0,sig0,stepsize=75,dispaxis='y',noblankcolumns=True)
    specm,varspec = extract_spectrum(data[:,dlb:dub+1],mupoly,sigpoly,dispaxis='y',var_in=var[:,dlb:dub+1])
    save_spectrum('/home/rumbaugh/KAST/Science/spec_skysub_noCR.%s%i_2.dat'%(side,ID),w,specm,var=varspec)
