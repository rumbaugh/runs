...
anninner = np.array([120,100,160,100,160,150,100,100])
...
for i in range(len(sigma)-3,len(sigma)):
    ...
    abs1.nh = 0.0
    fake_pha(1,get_arf(),get_rmf(),time)
    eflx2 = calc_energy_flux(2.0/(1+rsarr[i]),10.0/(1+rsarr[i]))
...
FILE=open('/home/rumbaugh/paperstuff/clus.lums.hard10.9.7.11.dat','w')
for i in range(0,len(cnt2flux)):
    FILE.write('%s %f %f %e\n'%(names[i],lums[i],lumerr[i],cnt2flux[i]))
    print '%s - count rate to flux conversion: %e\n'%(names[i],cnt2flux[i])
    print 'soft-band flux (10^-14 erg/s/cm^2): %f\n'%(fluxs[i])
