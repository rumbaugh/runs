...
for i in range(0,len(Temps)): 
    if i == 0: 
        ...
        pylab.text(Temps[i]-1.4,lumbol[i]-0.9,names[i],color='black',weight='extra bold')
    elif i == 4:
        aa = 1
    elif i == 5:
        pylab.text(Temps[i]+0.1,lumbol[i]-0.8,names[i],color='black',weight='extra bold')
    else:
        pylab.text(Temps[i],lumbol[i],names[i],color='black',weight='extra bold')
pylab.savefig('/home/rumbaugh/paperstuff/LxT.plot.bol.6.22.11.png')
pylab.close('all')
