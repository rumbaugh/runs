...
rinds=np.arange(0,len(halfabstaus),2)+1
np.random.shuffle(rinds)
randhalfdiffs=np.abs(halfabstaus[np.arange(0,len(halfabstaus),2)]-halfabstaus[rinds])
...
fullacc=np.abs(fullabstaus-taus[pairs-1])
halfacc=np.zeros(len(fullacc)*2)
halfacc[np.arange(0,len(fullacc)*2,2)]=np.abs(halfabstaus[np.arange(0,len(fullacc)*2,2)]-taus[pairs-1])
halfacc[np.arange(0,len(fullacc)*2,2)+1]=np.abs(halfabstaus[np.arange(0,len(fullacc)*2,2)+1]-taus[pairs-1])

figure(28)
clf()
hist(halfdiffs,range=(0,130),bins=26)
xlabel('Difference in Posterior Time Delay Between Half Ranges(days)')
ylabel('Fraction of Posterior')
title('Difference in Half Range Time Delays')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
...
figure(28)
clf()
hist(randhalfdiffs,range=(0,130),bins=26)
xlabel('Difference in Posterior Time Delay Between Random Half Ranges(days)')
ylabel('Fraction of Posterior')
title('Difference in Randomized Half Range Time Delays')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
...
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/randhalfdiffs.tau_posterior.8.19.15.png')


figure(27)
clf()
hist(fulldiffs,bins=40,range=(0,200))
xlabel('Difference in Posterior Time Delay Between Full and Half Range(days)')
ylabel('Fraction of Posterior')
title('Full vs. Half Range Time Delays')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
...
xlabel('Tau Accuracy')
ylabel('')
title('Time Delay Accuracy for Half (green) and Full (blue) Ranges')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
def mjrFormatter(x, pos, maxperc=len(halfacc)):
    return "%.2f"%(x*1./maxperc)
if ymax*1./len(halfacc) >= 0.5:
    fracstep=0.1
elif ymax*1./len(halfacc) <= 0.1:
    fracstep=0.01
else:
    fracstep=0.05
maxytick=int(ymax*1./len(halfacc)/fracstep)+1
ytickstep=fracstep*len(halfacc)
yticks(np.arange(0,maxytick*ytickstep,ytickstep))
ax=gca()
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tauacc.tau_posterior.8.19.15.png')

  
avghalfacc=0.5*(halfacc[np.arange(0,len(halfacc),2)]+halfacc[np.arange(1,len(halfacc),2)])
dblfullacc=np.zeros(len(halfacc))
dblfullacc[np.arange(0,len(halfacc),2)],dblfullacc[np.arange(1,len(halfacc),2)]=fullacc,fullacc


figure(30)
clf()
hist((halfacc-dblfullacc),bins=40)
title('Time Delay Improvement from Half to Full Range')
xlabel('Tau Accuracy Improvement')
ylabel('')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
...
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tauaccimprov.tau_posterior.8.19.15.png')



figure(31)
clf()
hist(halfabstaus,bins=40)
xlabel('Half Interval Time Delays (days)')
ylabel('Percentage of Posterior')
title('Half Range Time Delays')
xmin,xmax=xlim()
ymin,ymax=ylim()
xtmp=np.arange(xmin,xmax,(xmax-xmin)*1./10000)
xlim(xmin,xmax)
def mjrFormatter(x, pos, maxperc=len(halfabstaus)):
    return "%.2f"%(x*1./maxperc)
if ymax*1./len(halfabstaus) >= 0.5:
    fracstep=0.1
elif ymax*1./len(halfabstaus) <= 0.1:
    fracstep=0.01
else:
    fracstep=0.05
maxytick=int(ymax*1./len(halfabstaus)/fracstep)+1
ytickstep=fracstep*len(halfabstaus)
yticks(np.arange(0,maxytick*ytickstep,ytickstep))
ax=gca()
ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
draw()
savefig('/home/rumbaugh/Fermi/TimeBombs/plots/halfabstaus_hist.tau_posterior.8.19.15.png')
