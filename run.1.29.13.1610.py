...
#find time delays
chisqmatAB = chi_squared_min_delays(Aflux,Bflux,ltime,ltime,Aerr,Berr,t_grid,maxtimestep,timestep,minmu,maxmu,mustep,smooth_param=np.array([10.]),output=3,use_overlap_mean=True,ALAG=ALAG,chisqmatrix=True)
pylab.imshow(chisqmatAB,extent=[minmu,maxmu,-1*maxtimestep,maxtimestep],aspect='auto',origin='lower')
pylab.savefig('/home/rumbaugh/chisq_plot.1608_delay_AB.1.29.13.png')
pylab.close('all')
t1 = time.time()
print "33%% Done. Elapsed Time: %f seconds"%(t1-st1)
if not justone:
    chisqmatBC = chi_squared_min_delays(Cflux,Bflux,ltime,ltime,Cerr,Berr,t_grid,maxtimestep,timestep,minmu,maxmu,mustep,smooth_param=np.array([5]),output=3,use_overlap_mean=True,ALAG=CLAG,chisqmatrix=True)
    pylab.imshow(chisqmatBC,extent=[minmu,maxmu,-1*maxtimestep,maxtimestep],aspect='auto',origin='lower')
    pylab.savefig('/home/rumbaugh/chisq_plot.1608_delay_BC.1.29.13.png')
    pylab.close('all')
    t2 = time.time()
    print "66%% Done. Elapsed Time: %f seconds"%(t2-st1)
    chisqmatBD = chi_squared_min_delays(Dflux,Bflux,ltime,ltime,Derr,Berr,t_grid,maxtimestep,timestep,minmu,maxmu,mustep,smooth_param=np.array([5]),output=3,use_overlap_mean=True,ALAG=DLAG,chisqmatrix=True)
    pylab.imshow(chisqmatBD,extent=[minmu,maxmu,-1*maxtimestep,maxtimestep],aspect='auto',origin='lower')
    pylab.savefig('/home/rumbaugh/chisq_plot.1608_delay_BD.1.29.13.png')
    pylab.close('all')
t3 = time.time()
