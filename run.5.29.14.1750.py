...
FILE=open('/home/rumbaugh/runs/run.5.29.14.1750.sh','w')
FILE.write("#!/bin/csh\n")

opening_string = "difmap << EOF\ninteger mfitniter\nmfitniter = 7\nlogical varpos\nvarpos = false\n"
setup_string = "select I\nmapunits arcsec\nmapsize 256,0.05\n"
fit_string = "uvtaper %f,%i\nmodelfit mfitniter\nselfcal false,false,60\nmodelfit mfitniter\nselfcal false,false, 60\nmodelfit mfitniter\nselfcal false,false,60\nmodelfit mfitniter\nselfcal false,false, 60\nmodelfit mfitniter\n"%(gval,uvmax)
end_string = "quit\nEOF\n"
...
for date,aipsdate in zip(dates,aips_dates):
    ...
    for source in tsources:
        ...
        outfile = '/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_UVT_0.001_300_5.29.14.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
