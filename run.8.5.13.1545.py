...
def delaylnprob(x,A,B,A_err,B_err,ltime,idelay,maxtime=117,mintime=-10,t_grid_spacing=0.5,smooth_param=10.):
    ...
    if ((x[0] > maxtime) | (x[0] < mintime)):
        ...
        if x[0] < 0:
            gltime1,gltime2,gltime3 = np.where(ltime1 < ltime1.max()+x[0]),np.where(ltime2 < ltime2.max()+x[0]),np.where(ltime3 < ltime3.max()+x[0])
        else:
            gltime1,gltime2,gltime3 = np.where(ltime1 > x[0]),np.where(ltime2 > x[0]+ltime2.min()),np.where(ltime3 > x[0]+ltime3.min())
        ...
        smB,smB_var = sm.gauss(ltime+x[0],Btmp,np.append(ltime1[gltime1],np.append(ltime2[gltime2],ltime3[gltime3])),smooth_param,y_var=Berrtmp*Berrtmp)
...
tau_init,mu3_init = np.array([31.5,36.5,85.5]),np.array([2.0124,1.03083,0.3471])
...
for i in range(0,3):
    ...
    sampler = emcee.EnsembleSampler(nwalkers,ndim,delaylnprob,args=[fluxes_dict[i+1][0],Bflux,lens2_errs[i],Berr,ltime,i])
    ...
    py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_hist_tau.B%s_delay.%s.gauss.ps'%(lens2_names[i],date))
    py.close()
    py.plot(sampler.flatchain[:,1])
    py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_chain_plot_mu1.B%s_delay.%s.gauss.ps'%(lens2_names[i],date))
    py.close()
    py.plot(sampler.flatchain[:,2])
    py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_chain_plot_mu2.B%s_delay.%s.gauss.ps'%(lens2_names[i],date))
    py.close()
    py.plot(sampler.flatchain[:,3])
    py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_chain_plot_mu3.B%s_delay.%s.gauss.ps'%(lens2_names[i],date))
    py.close()
    py.plot(sampler.flatchain[:,0])
    py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_chain_plot_tau.B%s_delay.%s.gauss.ps'%(lens2_names[i],date))
    py.close()
    for j in range(0,nwalkers):
        py.plot(sampler.chain[j,:,0])
        py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_all_chain_plot_tau.B%s_delay.%s.gauss.ps'%(lens2_names[i],date))
    py.close()
    for k in range(1,4):
        for j in range(0,nwalkers):
            py.plot(sampler.chain[j,:,k])
            py.savefig('/home/rumbaugh/B1608_files/B1608.emcee_output_all_chain_plot_mu%i.B%s_delay.%s.gauss.ps'%(k,lens2_names[i],date))
        py.close()
