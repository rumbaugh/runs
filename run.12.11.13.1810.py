...
FILE = open('/mnt/data2/rumbaugh/TDC/tdc1sample_for_test/emcee_anal.tau.extrange.%s.dat'%date,'w')
...
for rung in range(0,5):
    for i in range(0,9):
        ...
        cr = np.loadtxt('/mnt/data2/rumbaugh/TDC/tdc1sample_for_test/rung%i/emcee/%s.emcee_output_hist_tau_extended.%s.dat'%(rung,pair,date))
        tau_sort = np.sort(cr)
        inrange = 'Yes'
        if ((np.median(tau_sort) < ref_dict['rung'][rung][pair]['range'][0]) | (np.median(tau_sort) > ref_dict['rung'][rung][pair]['range'][1])): inrange = 'No'
        FILE.write('%25s %5.1f %5.1f %5.1f %3i %3i %3s\n'%(pair,np.median(tau_sort),tau_sort[int(0.16*len(tau_sort))],tau_sort[int(0.84*len(tau_sort))],ref_dict['rung'][rung][pair]['range'][0],ref_dict['rung'][rung][pair]['range'][1],inrange))
FILE.close()
