...
def sim_lc(lens):
    ...
    ttmp,mtmp,BA_disp = calc_disp_delay(eflux_t1,eflux_t2,days_t,days_t,flux_err1,flux_err2,maxtimestep,timestep,minmu,maxmu,mustep,'D_4_2',delta,output=2,opt=False,outfile='%s/../Disp_grid_out.%s.delta_10.5_%s.dat'%(proot,lens,date))
    print mtmp,mu,np.mean(y)/np.mean(y2)
    print ttmp,lag
    ttmp,mtmp,BA_disp = calc_disp_delay(eflux_t1,eflux_t2,days_t,days_t,flux_err1,flux_err2,maxtimestep,timestep,minmu,maxmu,mustep,'D_4_2',delta,output=2,,opt=True,outfile='%s/../Disp_grid_out.%s.delta_10.5_%s.dat'%(proot,lens,date))
    print mtmp,mu,np.mean(y)/np.mean(y2)
    print ttmp,lag
