...
for i in range(0,1): 
    ...
    os.system('ln -sf ../CALDB_PHOT_IMP/%s_caldb_phot/soft/stats.radec_netcntscor_netcnts_cnts_sig_flux.soft.dat .'%(names[i]))
    os.system('ln -sf ../CALDB_PHOT_IMP/%s_caldb_phot/hard/stats.radec_netcntscor_netcnts_cnts_sig_flux.hard.dat .'%(names[i]))
    #### Read in Photometry catalogs
    photcat1 = np.loadtxt('stats.radec_netcntscor_netcnts_cnts_sig_flux.soft.dat')
    id_soft,ra_soft,dec_soft,netcnts_corr_soft,netcnts_soft,counts_soft,sig_soft,offaxis_soft,flux_soft,wmask_soft,wsigf_soft,wsigs_soft,wsigh_soft,psfA_soft,psfB_soft,ang_soft = photcat1[:,0],photcat1[:,1],photcat1[:,2],photcat1[:,3],photcat1[:,4],photcat1[:,5],photcat1[:,6],photcat1[:,7],photcat1[:,8],photcat1[:,9],photcat1[:,10],photcat1[:,11],photcat1[:,12],photcat1[:,13],photcat1[:,14],photcat1[:,15]
    photcat2 = np.loadtxt('stats.radec_netcntscor_netcnts_cnts_sig_flux.hard.dat')
    id_hard,ra_hard,dec_hard,netcnts_corr_hard,netcnts_hard,counts_hard,sig_hard,offaxis_hard,flux_hard,wmask_hard,wsigf_hard,wsigs_hard,wsigh_hard,psfA_hard,psfB_hard,ang_hard = photcat2[:,0],photcat2[:,1],photcat2[:,2],photcat2[:,3],photcat2[:,4],photcat2[:,5],photcat2[:,6],photcat2[:,7],photcat2[:,8],photcat2[:,9],photcat2[:,10],photcat2[:,11],photcat2[:,12],photcat2[:,13],photcat2[:,14],photcat2[:,15]
    ...
    FILE = open(strname + '.xray_phot_imp.soft_hard_full.dat','w')
    for i in range(0,len(ra)):
        FILE.write("%f %f %E %E %E %E %E %E %f %f %f %f %f %f %i %f %f %f %f %f %f\n"%(ra[i], dec[i], flux_softz[i], flux_hardz[i], flux_fullz[i],  netcnts_corr_softz[i], netcnts_corr_hardz[i], netcnts_corr_fullz[i], sig_soft[i], sig_hard[i], sig_full[i], wsig_soft[i], wsig_hard[i], wsig_full[i], wmask[i],psfA_soft[i],psfB_soft[i],ang_soft[i],psfA_hard[i],psfB_hard[i],ang_hard[i]))
    FILE.close()
