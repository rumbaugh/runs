...
set_plot,'PS'
loadct,'13'
device,file="/home/rumbaugh/testLumvsRSplot.7.29.11.ps",/color
totalslum = []
totalhlum = []
yslum = []
yhlum = []
...
for i=0,4 do begin &$
   ...
   if i lt 0.5 then rsfity = [rsfity,rsfitt[nors[gic2]]] &$
   if i gt 1.5 then rsfito = [rsfito,rsfitt[nors[gic2]]] &$
   rsfit = [rsfit,rsfitt[ors[gic]]] &$
   if ((i gt 1.5) and (gic[0] gt -0.1)) then rsfito = [rsfito,rsfitt[ors[gic]]] &$
   if i lt 0.5 then rsfity = [rsfity,rsfitt[ors[gic]]] &$
   ictemp[gic] = 1 &$
   ic = [ic,ictemp] &$
   flum = slum+hlum &$
   slum += 0.0000000001
   ...
   slum = alog10(slum)+42
   hlum = alog10(hlum)+42
   totalslum = [totalslum,slum[nors[gic2]]] &$
   totalhlum = [totalhlum,hlum[nors[gic2]]] &$
   totalslum = [totalslum,slum[ors[gic]]] &$
   totalhlum = [totalhlum,hlum[ors[gic]]] &$
   nrsslum = [nrsslum,slum[nors[gic2]]] &$
   nrshlum = [nrshlum,hlum[nors[gic2]]] &$
   rsslum = [rsslum,slum[ors[gic]]] &$
   rshlum = [rshlum,hlum[ors[gic]]] &$
   if i lt 0.5 then yslum = [yslum,slum[nors[gic2]]] &$
   if i lt 0.5 then yhlum = [yhlum,hlum[nors[gic2]]] &$
   if i gt 1.5 then oslum = [oslum,slum[nors[gic2]]] &$
   if i gt 1.5 then ohlum = [ohlum,hlum[nors[gic2]]] &$
   if i lt 0.5 then yhlum = [yhlum,hlum[ors[gic]]] &$
   if i lt 0.5 then yslum = [yslum,slum[ors[gic]]] &$
   if ((i gt 1.5) and (gic[0] gt -0.1)) then ohlum = [ohlum,hlum[ors[gic]]] &$
   if ((i gt 1.5) and (gic[0] gt -0.1)) then oslum = [oslum,slum[ors[gic]]] &$
   nrsslum = [nrsslum,slum[nors[gic2]]] &$
