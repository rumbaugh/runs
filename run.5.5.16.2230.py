...
FILE=open('/home/rumbaugh/Chandra/lums_newphotoz_adam.1sig.5.5.16.dat','w')
FILE.write('# ID RA Dec z_peak nh lum_soft lum_hard lum_full\n')
cr=np.loadtxt('/home/rumbaugh/Chandra/fluxes_newphotoz_adam.1sig.5.5.16.dat',dtype={'names':('field','ID','RA','Dec','z_peak','nh','flux_soft','flux_hard','flux_full','P_inclus'),'formats':('|S24','i8','f8','f8','f8','f8','f8','f8','f8','f8')})
...
for i in range(0,len(cr['ID'])):
    FILE.write('%12s %i %9.5f %9.5f %f %f %E %E %E %f\n'%(cr['field'][i],cr['ID'][i],cr['RA'][i], cr['Dec'][i],cr['z_peak'][i], cr['nh'][i],lums[i][0],lums[i][1],lums[i][2], cr['P_inclus'][i]))
FILE.close()
