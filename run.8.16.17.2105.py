...
outdf=pd.DataFrame({'cluster':clusters,'temp':temps,'terrlo':terrlo,'terrhi':terrhi,'lum':lums[gdo]/(10.**44),'lumerr':lumerrs[gdo]/(10.**44),'sig':sigs,'sigerr':sigerrs,'N':nsig})
...
outdf=pd.merge(outdf,SFdf,on=['field','cluster'])

FILE=open('/home/rumbaugh/cluster_prop_tab.txt','w')
for i in range(0,len(outdf)):
    FILE.write("%s & %i & $%.1f^{+%.1f}_{-%.1f}$ & $%.1f\pm%.1f$ & $%i\pm%i$ & %i & $%.3f\pm%.3f$\\\\\n"%(name_dict[outdf.iloc[i].cluster],outdf.iloc[i].N,outdf.iloc[i].temp,outdf.iloc[i].terrhi,outdf.iloc[i].terrlo,outdf.iloc[i].lum,outdf.iloc[i].lumerr,np.round(outdf.iloc[i].sig,-1),np.round(outdf.iloc[i].sigerr,-1),outdf.iloc[i].MMCGvel,outdf.iloc[i].qfrac,outdf.iloc[i].qfracerr))
FILE.close()
