    ...
    for groupnum in np.sort(EVLA_obs_dict[EorL].keys()):
        ...
        for SBnum in np.sort(EVLA_obs_dict[EorL][groupnum].keys()):
            ...
            for field,fieldname in zip(lensfields_arr,lensnames):
                source = fieldname
                fitsfile = '/home/rumbaugh/B1938+666_files/%s.%sSB%i_%s.fixpos.%s.fits'%(source,EorL,groupnum,str(SBnum),date)
                ...
                hdutmp.close()
pf.PrimaryHDU(newdat,hdr).writeto('/home/rumbaugh/B1938+666_files/B1938.concat.fits')
