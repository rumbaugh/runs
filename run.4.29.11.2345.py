...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    for j in range(0,len(gmsz)):
        ...
        if ((i == 2) & ((j == 2) | (j == 6) | (j == 5) | (j == 4))):
            ...
            #Cl1324
            if j == 0: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]],msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold')
            if j == 2: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]],msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold')
            if j == 3: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.004,msDec[gmsq[gmsz[gzsort[j]]]]-0.036,AGNstr,color='red',weight='extra bold')
            if j == 1: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.011,msDec[gmsq[gmsz[gzsort[j]]]]-0.013,AGNstr,color='red',weight='extra bold')
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.024,msDec[gmsq[gmsz[gzsort[j]]]]-0.008,AGNstr,color='red',weight='extra bold')
        elif i == 3:
            #Cl1604
            if j == 1:
    ...
    savfile = '/home/rumbaugh/paperstuff/AGN.spatmap.color.%s.r_%3.1fMpc.4.29.11.png'%(names[i],cradmult)
