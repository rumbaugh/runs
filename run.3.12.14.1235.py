...
for kernel in ['triangle']:
    ...
    for use_cov_matrix in [True]:
        ...
        for use_Neff in [True]:
            ...
            FILE = open('/home/rumbaugh/sim_ltcurve_testing/results/diftab.TDC_MCMC_%s_%s%s_%s.dat'%(kernel,cmstr,Neffstr,date),'w')
            ...
            for metric in dict_tmp:
                ...
                plt.savefig('/home/rumbaugh/sim_ltcurve_testing/results/plots/hist.%s.TDC_MCMC_%s_%s%s_%s.png'%(metric,kernel,cmstr,Neffstr,date))
                if metric == 'tau_abs_dif':
                    ...
                    plt.tick_params(which='major',length=8,width=2,labelsize=14)
                    plt.tick_params(which='minor',length=4,width=1.5,labelsize=14)
                    plt.savefig('/home/rumbaugh/sim_ltcurve_testing/results/plots/hist.%s.sm_err_only.TDC_MCMC_%s_%s%s_%s.png'%(metric,kernel,cmstr,Neffstr,date))
            plt.figure(1)
            plt.rc('axes',linewidth=2)
            plt.fontsize = 14
            plt.tick_params(which='major',length=8,width=2,labelsize=14)
