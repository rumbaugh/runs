...
for source in sources:
    ...
    if source == '0414+573':
        ...
        #tdates = np.delete(tdates,gdel)
    FILE = open('/home/rumbaugh/EVLA/light_curves/VLA_lc_aipsredux_wjet_5.28.14_%s.jvarpos.dat'%source,'w')
    for date in tdates:
        if date[1] == '.':
        ...
        if date not in tdates[gdel]:
            FILE.write('%f'%daytot)
            cr = np.loadtxt('/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_wjet_5.28.14.%s.jvarpos.%s.mod'%(source,date),dtype='string',comments='!',usecols=(0,1,2))
            num_img = 4
...
for lens in ['0414']:
    ...
    cr = np.loadtxt('/home/rumbaugh/EVLA/light_curves/VLA_lc_aipsredux_wjet_5.28.14_%s.jvarpos.dat'%(lens_dict[lens]['fullname']))
    ...
    if lens != '0712': plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s.lc_norm_plot_offset_wjet.%s.jvarpos.png'%(lens,date))
