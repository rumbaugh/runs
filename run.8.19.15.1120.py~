...
pairs = np.arange(70,90)
pairs = np.delete(pairs,[6])
...
def reviewresults(pair,ldate='8.18.15',ddate='5.13.15',cbase='/home/rumbaugh/Fermi/TimeBombs/output/',showresults=False,ntrials=10000,testplot=False,errstd=np.sqrt(2)*7.5,T='ACFT',showfullsample=False):
    tau,mu = cr[:,0][pair-1],cr[:,1][pair-1]
    sampleFile='%ssample.testlightcurve_test_%i.%s.dat'%(cbase,pair,ldate)
    sample_infoFile='%ssample_info.testlightcurve_test_%i.%s.dat'%(cbase,pair,ldate)
    levelsFile='%slevels.testlightcurve_test_%i.%s.dat'%(cbase,pair,ldate)
    DataFile='/home/rumbaugh/Fermi/data/test/testlightcurve_%i.tau_%.2f.mu_%.3f.%s.dat'%(pair,tau,mu,ddate)
    weightFile='%sweights.testlightcurve_test_%i.tau_%.2f.mu_%.3f.%s.dat'%(cbase,pair,tau,mu,ldate)
    posteriorFile='%sposterior.testlightcurve_test_%i.tau_%.2f.mu_%.3f.%s.dat'%(cbase,pair,tau,mu,ldate)
    plotfilebase='testlightcurve_test_%i.tau_%.2f.mu_%.3f.%s.png'%(pair,tau,mu,ldate)
    ...
    #PPMC(T,posteriorFile,DataFile,ntrials,plotfile=None,plotfile2='/home/rumbaugh/Fermi/TimeBombs/plots/PPMC_Td_vs_tau_%s_testlightcurve_test_%i.tau_%.2f.mu_%.3f.%s.png'%(T,pair,tau,mu,date),figN=7,testplot=testplot,errstd=errstd,truetau=tau)
...
for i,pair in zip(pairs-1,pairs):
    crpost=reviewresults(pair,showresults=True)
    crFP = np.loadtxt('/home/rumbaugh/Fermi/data/test/testlightcurve_%i.flare_params_truthvalues.tau_%.2f.mu_%.3f%s.dat'%(i+1,taus[i],mus[i],ddate))
    #crw=np.loadtxt('%sweights.testlightcurve_test_%i.tau_%.2f.mu_%.3f.%s.dat'%(cbase,pair,taus[i],mus[i],ldate))
    #crs=np.loadtxt('%ssample.testlightcurve_test_%i.tau_%.2f.mu_%.3f.%s.dat'%(cbase,pair,taus[i],mus[i],ldate))
    #gw=np.argsort(crw)
    ...
    draw()
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_hist.testlightcurve_test_%i.tau_%.2f.mu_%.3f.AIC.%s.png'%(i+1,taus[i],mus[i],date))
    ...
    ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(mjrFormatter))
    draw()
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_hist.testlightcurve_test_%i.tau_%.2f.mu_%.3f.BIC.%s.png'%(i+1,taus[i],mus[i],date))
