import numpy as np
import os
os.chdir('/home/rumbaugh/VLA/AF377/data')
...
FILE=open('/home/rumbaugh/runs/run.2.4.15.1720.sh','w')
FILE.write("#!/bin/csh\n")
...
for date,aipsdate in zip(dates,aips_dates):
    ...
    for source in tsources:
        ...
        uvfile = '/home/rumbaugh/AIPS/FITS/AF377_%s_20%s'%(source,aipsdate)
        FILE.write('obs %s\n'%uvfile)
        FILE.write(setup_string)
        #if source == '0414+573':
        #    FILE.write('rmod /home/rumbaugh/VLA/AF377/models/prelim_fit.0414+573.5.28.01.mod')
        #else:
        FILE.write('rmod /home/rumbaugh/VLA/AF377/models/20010212_%s_g.mod\n'%(source[:4]))
        FILE.write(fit_string)
        outfile = '/home/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_LR_wrms_%s.%s.fixpos.%s.mod'%(source,pdate,date)
        FILE.write('wmod %s\n'%outfile)
        FILE.write(end_string)
    for source in np.append(tCSOs,'1035+564'):
        FILE.write(opening_string)
        uvfile = '/home/rumbaugh/AIPS/FITS/AF377_%s_20%s'%(source,aipsdate)
        FILE.write('obs %s\n'%uvfile)
        FILE.write(setup_string)
        ...
        FILE.write(fit_string)
        outfile = '/home/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_LR_wrms.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
        FILE.write(end_string)
FILE.close()
