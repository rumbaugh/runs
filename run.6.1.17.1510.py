...
ilo,numilo,ilomjdlo,ilomjdhi,ihi,numihi,ihimjdlo,ihimjdhi,rlo,numrlo,rlomjdlo,rlomjdhi,rhi,numrhi,rhimjdlo,rhimjdhi=np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd)),np.zeros(len(crd))
...
for DBID,idb in zip(crd['DBID'],np.arange(len(crd))):
    ...
    cr=cr[outlier_arr==0]
    for b,lo_arr,lonumarr,lomjdloarr,lomjdhiarr,hi_arr,hinumarr,himjdhiarr,himjdhiarr in zip(['r','i'],[rlo,ilo],[numrlo,numilo],[rlomjdlo,ilomjdlo],[rlomjdhi,ilomjdhi],[rhi,ihi],[rhinum,ihinum],[rhimjdlo,ihimjdlo],[rhimjdhi,ihimjdhi]):
...
np.savetxt('/home/rumbaugh/var_database/Y3A1/DR7_full_magdiffs_wDBID.6.1.17.tab',outcr,header='RA DEC Redshift MJD_lo g_lo sig_lo flag_lo MJD_hi g_hi sig_hi flag_hi RA_DES DEC_DES DBID y3a1_mag_auto_g i_lo numilo i_lomjdlo i_lomjdhi i_hi numihi i_himjdlo i_himjdhi r_lo numrlo r_lomjdlo r_lomjdhi r_hi numrhi r_himjdlo r_himjdhi',fmt='%f %f %f %f %f %f %i %f %f %f %i %f %f %24s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f')
