...
sigma = np.array([1070,893,892,917,677,717,812,776,945])
sigerr = np.array([80,139,124,118,141,133,77,141,191])
...
pylab.savefig('/home/rumbaugh/LxT.plot.bol.4.2.12.png')
pylab.close('all')
...
for i in range(0,len(Temps)): 
    if i == 0: 
        ...
        pylab.text(sigma[i]-95,lumbol[i]-0.17,names[i],color='black',weight='extra bold')
    elif i == 3:
        pylab.text(sigma[i]+20,lumbol[i]-0.14,names[i],color='black',weight='extra bold')
    elif i == 4:
        pylab.text(sigma[i]-60,lumbol[i]+0.15,names[i],color='black',weight='extra bold')
    elif i == 6:
        pylab.text(sigma[i]-95,lumbol[i]+0.022,names[i],color='black',weight='extra bold')
    elif i == 7:
        pylab.text(sigma[i]-155,lumbol[i]+0.05,names[i],color='black',weight='extra bold')
    elif i == 8:
...
pylab.yticks([1,5],['1','5'])
pylab.savefig('/home/rumbaugh/sigLx.plot.4.2.12.png')
pylab.close('all')
print '\nsig-Lx\n'
for i in range(0,len(names)):
    distmin = 99999
...
for i in range(0,len(Temps)): 
    if i == 0: 
        ...
        pylab.text(Temps[i]-0.82,sigma[i]-23,names[i],color='black',weight='extra bold')
    elif i == 1:
...
pylab.xticks([1,5],['1','5'])
pylab.savefig('/home/rumbaugh/sigT.plot.4.2.12.png')
pylab.close('all')
