...
for lens in ['1030','0712','1938']:
    ...
    xlim = pylab.xlim()
    pylab.plot(np.array(xlim),avg_err*np.ones(2),ls='dotted',c='green')
    pylab.plot(np.array(xlim),rms_err*np.ones(2),ls='dashed',c='red')
    pylab.plot(np.array(xlim),-avg_err*np.ones(2),ls='dotted',c='green')
    pylab.plot(np.array(xlim),-rms_err*np.ones(2),ls='dashed',c='red')
    pylab.xlim(xlim)
    pylab.savefig('/home/rumbaugh/sub_lc_test.%s.9.8.14.png'%(lens))
