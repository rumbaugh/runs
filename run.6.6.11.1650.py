...
bgcnt = -1
numBGs = np.array([2,4,3,2,2])
BGRA1arr = np.array([201.33,201.375,275.7,275.7,275.45,274.85,5.915,6.04,6.12,241.2,241.46,268.86,269.3])
BGRA2arr = np.array([201.43,201.43,275.93,275.85,275.65,274.95,6.02,6.09,6.19,241.375,241.57,268.92,269.45])
BGDec1arr = np.array([30.32,30.735,68.38,68.5,68.66,68.58,4.55,4.205,4.425,42.87,43.0,66.44,66.68])
BGDec2arr = np.array([30.465,30.88,68.43,68.55,68.69,68.6,4.6,4.26,4.52,42.94,43.13,66.65,66.74])
...
#        sI = get_colvals(crs,'col18')
    ...
    FILEBG = open('/home/rumbaugh/paperstuff/compcorrcats/testBG.%s.cols.6.6.11.dat'%(names[i]),'w')
    ...
    clustotRS = 0.0
    clustotNRS = 0.0
    FILECL = open('/home/rumbaugh/paperstuff/compcorrcats/testCL.%s.cols.6.6.11.dat'%(names[i]),'w')
    if i == 0:
        #Cl1324
