...
#while ((i < 1) & (endloop < 0.5)):
    ...
    print '\n%s'%vis
    lenses = 1
    lensarr = np.arange(1) + 6
    if EarlyorLate_arr[i] == 'Early': 
        lenses = 2
        lensarr = np.array([4,6])
        if SBgrouparr[i] == 1: lensarr = np.array([3,6])
    while ((ifield < lenses) & (endloop < 0.5)):
        ispw = 0
        curfield = lensarr[ifield]
        #curant = 'ea%02i'%(iant+1)
        #print 'antenna = %s (badants: %s)'%(curant,badants_arr[i])
        while ((ispw < 2) & (endloop < 0.5)):
            print 'spw = %i'%ispw
            plotms(vis=vis,xaxis='u',yaxis='v',field='%i'%curfield,spw='%i'%ispw)
            gotcon = 0
