...
py.fontsize = 14
#py.savefig('/home/rumbaugh/EVLA/light_curves/Dispersions/Disp.con_plot.B0712.%s.%s.png'%(disp_type,date))
...
#V = np.arange(10)/10.*(np.max(Z)-np.min(Z))+np.min(Z)
    V = np.exp(np.arange(10)/10.*(np.max(np.log(Z))-np.min(np.log(Z)))+np.min(np.log(Z)))
    if delta == 10.5: V = 10**-8*np.array([2.,2.5,3.,4.,5.5])
    print V
    py.clf()
    py.rc('axes',linewidth=2)
    py.fontsize = 14
    ...
    py.xlabel('Time Delay (days)')
    py.fontsize = 14
    py.axvline(9,lw=2,c='k',ls='dashed')
    #py.ylim(4.75,5.375)
    py.ylim(3.840758,4.694260)
    py.savefig('/home/rumbaugh/EVLA/light_curves/Dispersions/Disp.con_plot.B0712.%s.delta_%.1f.%s.png'%(disp_type,delta,date))
    ...
    py.fontsize = 1
    py.axvline(9,lw=2,c='k',ls='dashed')
    py.savefig('/home/rumbaugh/EVLA/light_curves/Dispersions/Disp.tau_1D_plot.B0712.%s.delta_%.1f.%s.png'%(disp_type,delta,date))
