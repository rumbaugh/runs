...
crn = read_file("/home/rumbaugh/COSMOS/lens_sample_id_z_exp_rc.dat")
names = get_colvals(crn,'col1')
Rcore = get_colvals(crn,'col4')

FILE=open("/scratch/rumbaugh/ciaotesting/analysis/lens.analysis.Amp.1.17.11.dat","w")
for i in range(0,len(names)):
    load_data('/scratch/rumbaugh/ciaotesting/%i/acis%i.img.500-2000.S07.nops.norand.fits'%(names[i],names[i]))
    set_coord("physical")
    ostemp = os.system('dmstat /scratch/rumbaugh/ciaotesting/%i/conv.beta.12.4.10.fits verbose=0'%(names[i]))
    ...
    centemp = centemp[0]
    bgtemp = calc_data_sum2d('circle(%s,130)-circle(%s,120)'%(centemp,centemp))
    bkg = bgtemp/(m.pi*(130**2-120**2))
    ...
    FILE.write('%5i %5.2f %9.7f %9.7f\n'%(names[i],Rcore[i],bkg,Amp)) 
FILE.close()
