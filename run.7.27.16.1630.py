...
shiftdict={'rxj1716': -.0013,'cl1604': 0,'cl0023': +0.013, 'rxj0910': .015, 'rxj1757':0.0095,'rxj1821':0.0018}
...
for field in ['rxj0910','cl0023','rxj1757']:
    ...
    dz=shiftdict[field]/(1.+crzpz)
    P_inclus=np.zeros(np.shape(crpz)[0])
    for i in range(0,np.shape(crpz)[0]):
        ...
        g=np.where((pz[0]>fzl+dz[i])&(pz[0]<fzu+dz[i]))[0]
        P_inclus[i]=np.sum(zstep[g]*pz[1][g])
    np.savetxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s.P_inclus_shift.7.26.16.dat'%(field,field,field),P_inclus)
