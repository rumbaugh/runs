...
oldspecloaddict={'names':('ID','mask','slit','ra','dec','magR','magI','magZ','z','zerr','Q','oldid','notes'),'formats':('|S16','|S16','|S8','f8','f8','f8','f8','f8','f8','f16','i8','|S16','|S32')}
specloaddict={'names':('ID','mask','slit','ra','dec','magR','magI','magZ','z','zerr','Q','notes'),'formats':('|S16','|S16','|S8','f8','f8','f8','f8','f8','f8','f16','i8','|S32')}
cl1604specloaddict={'names':('ID','mask','slit','ra','dec','magR','magI','magZ','z','zerr','Q','oldid','pflags','acsRA','acsDec','acsID','f606','f814','notes'),'formats':('|S16','|S16','|S8','f8','f8','f8','f8','f8','f8','f16','i8','|S32','|S32','f8','f8','|S32','f8','f8','|S32')}
...
for field in targets[:-1]:
    print field
    specfile='/home/rumbaugh/Chandra/speccats/%s_spec.cat'%field
    try:
        crs=np.loadtxt(specfile,dtype=oldspecloaddict)
        g=np.where((crs['Q']>2.5)&(crs['z']>=spec_dict[field]['z'][1])&(crs['z']<=spec_dict[field]['z'][2]))[0]
        np.savetxt('/home/rumbaugh/Chandra/speccats/cats4coadd_2.11.16/%s_spec4coadd.cat'%field,crs[g],fmt='%16s %12s %12s %f %f  %f  %f  %f  %f  %f %i %s %s',header='ID mask slit ra dec magR magI magZ z zerr Q oldid notes')
    except:
        crs=np.loadtxt(specfile,dtype=specloaddict)
        g=np.where((crs['Q']>2.5)&(crs['z']>=spec_dict[field]['z'][1])&(crs['z']<=spec_dict[field]['z'][2]))[0]
        np.savetxt('/home/rumbaugh/Chandra/speccats/cats4coadd_2.11.16/%s_spec4coadd.cat'%field,crs[g],fmt='%16s %12s %12s %f %f  %f  %f  %f  %f  %f %i %s',header='ID mask slit ra dec magR magI magZ z zerr Q notes')
field='cl1604'
specfile='/home/rumbaugh/Chandra/speccats/%s_spec.cat'%field
crs=np.loadtxt(specfile,dtype=cl1604specloaddict)
g=np.where((crs['Q']>2.5)&(crs['z']>=spec_dict[field]['z'][1])&(crs['z']<=spec_dict[field]['z'][2]))[0]
np.savetxt('/home/rumbaugh/Chandra/speccats/cats4coadd_2.11.16/%s_spec4coadd.cat'%field,crs[g],fmt='%16s %12s %12s %f %f  %f  %f  %f  %f  %f %i %s %s %f %f %s %f %f %s',header='ID mask slit ra dec magR magI magZ z zerr Q oldid photflags acsRA acsDec acsID f606 f814 notes')
