...
for i in range(0,len(pcdf)):
    ...
    ax=plt.gca()
    ax.text(0.5,0.95,name_dict[pcdf.cluster.iloc[i]],color='k',horizontalalignment='center',verticalalignment='top',transform=ax.transAxes,fontsize=10)
    ...
    if panelcnt%3==0:
        plt.ylabel(r'Dec - Dec$_{Xray}$ (arcmin)',fontsize=8)
        ax.set_yticklabels(ylabs,fontsize=8)
    else:
        ax.set_yticklabels('')
    if panelcnt>=5-3*panel:
        ax.set_xticklabels(xlabs,fontsize=8)
        plt.xlabel(r'RA - RA$_{Xray}$ (arcmin)',fontsize=8)
    else:
        ax.set_xticklabels('')
    ...
    if panelcnt>8: 
        ...
        plt.savefig('/home/rumbaugh/DEplots1.pdf',dpi=600)
        panelcnt=0
        panel=1
        plt.clf()
plt.subplots_adjust(hspace=0)
...
plt.savefig('/home/rumbaugh/DEplots2.pdf',dpi=600)
