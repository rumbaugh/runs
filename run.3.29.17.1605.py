...
qry='SELECT g.DR7ID,g.SDSS_NAME,g.DBID,y.SPREAD_MODEL,y.SPREAD_MODEL_ERR FROM RUMBAUGH.GEMINI_TARGET g,DES_ADMIN.Y3A1_COADD_OBJECT_SUMMARY y,RUMBAUGH.DATABASEIDS d WHERE y.COADD_OBJECT_ID=d.COADD_OBJECT_ID AND d.DATABASEID=g.DBID'
DF=con.query_to_pandas(qry)
gs=np.argsort(DF['DR7ID'])
outcr=DF[gs]
np.savetxt('/home/rumbaugh/gemini.spread_model.dat',outcr,fmt='%i %s %s %f %f',header='DR7ID SDSS_NAME DBID SPREAD_MODEL SPREAD_MODEL_ERR')
