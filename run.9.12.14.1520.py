...
tau_disp = {'0712': 44., '1030': -127.5, '1938': 50.}
mu_disp = {'0712': 1./4.391267, '1030': 1/12.561223, '1938': 0.195245}
...
for lens in ['1030','0712','1938']:
    ...
    gl = np.where(ltime_mod<ltime_mod[-1]-tau)[0]
    if tau < 0: gl = np.where(ltime_mod>ltime_mod[0]-tau)[0]
    Aflux_mod = Aflux[g]*mu
    Aerr_mod = Aerr[g]*mu
    ltime_mod = ltime[g]+tau
    #Aflux_sm,Avar_sm = sm.interpolate(ltime_mod,Aflux_mod,ltime[g[gl]],y_var=Aerr_mod**2)
    Aflux_sm,Avar_sm = sm.interpolate(ltime[g],Aflux_mod,ltime_mod[gl],y_var=Aerr_mod**2)
    ...
    pylab.plot(ltime[g],Aflux[g]*mu,color='red')
    pylab.plot(ltime[g]+tau,Bflux[g],color='blue')
    pylab.scatter(ltime[g],Aflux[g]*mu,color='red')
    pylab.scatter(ltime[g]+tau,Bflux[g],color='blue')
    pylab.savefig('/home/rumbaugh/match.sub_lc_test.%s.%s.png'%(lens,date))
    pylab.figure(1)
    pylab.clf()
    pylab.rc('axes',linewidth=2)
    pylab.fontsize = 14
    ...
    pylab.fontsize = 14
    pylab.savefig('/home/rumbaugh/sub_lc.%s.%s.png'%(lens,date))
