...
FILE=open('/home/rumbaugh/Chandra/ORELSE.clus_sigs_wZ.dat','w')
...
for clus in crf['cluster']:
    ...
    zmin,zmax=np.min(allzs[allsig[2]]),np.max(allzs[allsig[2]])
    print clus,allsig[0]
    plt.figure(1)
    plt.clf()
    zhist(allzs[allsig[2]],zclus=zC,fname='/home/rumbaugh/Chandra/plots/clus_sig.%s.%s.zhist.wbounds.2.5.17.png'%(field,clus),zrange=(spec_dict[field]['z'][1],spec_dict[field]['z'][2]),zbins=int((-spec_dict[field]['z'][1]+spec_dict[field]['z'][2])/0.001))
    plt.figure(2)
    plt.clf()
    ...
    if len(gblue)>3:
        bluesig=CalcVelDisp(allzs2[gblue],returnclipped=True)
    else:
        bluesig=(0,0,np.zeros(0))
    if len(gred)>3:
        redsig=CalcVelDisp(allzs2[gred],returnclipped=True)
    else:
        redsig=(0,0,np.zeros(0))
    print allsig2[0]
    print bluesig[0]
    print redsig[0]
    FILE.write('%12s %14s %6.1f %6.1f %3i %6.1f %6.1f %3i %6.1f %6.1f %3i %6.1f %6.1f %3i %.6f %.6f\n'%(field,clus,allsig[0],allsig[1],len(allsig[2]),allsig2[0],allsig2[1],len(allsig2[2]),bluesig[0],bluesig[1],len(bluesig[2]),redsig[0],redsig[1],len(redsig[2]),zmin,zmax))
FILE.close()
