...
set_plot,'PS'
loadct,'13'
device,file="/home/rumbaugh/Lum+RSplots.8.9.11.ps",/color
totalslum = []
totalhlum = []
yslum = []
yhlum = []
...
for i=0,4 do begin &$
   ...
   if i eq 1 then gic = where((sz ge zlb[i]) and (sz le zhb[i]) and (rsfitt lt 10)) &$
   ...
   rsfit = [rsfit,rsfitt[gic]] &$
   print,ids[i]," ",rsfitt[gic] &$
   ictemp[gic] = 1 &$
   ic = [ic,ictemp] &$
   flum = slum+hlum &$
   slum += 0.0000000001
   ...
   slum = alog10(slum)+42
   hlum = alog10(hlum)+42
   ;if gic2[0] gt -0.1 then totalslum = [totalslum,slum[nors[gic2]]] &$
   ;if gic2[0] gt -0.1 then totalhlum = [totalhlum,hlum[nors[gic2]]] &$
   totalslum = [totalslum,slum[gic]] &$
   totalhlum = [totalhlum,hlum[gic]] &$
   rsslum = [rsslum,slum[gic]] &$
   rshlum = [rshlum,hlum[gic]] &$
   ;if gic2[0] gt -0.1 then nrsslum = [nrsslum,slum[nors[gic2]]] &$
   ;if gic2[0] gt -0.1 then nrshlum = [nrshlum,hlum[nors[gic2]]] &$
...
plot,[0-1],[0-1],/nodata,XTITLE=['RS Offset (RS Widths)'],YTITLE=['Log Luminosity'],XRANGE=[-9,4],YRANGE=[41,45],/xsty,/ysty,XTHICK=4,YTHICK=4,CHARSIZE=1.3,CHARTHICK=4
oplot,rsfit,totalflum,PSYM=4
