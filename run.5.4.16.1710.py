...
for s in [1,2]:
    ...
    FILE.write('# sample band netflux netflux_err netcnts netcnts_err srccnts bkgcnts srcarea bkgarea num_src\n')
    for sample in ['cut','nocut']:
        ...
        srcflux,bkgflux,netflux,nfluxerr={x:0. for x in bands},{x:0. for x in bands},{x:0. for x in bands},{x:0. for x in bands}
        srcarea,bkgarea={x:0. for x in bands},{x:0. for x in bands}
        for j,band in zip(np.arange(3),bands):
            numsrc=0
            for field in fields:
                ktmp=crk['k_%s'%band][gks[field]]
                ...
                for i in range(0,len(cr)):
                    tmpstr=cr[i].split(' #')
                    cntstmp=calc_data_sum2d(tmpstr[0])
                    srccnts[band]+=cntstmp
                    srcflux[band]+=cntstmp*ktmp
                ...
                for i in range(0,len(cr)):
                    ...
                    bkgtmp=calc_data_sum2d('ellipse(%f,%f,%f,%f,%f)'%(ra,dec,annA_outer,annB_outer,ang))-calc_data_sum2d('ellipse(%f,%f,%f,%f,%f)'%(ra,dec,annA_inner,annB_inner,ang))
                    bkgcnts[band]+=bkgtmp
                    bkgflux[band]+=bkgtmp*ktmp
            ...
            netflux[band]=srcflux[band]-bkgflux[band]*srcarea[band]/bkgarea[band]
            nfluxerr[band]=ktmp*ncntserr[band]
            print '\n%i (%s) - \nnetcnts: %.1f +/- %.1f\nsrccnts: %.1f\nbkgcnts: %.1f\n'%(s,band,netcnts[band],ncntserr[band],srccnts[band],bkgcnts[band])
            FILE.write('%5s %4s %E %E %.2f %.2f %.2f %.2f %.2f %.2f %i\n'%(sample,band,netflux[band],nfluxerr[band],netcnts[band],ncntserr[band],srccnts[band],bkgcnts[band],srcarea[band],bkgarea[band],numsrc))
    FILE.close()
