...
try:
	...
	magbins = [-100,5,16,18,20,22,24,26,50,999]

try:
	mag_prior
except NameError:
	mag_prior = np.zeros(len(magbins))+1.0

mag_prior[0],mag_prior[len(mag_prior)-1],mag_prior[len(mag_prior)-2] = 0.0,0.0,0.0

if len(magbins) != len(mag_prior): sys.exit("magbins and mag_prior must have same length")
...
try: 
	...
	Ra_aim = 5.9623443182206
try:
	Dec_aim
except NameError:
	Dec_aim = 4.3820249449499

try:
	optical_cat
except NameError:
	optical_cat = "/home/rumbaugh/LFC/cl0023_radecIDmags.cat"
radio_cat = ""
try:
	xray_cat
except NameError:
	xray_cat = "/scratch/rumbaugh/ciaotesting/Cl0023/7914/photometry/Cl0023.xray_phot.soft_hard_full.dat"

try:
	matching_dir
except NameError:
	matching_dir = "/scratch/rumbaugh/ciaotesting/Cl0023/7914/opt_match"
try:
	reg_dir
except NameError:
...
try:
	...
	radius_opt_cen = 725.0
area_opt_cen = m.pi*(radius_opt_cen**2)

try:
	output_file
except NameError:
	output_file = matching_dir + "/opt_Xray_matched_catalog_3high.twk.10.8.11.dat"
try:
	out_reg
except NameError:
	out_reg = reg_dir + "/matched_3high.twk.10.8.11.reg"
...
def FindCloseSources(ra,dec,tol,ra_opt,dec_opt,usemin):
	...
	ra_box_temp_ind1 = np.where((ra_opt >= ra-tol/3600.0) & (ra_opt <= ra+tol/3600.0))
...
def OptMatch(raX,decX,d_xerr,ra_opt,dec_opt,usemin):
	...
	if len(close_ind) > 0:
		...
		prior_close = prior_arr[close_ind]
		...
		for j in range(0L,len(close_ind)):
			...
			close_likelihoods[j] = prior_close[j]*m.exp(-0.5*(close_dist[j]**2)/(d_xerr**2))/(d_xerr**2.0*numdens_close[j])
...
#		loc_num_dens_gtMag[i] = num_gt_limit/area_opt_cen
	for i in range(0L,len(ra_opt)):
		...
		ind_local = ind_local[0]
		magI_loc = magI_opt[ind_local]
		ind_lemagI = np.where((magI_loc <= magbins[np.searchsorted(magbins,magI_opt[i])]) & (magI_loc >= magbins[np.searchsorted(magbins,magI_opt[i])-1]))
		ind_lemagI = ind_lemagI[0]
		num_gt_limit = len(ind_lemagI)
		loc_num_dens_gtMag[i] = num_gt_limit/area_opt_cen
		prior_arr[i] = mag_prior[np.searchsorted(magbins,magI_opt[i])-1]
#Optical Matching
...
matched_magI_opt1 = np.zeros(len(raX))
matched_magI_opt2 = np.zeros(len(raX))
matched_magI_opt3 = np.zeros(len(raX))
matched_magI_opt1.fill(-1)
matched_magI_opt2.fill(-1)
matched_magI_opt3.fill(-1)
...
for usemin in range(0,2):
	...
	#print probability_ij, probability_nonej
		...
		if num_matches == 1:
			matched_magI_opt1[i] = magI_opt[like_inds[0]]
		...
		#print opt_matched_temp_ind
			q = 0
			opt_matched_mag_temp = np.array([-1.0,-1.0,-1.0])
			...
			while ((q < 3) and (q < len(opt_matched_temp_ind))):
				opt_matched_mag_temp[q] = magI_opt[like_inds[opt_matched_temp_ind[q]]]
				...
				q += 1
			opt_matched_magI_temp = opt_matched_mag_temp
			matched_magI_opt1[i] = opt_matched_magI_temp[0]
			...
			matched_magI_opt2[i] = opt_matched_magI_temp[1]
			...
			matched_magI_opt3[i] = opt_matched_magI_temp[2]
	...
	if usemin == 0: 
		temp_prior = np.zeros(len(magbins))
		tot_pr = 0.0
		for imp in range(0,len(magbins)-1):
			gmp = np.where((matched_magI_opt1 > magbins[imp]) & (matched_magI_opt1 < magbins[imp+1]) & (matched_num_matches > 0.1))
			if len(gmp) > 0: gmp = gmp[0]
			for imp2 in range(0,len(gmp)):
				tot_pr += matched_prob_opt1[gmp[imp2]]
				temp_prior[imp] += matched_prob_opt1[gmp[imp2]]
			gmp = np.where((matched_magI_opt2 > magbins[imp]) & (matched_magI_opt2 < magbins[imp+1]) & (matched_num_matches > 1.1))
			if len(gmp) > 0: gmp = gmp[0]
			for imp2 in range(0,len(gmp)):
				tot_pr += matched_prob_opt2[gmp[imp2]]
				temp_prior[imp] += matched_prob_opt2[gmp[imp2]]
			gmp = np.where((matched_magI_opt3 > magbins[imp]) & (matched_magI_opt3 < magbins[imp+1]) & (matched_num_matches > 2.1))
			if len(gmp) > 0: gmp = gmp[0]
			for imp2 in range(0,len(gmp)):
				tot_pr += matched_prob_opt3[gmp[imp2]]
				temp_prior[imp] += matched_prob_opt3[gmp[imp2]]	
	if usemin <= 0.1:
	...
	for i in range(0L,len(matched_num_matches)):
		...
		for k in range(0L,3):
			if ((temp_ind_opts[k] != -1) and(temp_ind_opts[k] != "-1")):
				temp_new_inds[k] = int(newid_opt[int(temp_ind_opts[k])])
		FILE.write(str(matched_indX[i]) + " " + str(matched_raX[i]) + " " + str(matched_decX[i]) + " " + str(matched_errX[i]) + " " + str(matched_num_matches[i]) + " " + str(matched_ra_opt1[i]) + " " + str(matched_dec_opt1[i]) + " " + str(temp_new_inds[0]) + " " + str(matched_prob_opt1[i]) + " "  + str(matched_like_opt1[i]) + " " + str(matched_ra_opt2[i]) + " " + str(matched_dec_opt2[i]) + " " + str(temp_new_inds[1]) + " " + str(matched_prob_opt2[i]) + " "  + str(matched_like_opt2[i]) + " " + str(matched_ra_opt3[i]) + " " + str(matched_dec_opt3[i]) + " " + str(temp_new_inds[2]) + " "  + str(matched_prob_opt3[i]) + " " + str(matched_like_opt3[i]) + " " + str(matched_prob_none[i])	+ "\n")
