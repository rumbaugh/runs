...
crcr = read_file('/home/rumbaugh/clusters.z+pos+mpc.4.29.11.dat')
...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    if i == 0:
        ...
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=6,color='blue')
        g = np.where((sz[gsq[gsz]] > 0.725) & (sz[gsq[gsz]] <= 0.765))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=6,color=html_teal)
        g = np.where((sz[gsq[gsz]] > 0.765))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=6,color='green')
        g = np.where(sz[gsq[gsz]] < 0.68)
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=6,color=html_purp)
        g = np.where(sz[gsq[gsz]] < 0.665)
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=6,color=html_brwn)
        leg = pylab.legend(('0.68 < z < 0.725','0.725 < z < 0.765','z > 0.765','z < 0.68','z < 0.655'),loc=7,markerscale=0.8,labelspacing=0.3)
        icoff = 5
        F = pylab.gcf()
        DefaultSize = F.get_size_inches()
        F.set_size_inches( (DefaultSize[0]*0.8, DefaultSize[1]*1.5))
        for ic in range(0,5):
            plotcircle(RAcen=clusRA[ic+icoff],DECcen = clusDec[ic+icoff],rad=crads[ic+icoff])
            if ic == 0: pylab.text(clusRA[ic+icoff]-0.025,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 1: pylab.text(clusRA[ic+icoff]+0.04,clusDec[ic+icoff]-0.009,str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 2: pylab.text(clusRA[ic+icoff]+0.04,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 3: pylab.text(clusRA[ic+icoff]-0.025,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 4: pylab.text(clusRA[ic+icoff]-0.025,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
        #2 Extra
    ...
    if i == 2:
        ...
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=10,color=html_purp)
        g = np.where((sz[gsq[gsz]] >= 0.832) & (sz[gsq[gsz]] < 0.841))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=10,color='blue')
        g = np.where(sz[gsq[gsz]] >= 0.841)
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=10,color=html_teal)
        leg = pylab.legend(('z < 0.832','0.832 < z < 0.841','z > 0.841'),loc=3,markerscale=0.8,labelspacing=0.3)
        for t in leg.get_texts():
            t.set_fontsize('small') 
        icoff = 0
        for ic in range(0,4):
            plotcircle(RAcen=clusRA[ic+icoff],DECcen = clusDec[ic+icoff],rad=crads[ic+icoff])
            if ic == 0: pylab.text(clusRA[ic+icoff]+0.027,clusDec[ic+icoff]-0.01,str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 1: pylab.text(clusRA[ic+icoff]+0.0,clusDec[ic+icoff]+0.02,str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 2: pylab.text(clusRA[ic+icoff]+0.0,clusDec[ic+icoff]-0.03,str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 3: pylab.text(clusRA[ic+icoff]-0.02,clusDec[ic+icoff]-0.0,str(cnam[ic+icoff]),color='black',weight='extra bold')
    ...
    if i == 3:
        ...
        leg = pylab.legend(('z < 0.857','0.857 < z < 0.885','0.885 < z < 0.91','0.91 < z < 0.93','0.93 < z < 0.942','z > 0.942'),loc=4,markerscale=0.8,labelspacing=0.3)
        F = pylab.gcf()
        DefaultSize = F.get_size_inches()
        ...
        for ic in range(0,10):
            plotcircle(RAcen=clusRA[ic+icoff],DECcen = clusDec[ic+icoff],rad=crads[ic+icoff])
            if ic == 0: pylab.text(clusRA[ic+icoff]-0.03,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
            if ic == 1: pylab.text(clusRA[ic+icoff]-0.017,clusDec[ic+icoff]-0.025,str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 2: pylab.text(clusRA[ic+icoff]-0.03,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 3: pylab.text(clusRA[ic+icoff],clusDec[ic+icoff]+0.018,str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 4: pylab.text(clusRA[ic+icoff]-0.003,clusDec[ic+icoff]-0.006,str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 5: pylab.text(clusRA[ic+icoff]+0.017,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 6: pylab.text(clusRA[ic+icoff]+0.012,clusDec[ic+icoff]+0.005,str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 7: pylab.text(clusRA[ic+icoff],clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 8: pylab.text(clusRA[ic+icoff]-0.008,clusDec[ic+icoff]+0.004,str(cnam[ic+icoff]),color='black',weight='extra bold') 
            if ic == 9: pylab.text(clusRA[ic+icoff]-0.03,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold')
    if i == 4:
        #RXJ1757
    ...
    for j in range(0,len(gmsz)):
        ...
        if ((i == 2) & ((j == 2) | (j == 6) | (j == 5) | (j == 4))):
            #Cl0023
            if j == 2: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.000,msDec[gmsq[gmsz[gzsort[j]]]]+0.005,AGNstr,color='red',weight='extra bold')
            if j == 6: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]-0.011,AGNstr,color='red',weight='extra bold')
            if j == 5: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.004,msDec[gmsq[gmsz[gzsort[j]]]]+0.005,AGNstr,color='red',weight='extra bold')
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.003,msDec[gmsq[gmsz[gzsort[j]]]]-0.012,AGNstr,color='red',weight='extra bold')
        elif ((i == 0)):
            #Cl1324
            if j == 0: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold')
            if j == 1: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]],msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold')
            if j == 3: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]],msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold')
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.004,msDec[gmsq[gmsz[gzsort[j]]]]-0.036,AGNstr,color='red',weight='extra bold')
            if j == 2: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.011,msDec[gmsq[gmsz[gzsort[j]]]]-0.013,AGNstr,color='red',weight='extra bold')
            if j == 5: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.024,msDec[gmsq[gmsz[gzsort[j]]]]-0.008,AGNstr,color='red',weight='extra bold')
        elif i == 3:
            #Cl1604
            if j == 1:
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.013,msDec[gmsq[gmsz[gzsort[j]]]]-0.006,AGNstr,color='red',weight='extra bold')
            elif (j == 9):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.022,msDec[gmsq[gmsz[gzsort[j]]]]-0.007,AGNstr,color='red',weight='extra bold')  
            elif (j == 2):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.004,msDec[gmsq[gmsz[gzsort[j]]]]-0.009,AGNstr,color='red',weight='extra bold') 
            elif (j == 4):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.008,msDec[gmsq[gmsz[gzsort[j]]]]-0.001,AGNstr,color='red',weight='extra bold') 
            elif (j == 5):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.004,msDec[gmsq[gmsz[gzsort[j]]]]+0.006,AGNstr,color='red',weight='extra bold') 
            elif (j == 6):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.007,msDec[gmsq[gmsz[gzsort[j]]]]-0.00,AGNstr,color='red',weight='extra bold')
            elif (j == 7):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.005,msDec[gmsq[gmsz[gzsort[j]]]]-0.00,AGNstr,color='red',weight='extra bold')
            elif (j == 8):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.00,msDec[gmsq[gmsz[gzsort[j]]]]+0.006,AGNstr,color='red',weight='extra bold')
            else:
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold')
        elif (i == 1):
            #NEP5281
            if (j == 0): 
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.008,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold') 
            if (j == 2): 
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.011,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold')
            if (j == 1):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.006,msDec[gmsq[gmsz[gzsort[j]]]]+0.001,AGNstr,color='red',weight='extra bold')
        elif (i == 4):
            #RXJ1757
            pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.009,msDec[gmsq[gmsz[gzsort[j]]]]-0.013,AGNstr,color='red',weight='extra bold')
        else:
            pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold')  
    #FILE.close()
    ...
    savfile = '/home/rumbaugh/paperstuff/AGN.spatmap.color.%s.r_%3.1fMpc.5.3.11.png'%(names[i],cradmult)
