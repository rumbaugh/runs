...
a=ax1.hist(baseline/(1.+crd['z']),range=(0,3450),bins=23,color='k',edgecolor='k',facecolor='None',lw=2)
b=ax2.plot(np.sort(baseline/(1.+crd['z'])),(np.arange(len(crmd))+1.)/len(crmd[np.sort(baseline/(1.+crd['z'])<3450)]),lw=2,color='r')
...
for buff in np.array(['0','100','300','600','inf']):
    ...
    a=ax1.hist(baseline/(1.+crd['z']),range=(0,3450),bins=23,color='k',edgecolor='k',facecolor='None',lw=2)
    b=ax2.plot(np.sort(baseline/(1.+crd['z'])),(np.arange(len(crmd))+1.)/len(crmd[np.sort(baseline/(1.+crd['z'])<3450)]),lw=2,color='r')
    a2=ax1.hist(baseline/(1.+crd['z']),weights=1./corr_weights,range=(0,3450),bins=23,color='k',edgecolor='k',facecolor='None',ls='dashed',lw=2)
    ...
    ax2.set_xlim(0,3450)
    ax2.set_ylim(0,1)
    fig.savefig('/home/rumbaugh/var_database/Y3A1/plots/MaxChangeBaselinePlot.RF.DR7_EVQs.corr_wbuff_%s.4.19.17.png'%buff)
