...
a=ax.hist(baseline/(1.+crd['z']),range=(0,3400),bins=22,color='k',edgecolor='k',facecolor='None',lw=2)
...
for buff in [0,100,300,600]:
    ...
    a=ax.hist(baseline/(1.+crd['z']),range=(0,3400),bins=22,color='k',edgecolor='k',facecolor='None',lw=2)
    b=ax2.plot(np.sort(baseline/(1.+crd['z'])),(np.arange(len(crmd))+1.)/len(crmd),lw=2,color='r')
    a2=ax.hist(baseline/(1.+crd['z']),weights=1./corr_weights,range=(0,3400),bins=22,color='k',edgecolor='k',facecolor='None',ls='dashed',lw=2)
    b2=ax2.plot(a2[1][1:],np.cumsum(a2[0])*1./np.sum(a2[0]),lw=2,ls='dashed',color='r')
    totdetfrac=np.sum(a[0])*1./np.sum(a2[0])
    ax2.text(0.96,0.5,'Overall Detection\nFraction: %4.1f%%'%(totdetfrac))
    ax.set_xlabel('Maximum Change Baseline (Restframe days)')
    ax.set_ylabel(r'N$_{obj}$')
    ax2.set_ylabel('Cumulative Fraction')
    #ax.axhline(lw=4,color='k')
