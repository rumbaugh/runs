set_plot,'PS'
loadct,13
device,file='/home/rumbaugh/LumvsRS.nospectar.11.27.11.ps',/color
...
if ((i ne 11) and (i ne 2)) then readcol,pnames[i],pID,pra,pdec,pR,pI,pZ,format='A,D,D,D,D,D',/silent &$
if ((i eq 2)) then readcol,pnames[i],pID,oldpID,pra,pdec,pR,pI,pZ,format='A,I,D,D,D,D,D',/silent &$
...
;print,cnt &$
cnt++ &$
mfile = '/home/rumbaugh/paperstuff/' + mnames[i] + '.opt_Xray_matched_catalog_3high.corrected.twk.8.4.11.dat' &$
...
;;if i eq 1 then gnt = where((nm gt 0.1) and (inACS gt 0.3)) &$ ; not
;;this one
...
for j=0,n_elements(gnotar)-1 do begin &$
   ...
   if i ne 11 then gli = where(pID eq optID[gnotar[j]]) &$
   ;if i eq 1 then gli = long(optID[gnotar[j]])-2 &$
   if i ne 11 then rprime[j] = pR[gli[0]] &$
   if i ne 11 then iprime[j] = pI[gli[0]] &$
...
print,'Lum < 42.5, -1>C>-2: ',n_elements(gtt)
   ...
   if i ne 11 then gli = where(pID eq optID[gnt[gtar[j]]]) &$
   ;if i eq 1 then gli = long(optID[gnt[gtar[j]]])-2 &$
   if i ne 11 then rprimet[j] = pR[gli[0]] &$
   if i ne 11 then iprimet[j] = pI[gli[0]] &$
