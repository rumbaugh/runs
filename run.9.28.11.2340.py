...
for i in range(0,len(Temps)):
    ...
    for j in range(0,len(lineX)):
        disttemp = m.sqrt(((Temps[i]-lineX[j])/TerrD[i])**2+((lumbol[i]-lineY[j])/lumboles[i])**2)
        if disttemp < maxdist: maxdist = disttemp
    print "%s - %5.3f sig"%(names[i],maxdist)
...
pylab.savefig('/home/rumbaugh/LxT.plot.bol.9.22.11.png')
pylab.close('all')
...
for i in range(0,len(sigma)):
    maxdist = 999999.9
    for j in range(0,len(lineX)):
        disttemp = m.sqrt(((sigma[i]-lineX[j])/sigerr[i])**2+((lumbol[i]-lineY[j])/lumboles[i])**2)
        if disttemp < maxdist: maxdist = disttemp
    print "%s - %5.3f sig"%(names[i],maxdist)
...
pylab.savefig('/home/rumbaugh/sigLx.plot.9.22.11.png')
pylab.close('all')
...
for i in range(0,len(Temps)):
    maxdist = 999999.9
    for j in range(0,len(lineX)):
        disttemp = m.sqrt(((Temps[i]-lineX[j])/TerrD[i])**2+((sigma[i]-lineY[j])/sigerr[i])**2)
        if disttemp < maxdist: maxdist = disttemp
    print "%s - %5.3f sig"%(names[i],maxdist)
...
for i in range(0,len(Temps)): 
    if i == 0: 
        pylab.text(Temps[i]+0.,sigma[i]+40,names[i],color='black',weight='extra bold')
    elif i == 2:
        ...
        pylab.text(Temps[i]-0.7,sigma[i]+10,names[i],color='black',weight='extra bold')
    elif i == 8:
        pylab.text(Temps[i]+0.1,sigma[i]+13,names[i],color='black',weight='extra bold')
    else:
        pylab.text(Temps[i],sigma[i],names[i],color='black',weight='extra bold')
pylab.savefig('/home/rumbaugh/sigT.plot.9.22.11.png')
pylab.close('all')
