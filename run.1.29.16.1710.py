...
FILEall.write('# Mag Color RA Dec Redshift Field\n')
for field in targets[np.argsort(zlist)]:
    ...
    for n in range(0,len(i)): FILEall.write('%f %f %f %f %f %s\n'%(i[n],r[n]-i[n],crs['RA'][gCMD[n]],crs['Dec'][gCMD[i]],redshift[n],field))
