...
FILES3=open('/home/rumbaugh/Chandra/coadd_cats/spectra_AGN.Region3.lumcut.%s.dat'%(date),'w')
FILES4=open('/home/rumbaugh/Chandra/coadd_cats/spectra_AGN.Region4.lumcut.%s.dat'%(date),'w')
...
for field in targets:
    ...
    for i in range(0,len(gAGN)):
        d1604=SphDist(crs['ra'][gAGN[i]],crs['dec'][gAGN[i]],crRS['RA'],crRS['Dec'])
        g0=np.argsort(d1604)[0]
        rso[i]=crRS['RSoffset'][g0]
    for i in range(0,len(gAGN)):
        gs=gAGN[i]
        ...
        if np.log10(crxl['lum_full'][gAGN2[i]])>=43.3:
            if rso[i]>3: 
                ...
                rfile,region=None,2
        elif np.log10(crxl['lum_full'][gAGN2[i]])>=42.5:
            if rso[i]<1:
