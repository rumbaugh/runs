...
FILE=open('/home/rumbaugh/runs/run.5.20.14.1500.sh','w')
FILE.write("#!/bin/csh\n")

opening_string = "difmap << EOF\ninteger mfitniter\nmfitniter = 7\nlogical varpos\nvarpos = false\n"
setup_string = "select I\nmapunits arcsec\nmapsize 256,0.05\n"
fit_string = "uvrange 0,%i\nmodelfit mfitniter\nselfcal false,false,60\nmodelfit mfitniter\nselfcal false,false, 60\nmodelfit mfitniter\nselfcal false,false,60\nmodelfit mfitniter\nselfcal false,false, 60\nmodelfit mfitniter\n"%uvmax
end_string = "quit\nEOF\n"
...
for date,aipsdate in zip(dates,aips_dates):
    ...
    for source in tsources:
        ...
        FILE.write('rmod /mnt/data2/rumbaugh/EVLA/11A-138/scripts/model.0414.wgauss.5.20.14.mod\n')
        FILE.write(fit_string)
        outfile = '/mnt/data2/rumbaugh/VLA/AF377/difmap_results/fit_aipsredux_UVT_wgauss_5.20.14.%s.fixpos.%s.mod'%(source,date)
        FILE.write('wmod %s\n'%outfile)
