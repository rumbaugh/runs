...
set_plot,'PS'
loadct,'13'
device,file="/home/rumbaugh/LumvsRSplot.8.17.11.ps",/color
totalslum = []
totalhlum = []
kslum = []
khlum = []
...
for i=0,4 do begin &$
   ...
   if i eq 1 then readcol,"/home/rumbaugh/LFC/FINAL.matched.1604.specnXray.nov2010.rumbaugh.cat",LFCid,mask,slit,raopt,decopt,qqrB,qqiB,qqzB,mz,mzerr,mq,oldid,mskACA,raACA,decACA,idACA,m606,m814,idX,raX,decX,errX,Rel,Sig,format='A,A,I,D,D,D,D,D,D,D,I,A,A,D,D,A,D,D,I,D,D,D,D,D' &$
  ...
  ; if ((i eq 1) and (Xray_ID[iinds[ri]] eq 9)) then print,'bagel' &$
   if ((i eq 1) and (Xray_ID[iinds[ri]] eq 9)) then zbtemp[ri] = qqzB[1] &$
   ...
   if i eq 1 then print,10^(ibtemp[gs[2]]/(-2.5))/10^(s814[gpair2[0]]/(-2.5))
   if i lt 3.4 then print,'z-band'
   if i lt 3.4 then print,zbtemp[gs[gks]]
   if i lt 3.4 then print,pairsZ
   if i lt 3.4 then print,10^(zbtemp[gs[gks]]/(-2.5))/Zflux
   if i eq 1 then print,zbtemp[gs[2]]
   if i eq 1 then print,spz[gpair2[0]]
   if i eq 1 then print,10^(zbtemp[gs[2]]/(-2.5))/10^(spz[gpair2[0]]/(-2.5))
...
xyouts,[-5,-2.5,-2.5,1,1,-5],[43.9,43.75,43.05,42.3,43.75,42.3],['1','2','3','4','5','6'],CHARSIZE=2,CHARTHICK=7
yt = [44,30,30,43.3,43.3,30,30,50,50,44.32]
xt = [-1,-1,10,10,-200,-200,-3,-3,-1,-1]
oplot,xt,yt,linestyle=1,THICK=2
oplot,rsfitk,kflum,PSYM=7,SYMSIZE=2,THICK=4
oploterror,rsfity,yflum,colerrsy,DBLARR(n_elements(rsfity)),PSYM=4,color='60',THICK=4,SYMSIZE=2.5
oploterror,rsfito,oflum,colerrso,DBLARR(n_elements(rsfity)),PSYM=6,color='255',THICK=4,SYMSIZE=2.5
legend,['Cl1324,RXJ1821,RXJ1757','Cl1604,Cl0023'],color=['255','50'],PSYM=[6,4],SYMSIZE=[1.5,1.5],CHARTHICK=4,CHARSIZE=1.3,THICK=[4,4],box=0,/right
