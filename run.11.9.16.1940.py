...
for curid in IDs[:10]:
    ...
    SDSS_outcr=np.zeros((3,),dtype={'names':('DatabaseID','Survey','SurveyCoaddID','SurveyObjectID','RA','DEC','MJD','TAG','BAND','MAGTYPE','MAG','MAGERR','FLAG'),'formats':('i8','|S20','|S20','|S20','f8','f8','f8','|S20','|S12','|S12','f8','f8','i8')})
        for b,ib in zip(POSSbands,np.arange(len(POSSbands))):
            p_outcr['DatabaseID'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['Survey'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['SurveyCoaddID'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['SurveyObjectID'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['RA'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['DEC'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['MJD'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['TAG'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['BAND'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['MAGTYPE'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['MAG'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['MAGERR'][len(gsci)*ib:len(gsci)*(ib+1)],p_outcr['FLAG'][len(gsci)*ib:len(gsci)*(ib+1)]=DBID,'POSS','0','0',POSSra,POSSdec,POSSmjddict[b],'None',b,'PSF',POSSmagdict[b],POSSmagerrdict[b],0
            SDSS_outcr['DatabaseID'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['Survey'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['SurveyCoaddID'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['SurveyObjectID'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['RA'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['DEC'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['MJD'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['TAG'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['BAND'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['MAGTYPE'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['MAG'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['MAGERR'][len(gsci)*ib:len(gsci)*(ib+1)],SDSS_outcr['FLAG'][len(gsci)*ib:len(gsci)*(ib+1)]=DBID,'SDSS','0','0',SDSSra,SDSSdec,SDSSmjd,'None',b,'PSF',SDSSmagdict[b],SDSSmagerrdict[b],0
