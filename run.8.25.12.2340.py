...
if appendflag == 0:
    ...
    wora = 'a'
FILE = open('/local3/rumbaugh/EVLA/data/11A-138/difmap_results/B1938+666.A1_pos_compiled.8.25.12.dat','w')
...
#for i in range(0,len(datearr)):
    ...
    if ((i != 13) & (i != 6) & (i != 14) & (i != 16)): 
        gmod = np.loadtxt('/local3/rumbaugh/EVLA/data/11A-138/difmap_results/prelim_fit.B1938+666.%sSB%s_%s.8.25.12.mod'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i]),dtype='string',skiprows=4)
        gfluxest = gmod[:,0].copy()
        gradst = gmod[:,1].copy()
        gPAst = gmod[:,2].copy()
        gfluxes,grads,gPAs = np.zeros(len(gPAst)),np.zeros(len(gPAst)),np.zeros(len(gPAst))
        for j in range(0,len(gPAs)):
            ft,fr,fP = gfluxest[j],gradst[j],gPAst[j]
            gfluxes[j],grads[j],gPAs[j] = float(ft[0:len(ft)-1]),float(fr[0:len(fr)-1]),float(fP[0:len(fP)-1])
        A1ind = np.where((80 < gPAs) & (gPAs < 95))
        A1ind = A1ind[0]
        if len(A1ind) > 1:
            A1argsort = np.argsort(gfluxes[A1ind])
            if gfluxes[A1argsort[len(A1argsort)-1]] < gfluxes[A1argsort[0]]: sys.exit("argsort not working as thought, or I messed up")
            A1ind = A1ind[A1argsort[len(A1argsort)-1]]
        A1x = grads[A1ind]*m.sin((m.pi/180.0)*gPAs[A1ind])
        A1y = grads[A1ind]*m.cos((m.pi/180.0)*gPAs[A1ind])
        FILE.write('%s %s %s %s %s %f %f %f %f\n'%(EarlyorLate_arr[i],SBgrouparr[i],SBnumarr[i],montharr[i],datearr[i],grads[A1ind],gPAs[A1ind],A1x,A1y))
