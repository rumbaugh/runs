...
for i,pair in zip(pairs-1,pairs):
    ...
    n_comp=np.argsort(BICs)[0]+1
    classifier=GMM(n_components=n_comp)
    classifier.fit(taus)
    scores=classifier.score_samples(sort_post_taus)
    groupinds=np.argsort(scores[1])[:,-1]


    figure(27)
    clf()
    cind,mind=0,0
    for group,gind in zip(np.arange(1,n_comp+1),np.arange(0,n_comp)):
        scatter(sort_post_taus[groupinds==gind],fwidth_tmp[np.argsort(post_taus)[groupinds==gind]],s=4,color=color_arr[cind],marker=marker_arr[mind])
        cind+=1
    ...
    xlim(xmin,xmax)
    ylim(ymin,ymax)
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_vs_fwidth.testlightcurve_%i.tau_%.2f.mu_%.3f.%s.png'%(i+1,taus[i],mus[i],date))
