...
for i in range(0,7): 
    ...
    zoom_zs=all_zs[np.abs(all_zs-spec_dict[strname]['z'])<tol]
    x_zs=fullcat['redshift'][((fullcat['obsID']==strname)&(fullcat['Q']>2.5))]
    x_zoom_zs=x_zs[np.abs(x_zs-spec_dict[strname]['z'])<tol]
    plt.figure(1)
    plt.clf()
    ...
    plt.hist(zoom_zs,bins=80,range=(spec_dict[strname]['z']-tol,spec_dict[strname]['z']+tol),color='k')
    plt.hist(x_zoom_zs,bins=80,range=(spec_dict[strname]['z']-tol,spec_dict[strname]['z']+tol),color='r')
    plt.xlabel('Redshift')
    plt.ylabel('Number of sources')
    plt.title(namedict[strname])
    plt.savefig('/home/rumbaugh/Chandra/plots/z_hist_zoom.%s.%s.png'%(strname,date))
