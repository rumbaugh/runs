    ...
    if ((np.shape(cr)!=())&(np.shape(cr)!=(0,))):
        ...
        if len(gDES)>0:
            ...
            for ib,band in zip(np.arange(len(bands)),bands):
                ...
                if len(gb)>0:
                    ...
                    totmagerrdict[band]=np.append(totmagerrdict[band],cr['MAGERR'][gDES][gb])
        ...
        for b in stillgoodbands:
            if len(totmagdict[b])>1:
                ...
                totdiffstmp=np.abs(totmagdict[b][i1]-totmagdict[b][i2])
                ggooddiff=np.where((sigma>=3)&(totmagdict[b][i1]>1)&(totmagdict[b][i1]<30)&(totmagdict[b][i2]>1)&(totmagdict[b][i2]<30))[0]
                if len(ggooddiff)>0:
                    totdiffarr=np.append(totdiffarr,np.max(totdiffstmp[ggooddiff]))
