...
psfpdf=bpdf.PdfPages('/home/rumbaugh/LFC_color_param/template_colors.2.23.16.pdf')
...
for i in range(0,len(zlist)): 
    ax.scatter(np.ones(len(gy))*zlist[i],obsrs[i]-obsis[i])
...
ax=fig.add_subplot(1,1,1)
for i in range(0,len(zlist)): 
    ax.scatter(np.ones(len(gy))*zlist[i],obsis[i]-obszs[i])
ax.set_xlabel("Redshift")
ax.set_ylabel("i'-z'")
#ax.set_title("%s (i' err. < 0.05)"%field)
#ax.set_xlim(19,25)
