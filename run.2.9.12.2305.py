...
for i in range(7,9):
 ...
 #   if i == 5: ccnt += 1
    ...
    gs = gs[0]
    galls = np.arange(len(gs))
    gzs = np.where((sz[gs] > zlb[ccnt]) & (sz[gs] < zub[ccnt]))
    gzs = gzs[0]
    if i < 6.7:
...
#        pylab.ylabel("F606W-F814W")
    ...
    if (i < 4.5):
        if band == 'r-i':
            ...
            pylab.plot(np.arange(50),rsb[ccnt]-np.arange(50)*rsm[ccnt]+rsNSTD[ccnt]*rsSTD[ccnt],color='red')
        pylab.scatter(CMDx[g[gall]],CMDy[g[gall]],s=0.1,color='black')
    elif (i > 6.6):
        if band != 'r-i':
            pylab.plot(np.arange(50),rsb[ccnt]-np.arange(50)*rsm[ccnt]-rsNSTD[ccnt]*rsSTD[ccnt],color='red')
            pylab.plot(np.arange(50),rsb[ccnt]-np.arange(50)*rsm[ccnt]+rsNSTD[ccnt]*rsSTD[ccnt],color='red')
    ...
    print 'j'
    pylab.scatter(siB[gs[gzs]],srB[gs[gzs]]-siB[gs[gzs]],s=8,color='red')
    if i > 6.6:
        pylab.scatter(szB[gs[gzs]],siB[gs[gzs]]-szB[gs[gzs]],s=8,color='red')
...
pylab.savefig('/home/rumbaugh/CMD_full.%s.%s.2.9.12.png'%(names[i],band))
pylab.close('all')
