...
#for j in range(0,len(iarr)):
    ...
    j = 0
    while ((j < 3) & (endloop == 0)):
        plotms(vis=curvis,xaxis=xaxis_arr[j],yaxis='amp',selectdata=T,correlation='LL,RR',spw='',field=BPfield,averagedata=T,avgchannel=avgchannelarr[j],avgtime=avgtimearr[j],avgscan=avgscanarr[j],avgspw=T,coloraxis='antenna2',ydatacolumn='corrected')
        gotcon = 0
        con = raw_input("\nContinue? (y/n)\n")
        while gotcon == 0:
    ...
    #    tflagdata(vis=curvis,mode='manual',antenna=badants_arr[i])
