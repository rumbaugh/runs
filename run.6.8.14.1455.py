...
#for lens in ['1030']:
    ...
    hist2d(-1*tau,1./mu,V=np.array([0.668,0.954,0.997]))#, extent=[extents[j], extents[i]],
    #hist2d(tau,mu,V=np.array([4,11,406])/100000.)#, extent=[extents[j], extents[i]],
                   #plot_contours=plot_contours,
                   #plot_datapoints=plot_datapoints,
                   #weights=weights, **kwargs)
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/%s.chisq_conplot.interp.triangle_mod_plot.init_50.%s.png'%(lens,date))
