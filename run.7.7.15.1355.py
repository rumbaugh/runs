...
lcdata = FermiLightCurveData('/home/rumbaugh/Fermi/data/MonitoredSources/gll_asp_0457833600_v00.fit')

source_list=np.array(lcdata.sources)
for source in source_list:
    t, f, df, ul = lcdata.light_curve(source)
    clf()
    errorbar(t,f,yerr=df)
    xlabel('MJD')
    ylabel('flux (ph/cm^2/s)')
    ylim(0,1.1*max(f))
    savefig('/home/rumbaugh/Fermi/plots/MonitoredSources/fluxplot.%s.%s.png'%(source,date))
