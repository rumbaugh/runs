...
for i in range(0,len(fulldf)):
    ...
    fig=plt.gcf()
    ax0=fig.get_axes()[0]
    ax0.text(0.5,0.9,'%i - %s: ltau=%.1f(%.1f),lsig=%.1f'%(DBID,fulldf['class'].values[i],np.log10(fulldf.tau[i]),-np.log(fulldf.tau[i]),np.log10(fulldf.sigma)),fontsize=15,transform=fig.transFigure,horizontalalignment='center',color=cdict[fulldf['class'].values[i]])
