...
lxA,lxB,lxC,lxD=np.log10(crl0['lum_hard'][(((crl0['field']=='cl1350')|(crl0['field']=='rxj1221')|(crl0['field']=='rxj1757')|(crl0['field']=='rxj1821')|(crl0['field']=='rcs0224')))]),np.log10(crl0['lum_hard'][(((crl0['field']=='cl1324')|(crl0['field']=='cl1324_north')|(crl0['field']=='cl1324_south')|(crl0['field']=='rxj1716')))]),np.log10(crl0['lum_hard'][(((crl0['field']=='cl0023')|(crl0['field']=='cl1604')))]),np.log10(crl0['lum_hard'][(((crl0['field']=='rxj1053')|(crl0['field']=='cl0849')|(crl0['field']=='rxj0910')))])
...
plt.legend()
plt.savefig('/home/rumbaugh/Chandra/plots/lum_hist_hard.group_comps.6.15.16.png')
...
lA,lB,lC,lD=np.log10(crl['lum_hard'][((gfAGN_arr==True))]),np.log10(crl['lum_hard'][((gfAGN_arr==True)&(crl['redshift']<0.8))]),np.log10(crl['lum_hard'][((gfAGN_arr==True)&(crl['redshift']>=0.8)&(crl['redshift']<=0.96))]),np.log10(crl['lum_hard'][((gfAGN_arr==True)&(crl['redshift']>0.96))])
...
plt.ylabel('Number of Sources')
plt.legend()
plt.savefig('/home/rumbaugh/Chandra/plots/lum_hist_hard.field_comp.6.15.16.png')
...
#    ttmp=time.time()
#    print '%i%% Done. %.1fs elapsed. ETA: %.1fs'%(int(100*(il1+1.)/len(ls)),ttm#-st,(len(ls)-il1-1.)*(ttmp-st)/(il1+1))
...
#np.savetxt('/home/rumbaugh/Chandra/MC_KS_probs_hard.7.12.16.dat',out,fmt='%13s',header='                   Passive        Inter.   Cl0023&Cl1604     High-z        Field(all)     z<0.8      0.8<z<0.96      z>0.96')

member_dict={'Passive':119+160+102+75+131,'Intermediate':144+454,'Cl0023 & Cl1604':244+531,'High-z':74+72+142}
field_m=np.array([875,1074,1322])
totf=np.sum(field_m)
field_m=np.append(totf,field_m)
totAGN=0
for group,lt in zip(groupnames,[lxA,lxB,lxC,lxD]):
    numAGN=len(lt[lt>43])
    totAGN+=numAGN
    print '%12s - AGN frac = %.4f +/- %.4f'%(group,numAGN*1./member_dict[group],np.sqrt(numAGN)*1./member_dict[group])
print '%12s - AGN frac = %.4f +/- %.4f'%('Combined',totAGN*1./np.sum(member_dict.values()),np.sqrt(totAGN)*1./np.sum(member_dict.values()))
for i,lt,fldname in zip(np.arange(4),[lA,lB,lC,lD],['All','z<0.8','0.8<z<0.96','z>0.96']):
    numAGN=len(lt[lt>43])
    print '%12s - AGN frac = %.4f +/- %.4f'%(fldname,numAGN*1./field_m[i],np.sqrt(numAGN)*1./field_m[i])
