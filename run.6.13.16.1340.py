...
for target,it in zip(targets,np.arange(0,len(targets))):
    for band,ib in zip(['soft','hard','full'],np.arange(0,3)):
        if target=='cl1324':
            cr1=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s.X-ray_lum_limit.%s.dat'%('cl1324_north','cl1324_north','cl1324_north',band,date))
            cr2=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s.X-ray_lum_limit.%s.dat'%('cl1324_south','cl1324_south','cl1324_south',band,date))
            cr=0.5*(cr1+cr2)
        else:
            cr=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s.X-ray_lum_limit.%s.dat'%(target,target,target,band,date))
        plt.figure(1)
        plt.clf()
...
for target,it in zip(targets,np.arange(0,len(targets))):
    for band,ib in zip(['soft','hard','full'],np.arange(0,3)):
        if target=='cl1324':
            cr1=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s.X-ray_lum_limit_obs.%s.dat'%('cl1324_north','cl1324_north','cl1324_north',band,date))
            cr2=np.loadtxt('/home/rumbaugh/Chandra/ImperialPipeline/ORELSE/%s/proc/%s/%s_%s.X-ray_lum_limit_obs.%s.dat'%('cl1324_south','cl1324_south','cl1324_south',band,date))
            cr=0.5*(cr1+cr2)
        else:
