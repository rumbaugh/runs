...
crmd=np.loadtxt('/home/rumbaugh/milliquas_y1a1_match.csv',dtype={'names':('MQ_ROWNUM','RA','DEC','HPIX','COADD_OBJECTS_ID'),'formats':('i8','f8','f8','i8','i8')},delimiter=',',skiprows=1)
...
#PYQ='SELECT * FROM MCARRAS2.Y1A1_HPIX WHERE COADD_OBJECTS_ID in (%s)'%pstr
...
if calcmatchlist:
    for cid in mCIDs:
        ...
        if len(g)==1:
            ...
            PYQ='SELECT * FROM MCARRAS2.Y1A1_HPIX WHERE COADD_OBJECTS_ID=%i'%cid
            PDF=con.query_to_pandas(PYQ)
            ...
            break
    np.savetxt('/home/rumbaugh/matchlist_10.17.16.dat',matchlist,fmt='%i')
else:
...
for iHP,HP in zip(np.arange(len(HPlist)),HPlist):
    ...
    gk=np.in1d(YDF['COADD_OBJECTS_ID'],mCIDs,invert=True)
    if np.max(gk)>0:
        ...
        for i in range(0,len(ra_mtmp)):
            ...
            if len(gn)>0:
                ...
                if len(gd)>0:
                    ...
                    crtmp[:,4][i]=YDF['COADD_OBJECTS_ID'][gk][gn[np.argsort(np.array(tmpdist[gd]))[0]]]
    outcr=np.concatenate((outcr,crtmp),axis=0)
endt=time.time()
lptime=endt-st
print 'loop done. Took %.2f seconds'%(endt-st)
np.savetxt('/home/rumbaugh/milliquas_y1a1_match_pass2.csv',outcr,fmt='%i,%f,%f,%i,%i',header='MQ_ROWNUM,RA,DEC,HPIX,COADD_OBJECTS_ID',comments='')
print 'file saved'
trend=time.time()
