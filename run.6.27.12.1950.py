...
ccnt = 0
FILE=open('/home/rumbaugh/veldisps.3.14.12.dat','w')
FILE.write("# Structure  Numgals_MAD_xray Numgals_MAD_BCG Numgals_MAD_RGP  Numgals_FP_xray Numgals_FP_BCG Numgals_FP_RGP  Numgals_BW_xray Numgals_BW_BCG Numgals_BW_RGP  Numgals_gap_xray Numgals_gap_BCG Numgals_gap_RGP MAD_xray MAD_BCG MAG_RGP FP_xray FP_BCG FP_RGP BW_xray BW_BCG BW_RGP gap_xray gap_BCG gap_RGP\n")
for i in range(0,len(names)):
    ...
    gerrs,merrs,ferrs,berrs = np.zeros(3),np.zeros(3),np.zeros(3),np.zeros(3)
    ...
    while len(gxtemp) < prevx:
        ...
        velsxtemp = (sz[g[gx]]-biweight_loc(sz[g[gx[gxtemp]]]))*(3.0*10**5)/(1+sz[g[gx]])
        meanx = biweight_loc(velsxtemp[gxtemp])
        FPs[0] = fpseudosigma(velsxtemp[gxtemp])
    gxlens[2] = len(gxtemp)
    prevx,prevBCG,prevRGP = 9999,9999,9999
    gxtemp,gBCGtemp,gRGPtemp = np.arange(len(gx)),np.arange(len(gBCG)),np.arange(len(gRGP))
    meanx = biweight_loc(velsx)
    ...
    while len(gxtemp) < prevx:
        ...
        velsxtemp = (sz[g[gx]]-biweight_loc(sz[g[gx[gxtemp]]]))*(3.0*10**5)/(1+sz[g[gx]])
        meanx = biweight_loc(velsxtemp[gxtemp])
        biweights[0] = biweight_scale(velsxtemp[gxtemp])
    gxlens[3] = len(gxtemp)
    prevx,prevBCG,prevRGP = 9999,9999,9999
    gxtemp,gBCGtemp,gRGPtemp = np.arange(len(gx)),np.arange(len(gBCG)),np.arange(len(gRGP))
    meanx = biweight_loc(velsx)
    ...
    while len(gxtemp) < prevx:
        ...
        velsxtemp = (sz[g[gx]]-biweight_loc(sz[g[gx[gxtemp]]]))*(3.0*10**5)/(1+sz[g[gx]])
        meanx = biweight_loc(velsxtemp[gxtemp])
        gappers[0] = gapper(velsxtemp[gxtemp])
    gxlens[0] = len(gxtemp)
    ...
    while len(gBCGtemp) < prevBCG:
        ...
        MADs[1] = MAD(velsBCGtemp[gBCGtemp])
    ...
    while len(gBCGtemp) < prevBCG:
        ...
        velsBCGtemp = (sz[g[gBCG]]-biweight_loc(sz[g[gBCG[gBCGtemp]]]))*(3.0*10**5)/(1+sz[g[gBCG]])
        meanBCG = biweight_loc(velsBCGtemp[gBCGtemp])
        FPs[1] = fpseudosigma(velsBCGtemp[gBCGtemp])
    ...
    while len(gBCGtemp) < prevBCG:
        ...
        velsBCGtemp = (sz[g[gBCG]]-biweight_loc(sz[g[gBCG[gBCGtemp]]]))*(3.0*10**5)/(1+sz[g[gBCG]])
        meanBCG = biweight_loc(velsBCGtemp[gBCGtemp])
        biweights[1] = biweight_scale(velsBCGtemp[gBCGtemp])
    ...
    while len(gBCGtemp) < prevBCG:
        ...
        velsBCGtemp = (sz[g[gBCG]]-biweight_loc(sz[g[gBCG[gBCGtemp]]]))*(3.0*10**5)/(1+sz[g[gBCG]])
        meanBCG = biweight_loc(velsBCGtemp[gBCGtemp])
        gappers[1] = gapper(velsBCGtemp[gBCGtemp])
    ...
    while len(gRGPtemp) < prevRGP:
        ...
        MADs[2] = MAD(velsRGPtemp[gRGPtemp])
    ...
    while len(gRGPtemp) < prevRGP:
        ...
        velsRGPtemp = (sz[g[gRGP]]-biweight_loc(sz[g[gRGP[gRGPtemp]]]))*(3.0*10**5)/(1+sz[g[gRGP]])
        meanRGP = biweight_loc(velsRGPtemp[gRGPtemp])
        FPs[2] = fpseudosigma(velsRGPtemp[gRGPtemp])
    ...
    while len(gRGPtemp) < prevRGP:
        ...
        velsRGPtemp = (sz[g[gRGP]]-biweight_loc(sz[g[gRGP[gRGPtemp]]]))*(3.0*10**5)/(1+sz[g[gRGP]])
        meanRGP = biweight_loc(velsRGPtemp[gRGPtemp])
        biweights[2] = biweight_scale(velsRGPtemp[gRGPtemp])
    ...
    while len(gRGPtemp) < prevRGP:
        ...
        velsRGPtemp = (sz[g[gRGP]]-biweight_loc(sz[g[gRGP[gRGPtemp]]]))*(3.0*10**5)/(1+sz[g[gRGP]])
        meanRGP = biweight_loc(velsRGPtemp[gRGPtemp])
        gappers[2] = gapper(velsRGPtemp[gRGPtemp])
    gRGPlens[0] = len(gRGPtemp)



    FILE.write('%13s %3i %3i %3i %3i %3i %3i %3i %3i %3i %3i %3i %3i %f %f %f %f %f %f %f %f %f %f %f %f\n'%(names[i],gxlens[1],gBCGlens[1],gRGPlens[1],gxlens[2],gBCGlens[2],gRGPlens[2],gxlens[3],gBCGlens[3],gRGPlens[3],gxlens[0],gBCGlens[0],gRGPlens[0],MADs[0],MADs[1],MADs[2],FPs[0],FPs[1],FPs[2],biweights[0],biweights[1],biweights[2],gappers[0],gappers[1],gappers[2]))
