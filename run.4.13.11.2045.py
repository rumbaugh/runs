...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    if i == 0:
        pylab.xlim(201.516,200.8)
        xlocs = np.array([201.5,201.375,201.25,201.125,201.0,200.875])
        xlabs = np.array(['27$^{m}$00$^{s}$','30$^{s}$','26$^{m}$00$^{s}$','30$^{s}$','25$^{m}$00$^{s}$','13$^{h}$24$^{m}$30$^{s}$'])
        ylocs = np.array([30 + 10.0/60,30 + 15.0/60,30 + 20.0/60,30 + 25.0/60,30 + 30.0/60,30 + 35.0/60,30 + 40.0/60,30 + 45.0/60,30 + 50.0/60,30 + 55.0/60,31.0,30 + 5.0/60,31 + 10/60.0])
        ylabels = np.array(['30' + degree_symbol + "10'","15'","20'","25'","30'","35'","40'","45'","50'","55'",'31' + degree_symbol + "00'","05'","10'"])
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
        g = np.where(sz[gsq[gsz]] < 0.68)
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color=html_purp)
        g = np.where((sz[gsq[gsz]] > 0.68) & (sz[gsq[gsz]] <= 0.725))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color='blue')
        g = np.where((sz[gsq[gsz]] > 0.725) & (sz[gsq[gsz]] <= 0.765))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color=html_teal)
        g = np.where((sz[gsq[gsz]] > 0.765))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color='green')
        leg = pylab.legend(('z < 0.68','0.68 < z < 0.725','0.725 < z < 0.765','z > 0.765'),loc=1,markerscale=0.8,labelspacing=0.3)
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if i == 1:
        pylab.xlim(275.565,275.14)
        xlocs = np.array([275.5,275.375,275.25])
        xlabs = np.array(['22$^{m}$00$^{s}$','30$^{s}$','18$^{h}$21$^{m}$00$^{s}$'])
        ylocs = np.array([68 + 20.0/60,68 + 25.0/60,68 + 30.0/60,68 + 35.0/60])
        ylabels = np.array(['68' + degree_symbol + "20'","25'","30'","35'"])
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
    if i == 2:
        pylab.xlim(6.2,5.77)
        pylab.ylim(4 + 3.0/60,4 + 44.6/60)
        xlocs = np.array([5.875,6.0,6.125])
        xlabs = np.array(['0$^{h}$23$^{m}$30$^{s}$','24$^{m}$00$^{s}$','30$^{s}$'])
        ylocs = np.array([4 + 5.0/60,4 + 10.0/60,4 + 15.0/60,4 + 20.0/60,4 + 25.0/60,4 + 30.0/60,4 + 35.0/60,4 + 40.0/60])
        ylabels = np.array(['4' + degree_symbol + "05'","10'","15'","20'","25'","30'","35'","40'"])
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
        ...
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if i == 3:
        pylab.xlim(241.3,240.7)
        pylab.ylim(42+55.0/60,43+35.6/60)
        xlocs = np.array([240.75,240.875,241,241.125,241.25])
        xlabs = np.array(['16$^{h}$03$^{m}$00$^{s}$','30$^{s}$','04$^{m}$00$^{s}$','30$^{s}$','05$^{m}$00$^{s}$'])
        ylocs = np.array([43,43 + 5.0/60,43 + 10.0/60,43 + 15.0/60,43 + 20.0/60,43 + 25.0/60,43 + 30.0/60,43 + 35.0/60])
        ylabels = np.array(['43' + degree_symbol,"05'","10'","15'","20'","25'","30'","35'"])
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
        ...
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color=html_orng)
        g = np.where(sz[gsq[gsz]] >= 0.942)
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=12,color=html_brwn)
        ...
        for t in leg.get_texts():
            t.set_fontsize('small') 
    if i == 4:
        pylab.xlim(269.7,269.05)
        pylab.ylim(66 + 18.0/60, 66+43.2/60)
        xlocs = np.array([269.625,269.5,269.375,269.25,269.125])
        xlabs = np.array(['30$^{s}$','58$^{m}$00$^{s}$','30$^{s}$','57$^{m}$00$^{s}$','17$^{h}$56$^{m}$30$^{s}$'])
        ylocs = np.array([66 + 20.0/60,66 + 25.0/60,66.5,66 + 35.0/60,66 + 40.0/60])
        ylabels = np.array(['66' + degree_symbol + "20'","25'","30'","35'","40'"])
        pylab.yticks(ylocs,ylabels)
        pylab.xticks(xlocs,xlabs)
    if ((i == 1) | (i > 3.1)) : pylab.scatter(sRA[gsq[gsz]],sDec[gsq[gsz]],s=12)
    ...
    for j in range(0,len(gmsz)):
        ...
        if ((i == 2) & ((j == 2) | (j == 6) | (j == 5) | (j == 4))):
            #Cl0023
            if j == 2: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.000,msDec[gmsq[gmsz[gzsort[j]]]]+0.007,AGNstr,color='red',weight='extra bold')
            if j == 6: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.01,msDec[gmsq[gmsz[gzsort[j]]]]-0.019,AGNstr,color='red',weight='extra bold')
            if j == 5: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.005,AGNstr,color='red',weight='extra bold')
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.003,msDec[gmsq[gmsz[gzsort[j]]]]-0.026,AGNstr,color='red',weight='extra bold')
        elif ((i == 0) & ((j == 1) | (j == 4) | (j == 3))):
            #Cl1324
            if j == 3: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.004,msDec[gmsq[gmsz[gzsort[j]]]]-0.036,AGNstr,color='red',weight='extra bold')
            if j == 1: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.008,msDec[gmsq[gmsz[gzsort[j]]]]-0.013,AGNstr,color='red',weight='extra bold')
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.019,msDec[gmsq[gmsz[gzsort[j]]]]-0.006,AGNstr,color='red',weight='extra bold')
        elif i == 3:
            #Cl1604
            if j == 1:
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.01,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold')
            elif (j == 9):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.022,msDec[gmsq[gmsz[gzsort[j]]]]-0.015,AGNstr,color='red',weight='extra bold')  
            elif (j == 2):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.004,msDec[gmsq[gmsz[gzsort[j]]]]-0.018,AGNstr,color='red',weight='extra bold') 
            elif (j == 6):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.005,msDec[gmsq[gmsz[gzsort[j]]]]-0.007,AGNstr,color='red',weight='extra bold')
            elif (j == 7):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.005,msDec[gmsq[gmsz[gzsort[j]]]]-0.007,AGNstr,color='red',weight='extra bold')
            elif (j == 8):
            ...
            if (j == 2): 
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.008,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold')
            if (j == 1):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.001,AGNstr,color='red',weight='extra bold')
        elif (i == 4):
            #RXJ1757
    ...
    savfile = '/home/rumbaugh/paperstuff/AGN.spatmap.color.%s.4.7.11.png'%(names[i])
