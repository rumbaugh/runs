...
steps_arr=np.arange(0,29,IDsteplen,dtype='i8')
for curIDstep in steps_arr:
    ...
    for idcur,iid in zip(curIDS,np.arange(len(curIDs))):
        ...
        db_outcr['DatabaseID'],db_outcr['Survey'],db_outcr['SurveyCoaddID'],db_outcr['SurveyObjectID'],db_outcr['RA'],db_outcr['DEC'],db_outcr['MJD'],db_outcr['TAG'],db_outcr['BAND'],db_outcr['MAGTYPE'],db_outcr['MAG'],db_outcr['MAGERR'],db_outcr['FLAG']=DBID,'DES',idcur,np.array(DF['OBJID'])[gci],yra[gci],ydec[gci],mjd[gci],'NONE',band[gci],'PSF','MAG_PSF',mag[gci],magerr[gci],0
        ...
        np.savetxt('%s/%i/DES_data.tab'%(DB_path,DBID),DF[gci],fmt='%f %i %i %f %f %f %f %f %f %s %f',header=('MJD IMAGEID OBJECTID COADD_OBJECTS_ID RA DEC MAG_AUTO MAGERR_AUTO MAG_PSF MAGERR_PSF BAND EXPTIME'),comments='')
        np.savetxt('%s/%i/SDSS_data.tab'%(DB_path,DBID),fmt=('%f %f %f %i %i %f %f %f %f %f %f %f %f %f %f%i %i %i %i %i %i %i'),header=('MJD RA DEC OBJID NUMROW PSFMAG_U  PSFMAG_G  PSFMAG_R  PSFMAG_I  PSFMAG_Z  PSFMAGERR_U  PSFMAGERR_G  PSFMAGERR_R  PSFMAGERR_I  PSFMAGERR_Z RUN RERUN STRIPE THINGID MQ_ROWNUM COADD_OBJECTS_UD HPIX'),comments='')
