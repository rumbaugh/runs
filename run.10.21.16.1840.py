...
for curIDstep in steps_arr:
    ...
    for idcur,iid in zip(curIDs,np.arange(len(curIDs))):
        ...
        np.savetxt('%s/%i/DES_data.tab'%(DB_path,DBID),np.array(DF)[gci],fmt='%f %i %i %f %f %f %f %f %f %s %f',header=('MJD IMAGEID OBJECTID COADD_OBJECTS_ID RA DEC MAG_AUTO MAGERR_AUTO MAG_PSF MAGERR_PSF BAND EXPTIME'),comments='')
