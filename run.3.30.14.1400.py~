...
for imgname in imgnames:
    ...
    for i in range(0,ntrials):
        ...
        ttmp,mtmp,BA_disp_mat[i] = calc_disp_delay(fluxdict[imgname][g]+rC[g],Bflux[g2]+rB[g2],ltime,ltime,errdict[imgname][g],Berr[g2],maxtimestep,timestep,minmu,maxmu,mustep,'D_4_2a',delta,output=2,verbose=False)
        FILE.write('%E\n'%(BA_disp_mat[i]))
        if i == 0: 
            t0 = time.time()
...
for imgname in imgnames:
    ...
    for i in range(0,ntrials):
        ...
        np.random.shuffle(g2)
        ttmp,mtmp,BA_disp_mat[i] = calc_disp_delay(fluxdict[imgname][gAonly[g]]+rC[g],Bflux[gAonly[g2]]+rB[g2],ltime,ltime,errdict[imgname][gAonly[g]],Berr[gAonly[g2]],maxtimestep,timestep,minmu,maxmu,mustep,'D_4_2a',delta,output=2,verbose=False)
        FILE.write('%E\n'%(BA_disp_mat[i]))
        if i == 0: 
            t0 = time.time()
            print 'Initial ETA: %i seconds'%((t0-st)*(ntrials-1))
