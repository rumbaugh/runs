...
for i,pair in zip(pairs-1,pairs):
    ...
    xlim(xmin,xmax)
    ylim(ymin,ymax)
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_vs_fwidth.testlightcurve_%i.tau_%.2f.mu_%.3f.AIC.%s.png'%(i+1,taus[i],mus[i],date))

    figure(28)
    clf()
    cind,mind=0,0
    for group,gind in zip(np.arange(1,n_compB+1),np.arange(0,n_compB)):
        scatter(sort_post_taus[groupindsB==gind],fwidth_tmp[np.argsort(post_taus)[groupindsB==gind]],s=4,color=color_arr[cind],marker=marker_arr[mind])
        cind+=1
        if cind >= len(color_arr):
            mind+=1
            cind=0
    ...
    xlim(xmin,xmax)
    ylim(ymin,ymax)
    savefig('/home/rumbaugh/Fermi/TimeBombs/plots/tau_vs_fwidth.testlightcurve_%i.tau_%.2f.mu_%.3f.BIC.%s.png'%(i+1,taus[i],mus[i],date))
