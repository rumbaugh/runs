...
FILE2 = open('/home/rumbaugh/DE_counts.bkg_data.10.13.12.dat','w')
for i in range(0,len(sigma)):
    print names[i]
    FILE = open('/home/rumbaugh/DE_counts_profile.%s.10.13.12.dat'%names[i],'w')
    ann_arr = np.arange(50)*5+5
    #ann_arr = np.arange(24)*10+20
    ...
    SB_err_arr = np.zeros(len(ann_arr))
    if ((names[i] != '0910+5419') & (names[i] != 'Cl1324+3013')): load_data(str(namef[i]))
    set_coord('physical')
    set_model(beta2d.b)
    ...
    if ((names[i] != '0910+5419') & (names[i] != 'Cl1324+3013')):
        ...
        bgSB[i] = bgcnts[i]/(m.pi*(annouter2[i]*annouter2[i]-annouter1[i]*annouter1[i]))
        bgSBas[i] = bgSB[i]*4
    else:
        bgcnts[i] = (calc_data_sum2d('circle(%f,%f,%f)'%(xcens[i-1],ycens[i-1],annouter2[i-1]))-calc_data_sum2d('circle(%f,%f,%f)'%(xcens[i-1],ycens[i-1],annouter1[i-1])))
        bgSB[i] = bgcnts[i]/(m.pi*(annouter2[i-1]*annouter2[i-1]-annouter1[i-1]*annouter1[i-1]))
        bgSBas[i] = bgSB[i]*4
    ...
    SB_arr = (cnts_arr-np.append(np.zeros(1),cnts_arr[0:len(cnts_arr)-1]))/area_arr
    ...
    if ((names[i] != '0910+5419') & (names[i] != 'Cl1324+3013')):
        FILE2.write('%13s %f %f %f %i %i %i %i\n'%(names[i],bgcnts[i],bgSB[i],bgSBas[i],annouter1[i],annouter2[i],annouter1[i]/2,annouter2[i]/2))
    else:
        FILE2.write('%13s %f %f %f %i %i %i %i\n'%(names[i],bgcnts[i],bgSB[i],bgSBas[i],annouter1[i-1],annouter2[i-1],annouter1[i-1]/2,annouter2[i-1]/2))
