...
FILE=open('/home/rumbaugh/Chandra/ORELSE.DStests.dat','w')
...
for clus in crf['cluster']:
    ...
    if len(zs)>10:
        DSstat,P,delta=DStest(ras,decs,zs,use_vel_in=False)
        FILE.write('%12s %14s %f %f\n'%(field,clus,DSstat,P))
    else:
        continue
    for j in range(0,len(delta)):
        circsize = 100*np.exp(1.0*delta[j])
        if ((zs[j] >  avg_z-zsig) & (zs[j] < avg_z+zsig)): 
            tx = np.cos(np.linspace(0,2*np.pi,50)).tolist()
            ty = np.sin(np.linspace(0,2*np.pi,50)).tolist()
            xy1 = zip(tx,ty)
            plt.scatter(ras[j],decs[j],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='red',lw=3)
            #plt.scatter(ras[j],decs[j],s=secsize*0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='red')
        elif ((zs[j] < avg_z-zsig)):
            #plt.scatter(ras[j],decs[j],s=circsize,facecolors='green',edgecolor='black')
            tr = np.sqrt(circsize/np.pi)
            tx = [0] + np.cos(np.linspace(0,2*np.pi*0.25,50)).tolist()
            ty = [0] + np.sin(np.linspace(0,2*np.pi*0.25,50)).tolist()
            xy1 = zip(tx,ty)
            tx = [0] + np.cos(np.linspace(2*np.pi*0.25,2*np.pi*0.5,50)).tolist()
            ty = [0] + np.sin(np.linspace(2*np.pi*0.25,2*np.pi*0.5,50)).tolist()
            xy2 = zip(tx,ty)
            tx = [0] + np.cos(np.linspace(2*np.pi*0.5,2*np.pi*0.75,50)).tolist()
            ty = [0] + np.sin(np.linspace(2*np.pi*0.5,2*np.pi*0.75,50)).tolist()
            xy3 = zip(tx,ty)
            tx = [0] + np.cos(np.linspace(2*np.pi*0.75,2*np.pi,50)).tolist()
            ty = [0] + np.sin(np.linspace(2*np.pi*0.75,2*np.pi,50)).tolist()
            xy4 = zip(tx,ty)
            plt.scatter(ras[j],decs[j],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='green',lw=3)
            plt.scatter(ras[j],decs[j],s=0.25*circsize,marker=(xy2,0),facecolors='none',edgecolors='green',lw=3)
            plt.scatter(ras[j],decs[j],s=0.25*circsize,marker=(xy3,0),facecolors='none',edgecolors='green',lw=3)
            plt.scatter(ras[j],decs[j],s=0.25*circsize,marker=(xy4,0),facecolors='none',edgecolors='green',lw=3)
        else:
            tx = np.cos(np.linspace(0,2*np.pi,50)).tolist()
            ty = np.sin(np.linspace(0,2*np.pi,50)).tolist()
            xy1 = zip(tx,ty)
            plt.scatter(ras[j],decs[j],s=0.25*circsize,marker=(xy1,0),facecolors='none',edgecolors='blue',linestyle='dotted',lw=2)
    xlim=plt.xlim()
    plt.xlim(xlim[1],xlim[0])
    plt.title(clus)
    plt.savefig('/home/rumbaugh/Chandra/plots/DSplot.%s.png'%clus)
FILE.close()
