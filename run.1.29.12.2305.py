...
names2 = np.array(['Cl1324','RXJ1821','Cl0023','Cl1604','RXJ1757'])
...
crcr = read_file('/home/rumbaugh/paperstuff/clusters.z+pos+mpc.5.14.11.dat')
...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    if i == 0:
        ...
        for ic in range(0,10):
            ...
            if ic == 1: pylab.text(clusRA[ic+icoff]+0.052,clusDec[ic+icoff]-0.0104,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 2: pylab.text(clusRA[ic+icoff]+0.055,clusDec[ic+icoff]+0.002,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 3: pylab.text(clusRA[ic+icoff]-0.027,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 4: pylab.text(clusRA[ic+icoff]+0.054,clusDec[ic+icoff]-0.01,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 5: pylab.text(clusRA[ic+icoff]+0.05,clusDec[ic+icoff]-0.02,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            ...
            if ic == 8: pylab.text(clusRA[ic+icoff]-0.026,clusDec[ic+icoff]-0.032,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
    ...
    if i == 2:
        ...
        for ic in range(0,4):
            plotcircle(RAcen=clusRA[ic+icoff],DECcen = clusDec[ic+icoff],rad=crads[ic+icoff])
            if ic == 0: pylab.text(clusRA[ic+icoff]+0.034,clusDec[ic+icoff]-0.01,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 1: pylab.text(clusRA[ic+icoff]+0.0,clusDec[ic+icoff]+0.02,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 2: pylab.text(clusRA[ic+icoff]+0.0,clusDec[ic+icoff]-0.036,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 3: pylab.text(clusRA[ic+icoff]-0.02,clusDec[ic+icoff]-0.02,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
    if i == 3:
        #Cl1604
        ...
        for ic in range(0,9):
            plotcircle(RAcen=clusRA[ic+icoff],DECcen = clusDec[ic+icoff],rad=crads[ic+icoff])
            if ic == 0: pylab.text(clusRA[ic+icoff]-0.03,clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold',size=19)
            if ic == 1: pylab.text(clusRA[ic+icoff]-0.016,clusDec[ic+icoff]-0.03,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19) 
            ...
            if ic == 5: pylab.text(clusRA[ic+icoff]+0.018,clusDec[ic+icoff]-0.002,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19) 
            if ic == 6: pylab.text(clusRA[ic+icoff]+0.012,clusDec[ic+icoff]+0.004,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19) 
            if ic == 7: pylab.text(clusRA[ic+icoff],clusDec[ic+icoff],str(cnam[ic+icoff]),color='black',weight='extra bold',size=19) 
            if ic == 8: pylab.text(clusRA[ic+icoff]-0.008,clusDec[ic+icoff]+0.002,str(cnam[ic+icoff]),color='black',weight='extra bold',size=19) 
    ...
    #FILE.close()
    #pylab.scatter(msRA[gmsq[gmsz]],msDec[gmsq[gmsz]],marker = 'd',s=90,c='red')
    ...
    for j in range(0,len(gmsz)):
        ...
        AGNstr = str(1+j)
        if i == 46:
        #if ((i == 45) & (i == 2) & ((j == 2) | (j == 6) | (j == 5) | (j == 0) | (j == 4))):
            #Cl0023
            if j == 0: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold',size=16)
            if j == 2: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.006,msDec[gmsq[gmsz[gzsort[j]]]]+0.006,AGNstr,color='red',weight='extra bold',size=16)
            if j == 6: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]-0.017,AGNstr,color='red',weight='extra bold',size=16)
            if j == 5: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.009,msDec[gmsq[gmsz[gzsort[j]]]]+0.005,AGNstr,color='red',weight='extra bold',size=16)
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.006,msDec[gmsq[gmsz[gzsort[j]]]]-0.016,AGNstr,color='red',weight='extra bold',size=16)
        #elif ((i == 0)):
            #Cl1324
            #AGNstr = str(j)
            if j == 0: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold',size=16)
            if j == 1: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]],msDec[gmsq[gmsz[gzsort[j]]]]+0.01,AGNstr,color='red',weight='extra bold',size=16)
            if j == 3: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]],msDec[gmsq[gmsz[gzsort[j]]]]+0.008,AGNstr,color='red',weight='extra bold',size=16)
            if j == 4: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.027,msDec[gmsq[gmsz[gzsort[j]]]]-0.011,AGNstr,color='red',weight='extra bold',size=16)
            ...
            if j == 5: pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.032,msDec[gmsq[gmsz[gzsort[j]]]]-0.011,AGNstr,color='red',weight='extra bold',size=16)
        #elif i == 3:
            #Cl1604
            if j == 1:
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.017,msDec[gmsq[gmsz[gzsort[j]]]]-0.006,AGNstr,color='red',weight='extra bold',size=16)
            elif (j == 9):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.027,msDec[gmsq[gmsz[gzsort[j]]]]-0.007,AGNstr,color='red',weight='extra bold',size=16)  
            elif (j == 2):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.005,msDec[gmsq[gmsz[gzsort[j]]]]-0.011,AGNstr,color='red',weight='extra bold',size=16) 
            elif (j == 4):
                ...
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.006,msDec[gmsq[gmsz[gzsort[j]]]]-0.002,AGNstr,color='red',weight='extra bold',size=16)
            elif (j == 8):
            ...
            #NEP5281
            if (j == 0): 
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.008,msDec[gmsq[gmsz[gzsort[j]]]]+0.006,AGNstr,color='red',weight='extra bold',size=16) 
            if (j == 2): 
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]+0.02,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold',size=16)
            if (j == 1):
                pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.012,msDec[gmsq[gmsz[gzsort[j]]]]+0.001,AGNstr,color='red',weight='extra bold',size=16)
        #elif (i == 4):
            #RXJ1757
            ...
            #pylab.text(msRA[gmsq[gmsz[gzsort[j]]]]-0.003,msDec[gmsq[gmsz[gzsort[j]]]]+0.003,AGNstr,color='red',weight='extra bold',size=16)  
    #FILE.close()
    pylab.title('%s'%(names2[i]))
    pylab.xlabel('Right Ascension (J2000)')
    pylab.ylabel('Declination (J2000)')
    savfile = '/home/rumbaugh/paperstuff/spatmap.color.%s.1.29.12.png'%(names[i])
