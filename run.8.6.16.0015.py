...
clusters,fields=np.zeros(len(gdo),dtype='|S24'),np.zeros(len(gdo),dtype='|S24')
for i,i0 in zip(gdo,np.arange(len(gdo))):
    ...
    Ezs[i0]=Ez[gcc]
    clusters[i0],fields[i0]=crx['field'][g],crx['cluster'][g]
...
for i in range(0,len(gdo)):
    print '%12s %15s - Lx: %.3E, %.3E  Tx: %.3f  sig: %.0f'%(fields[i],clusters[i],lums[gdo][i],lumXW[i],temps[i],sigs[i])
