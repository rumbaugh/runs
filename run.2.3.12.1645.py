...
for i in range(0,len(names)):
    ...
    print '\n%s - \nUnweighted mean: (%s,%s) - using %i gal\nWeighted mean: (%s,%s) - using %i gal\nUnweighted mean, red: (%s,%s) - using %i gal\nWeighted mean, red: (%s,%s) - using %i gal\n'%(names[i],RAstrs[0],Decstrs[0],len(gz),RAstrs[1],Decstrs[1],len(gz),RAstrs[3],Decstrs[3],len(gr),RAstrs[2],Decstrs[2],len(gr))
