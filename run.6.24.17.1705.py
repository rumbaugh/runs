...
FILE=open('/home/rumbaugh/Chandra/ORELSE.cluster_netcounts.6.24.17.dat','w')
...
for i in np.arange(len(targets)):
    ...
    g=np.where((field==testfield)&(cr['cluster']=='Lynx_W'))[0]
    for j in range(0,len(g)):
        ...
        if crx['kt'][gx]>0:
            ...
            for band in ['soft','hard','full']:
                ...
                datax2=hdux2[0].data
                crspec=np.loadtxt('%s/%s_%s_spec.reg'%(curdir,cluster,band),dtype='|S128')
                if np.shape(crspec)==():
                ...
                xtmp,radt=float(xtmp[1]),int(radtmp[0])
                xt,yt=int(xtmp-x0),int(ytmp-y0)
                ...
                bradt=int(bradt)
                bxt,byt=int(bxtmp-x0),int(bytmp-y0)
