...
FILEstat = open("/home/rumbaugh/status.run.5.8.12.1705.txt",'w')
FILEstat.close()
FILE1 = open("/home/rumbaugh/baselines.test.5.8.12.dat",'w')
for i in range(0,6):
    baseline = np.zeros(2)
    for j in range(0,2):
        ...
        FILEstat = open("/home/rumbaugh/status.run.5.8.12.1705.txt",'a')
...
FILE1.close()
FILE2 = open("/home/rumbaugh/baselines.5.8.12.dat",'w')
for i in range(0,7):
    load_data("/home/rumbaugh/ChandraData/%s/master/conv.%s.soft.r_%s.5.8.12.fits"%(names[i],n_ids[i],r[i]))
    set_coord("physical")
    baseline = calc_data_sum2d("RECT(%i,%i,%i,%i)"%(xlo[i],ylo[i],xhi[i],yhi[i]))/((xhi[i]-xlo[i])*(yhi[i]-ylo[i])*1.0)
    ostemp = os.system('dmimgcalc infile=/home/rumbaugh/ChandraData/%s/master/conv.%s.soft.r_%s.5.8.12.fits infile2=none op="imgout=img1-%f" out=/home/rumbaugh/ChandraData/%s/master/tempbaseline.fits mode=h clob+'%(names[i],n_ids[i],r[i],baseline[j],names[i]))
    FILEstat = open("/home/rumbaugh/status.run.5.8.12.1705.txt",'a')
    FILEstat.write("%i - /home/rumbaugh/ChandraData/%s/master/tempbaseline.fits\n"%(ostemp,names[i]))
    FILEstat.close()
