...
for cluster,ic in zip(clbdf.index.unique().values,np.arange(len(clbdf.index.unique()))):
    ...
    lw_wt=(10.**(CMdf.lmass.values)).sum()
    lwra[ic],lwdec[ic]=(CMdf.ra.values*(10.**(CMdf.lmass.values))).sum()/lw_wt,(CMdf.dec.values*(10.**(CMdf.lmass.values))).sum()/lw_wt
    lwN[ic]=len(CMdf)
    ...
    tmplw_wt=np.sum(10.**(CMdf.lmass.values[grand]),axis=1)
    tmplwra,tmplwdec=np.sum(CMdf.ra.values[grand]*(10.**(CMdf.lmass.values[grand])),axis=1)/tmplw_wt,np.sum(CMdf.dec.values[grand]*(10.**(CMdf.lmass.values[grand])),axis=1)/tmplw_wt
...
outdf=pd.DataFrame({'field':clbdf.field,'cluster':clbdf.index.unique().values,'Xray_ra':clbdf["RA(deg)"],'Xray_dec':clbdf["Dec(deg)"],'MMCG_ra':mmcgra,'MMCG_dec':mmcgdec,'MWM_ra':lwra,'MWM_dec':lwdec,'MWM_Ngal':lwN,'dist_mmcg2x':dist_mmcg2x,'dist_mmcg2lw':dist_mmcg2lw,'dist_lw2x':dist_lw2x,'LWerr':LWerr})
outdf=outdf[['field','cluster','Xray_ra','Xray_dec','MMCG_ra','MMCG_dec','MWM_ra','MWM_dec','MWM_Ngal','dist_mmcg2x','dist_mmcg2lw','dist_lw2x','LWerr']]
