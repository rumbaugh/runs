...
for tau,it in zip(np.array([10]),np.array([1])):
    ...
    for i,pair in zip(pairs-1,pairs):
        ...
        pnflares=crpost[:,9]
        mean_pnflares=np.average(pnflares)
    ...
    #plt.title('%s'%lens)
    full_arr=np.concatenate(([np.abs(medtaus-tau)],[true_nflares],[mean_pnflares],[mean_amp],[pamp_tmp],[mean_Tfl],[fwidth_tmp],[mean_skew],[pskew_tmp]),axis=0)
    plt.figure = corner(full_arr)
