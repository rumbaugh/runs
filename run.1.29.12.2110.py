...
for i in range(0,len(names)):
    ...
    gcirc = find_reg_circ(pvals,imcensX[i],imcensY[i],srchdist)
    cp1 = time.time()
    for igc in range(0,len(igc[0])):
        if igc == 3: 
            cp2 = time.time()
            print 'Checkpoint 2 - Time Elapsed: %f seconds\nETA: %f seconds'%(cp2-cp1,(cp2-cp1)/3.0*len(igc[0])-cp2+cp1)
        tp1 = P_m(pvals,1,gcirc[0][igc],gcirc[1][igc],aperture)
        if tp1 < minp1:
            minp1 = tp1
