...
#for lens in ['1030']:
    ...
    plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/test.%s.tau_hist_plot.%s.png'%(lens,date))
    plt.clf()
    ...
    if source == '1938':
        plt.ylim(4.75,5.375)
        plt.ylabel('Flux Ratio C/B',fontsize=14)
        plt.xlabel('Time Delay B-C (days)',fontsize=14)
    if lens == '1938':plt.xlim(-60,60)
    yticklocs = plt.yticks()[0]
    if lens == '0712': 
        plt.xlim(-105,105)
        plt.axvline(9,lw=2,c='k',ls='dashed')
        plt.ylabel('Flux Ratio (A+B)/C',fontsize=14)
        plt.xlabel('Time Delay C-(A+B) (days)',fontsize=14)
        plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/test.%s.chisq_conplot.interp.triangle_mod_plot.%s.png'%(lens,date))
    if source == '1030':
        plt.xlim(-155,155)
        plt.ylabel('Flux Ratio A/B',fontsize=14)
        plt.xlabel('Time Delay B-A (days)',fontsize=14)
        plt.ylim(10.870289,13.285909)
        plt.axvline(tau1030,lw=2,c='k',ls='dashed')
        plt.savefig('/home/rumbaugh/EVLA/light_curves/plots/test.%s.chisq_conplot.interp.triangle_mod_plot.%s.png'%(lens,date))
    if source == '1938':
