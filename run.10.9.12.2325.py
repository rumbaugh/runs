...
for i in range(0,len(sigma)):
    ...
    if ((i != 4) & (i != 8)):
        ...
        set_coord('physical')
        set_model(beta2d.b)
        r200 = 2*(mpc[i]*60)*2*sigma[i]/(m.sqrt(500)*Hz[i])
        r200r = 2*(mpc[i]*60)*2*sigma[i]/(m.sqrt(200)*Hz[i])
        b.xpos = xcens[i]
        b.ypos = ycens[i]
        ...
        print calc_source_sum2d('circle(%f,%f,%f)'%(b.xpos.val,b.ypos.val,r200r))/(2*m.pi*b.ampl.val*b.r0.val*b.r0.val)
        cperc_tot =  (2*m.pi*b.ampl.val*b.r0.val*b.r0.val*(1-1.0/m.sqrt(1+r200r*r200r*1.0/b.r0.val*1.0/b.r0.val)))/(2*m.pi*b.ampl.val*b.r0.val*b.r0.val)
        FILE.write('%s %f\n'%(names[i],cperc_tot))
    if ((i == 3) | (i == 7)):
        r200 = 2*(mpc[i+1]*60)*2*sigma[i+1]/(m.sqrt(500)*Hz[i+1])
        r200r = 2*(mpc[i+1]*60)*2*sigma[i+1]/(m.sqrt(200)*Hz[i+1])
...
FILE.close()
FILE=open('/home/rumbaugh/paperstuff/clus.lums.soft.9.27.12.dat','w')
