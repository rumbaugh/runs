...
psfpdf=bpdf.PdfPages('/home/rumbaugh/Chandra/plots/centroid_comp.7.26.17.pdf')
...
df=pd.merge(df,mmcgdf,left_on=['field','cluster'],right_on=['field','id_cluster'],how='outer',suffixes=('','_MMCG'))
...
for clus in df.cluster.values:
    ...
    plt.scatter([df.LWM_ra.iloc[g]],[df.LWM_dec.iloc[g]],marker='s',s=50,color='magenta',label='LWM(SC)')
    plt.scatter([df.BCG_ra.iloc[g]],[df.BCG_dec.iloc[g]],marker='o',s=50,color='green',label='BCG')
    plt.scatter([df.ra_spec.iloc[g]],[df.dec_spec.iloc[g]],marker='o',s=50,color='red',label='MMCG')
