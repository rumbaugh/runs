...
#        FILEA.write('%s & %2i & %2i\\ %2i\\ %4.1f & +%2i\\ %2i\\ %2i & %5.3f\\\\ \n'%(names[i],argcnt,rahA[jj],ramA[jj],rasA[jj],decdA[jj],decmA[jj],decsA[jj],zA[jj]))
    ...
    #gsq = np.where((sq > 2.3) & (sI <= Ilim) & (sI >= Imax))
    gsq = np.where((sq > 2.3))
    gsq = gsq[0]
    ...
    if i == 0:
        ...
        g = np.where((sz[gsq[gsz]] > 0.72) & (sOS[gsq[gsz]] > 0.01) & (sOS[gsq[gsz]] < 0.2))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=10.3,color='red')
        g = np.where((sz[gsq[gsz]] < 0.72) & (sOS[gsq[gsz]] > 0.01) & (sOS[gsq[gsz]] < 0.2))
        pylab.scatter(sRA[gsq[gsz[g]]],sDec[gsq[gsz[g]]],s=10.3,color='black')
        leg = pylab.legend(('z < 0.68','0.68 < z < 0.725','0.725 < z < 0.765','z > 0.765'),loc=7,markerscale=0.8,labelspacing=0.3)
        icoff = 5
        F = pylab.gcf()
    ...
    if i == 0: savfile = '/home/rumbaugh/paperstuff/AGN.spatmap.color.%s.r_%3.1fMpc_nb.5.20.11.png'%(names[i],cradmult)
