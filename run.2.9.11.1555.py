...
for i in range(2,4):
    ...
    set_coord('physical')
    cnts1s = calc_data_sum2d('circle(%s,%i)'%(cen,bnds[i]))
    cnts1bg = calc_data_sum2d('circle(%s,%i)-circle(%s,%i)'%(cen,bgB,cen,bgA))
    cnts1 = calc_data_sum2d('circle(%s,%i)'%(cen,bnds[i]))-(bnds[i])**2*calc_data_sum2d('circle(%s,%i)-circle(%s,%i)'%(cen,bgB,cen,bgA))/(bgB**2-bgA**2)
    cntserr1 = m.sqrt(cnts1s+cnts1bg*((bnds[i])**2/(bgB**2-bgA**2))**2)
    ...
    flb = 0.5/(1.76)
    fub = 2.0/1.76
    n1 = calc_model_sum(0.5,2.0)
    rs.norm = 1.0/n1
    calc_model_sum(0.5,2.0)
    ...
    flux = cnts1*cnt2flux/time
    lum = flux*fourpiDl2
    lumerr = cntserr1*cnt2flux*fourpiDl2/time
    print lum
    print lumerr
