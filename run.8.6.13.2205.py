...
for mask in ['1131m3','1131m4']:
	for slit,i in zip(['1'],np.arange(1)):
		tdict = copy.deepcopy(files_dict[mask])
		print 'Slit #%i'%(i+1)
		lris_reduce_multislit(indir_dict[mask],outdir_dict[mask],files_dict[mask]['prefix'],tdict,mask,colors='both',sides=['top'],doCR=False,slitname=slit)
