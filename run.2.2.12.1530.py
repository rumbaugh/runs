...
for i in range(0,len(names)):
    ...
    while validBCGs < 3.4:
        isgood = 0
        si += 1
        gsi = FindCloseSources(RA[g[gall[grs[garg[si]]]]],Dec[g[gall[grs[garg[si]]]]],0.01,sRA[gs[galls[gzs[grss]]]],sDec[gs[galls[gzs[grss]]]],0)
        if len(gsi) > 0: 
            if ((sz[gs[galls[gzs[grss[gsi[0]]]]]] > centerzs[i]-0.01) & (sz[gs[galls[gzs[grss[gsi[0]]]]]] < centerzs[i]+0.01)): isgood = 1
        if ((len(gsi) == 0) | (isgood == 1)):
            ...
            #    print 'None'
    #print validBCGIDs
    print "%s - BCG Position\n(RA,Dec) = (%f,%f)\ni'/f814 magnitude: %f\n"%(names[i],RA[g[gall[grs[garg[validBCGIDs[0]]]]]],Dec[g[gall[grs[garg[validBCGIDs[0]]]]]],iB[g[gall[grs[garg[validBCGIDs[0]]]]]])
    print "%s - 2nd Position\n(RA,Dec) = (%f,%f)\ni'/f814 magnitude: %f\n"%(names[i],RA[g[gall[grs[garg[validBCGIDs[1]]]]]],Dec[g[gall[grs[garg[validBCGIDs[1]]]]]],iB[g[gall[grs[garg[validBCGIDs[1]]]]]])
    print "%s - 3rd Position\n(RA,Dec) = (%f,%f)\ni'/f814 magnitude: %f\n"%(names[i],RA[g[gall[grs[garg[validBCGIDs[2]]]]]],Dec[g[gall[grs[garg[validBCGIDs[2]]]]]],iB[g[gall[grs[garg[validBCGIDs[2]]]]]])
    print "%s - 4th Position\n(RA,Dec) = (%f,%f)\ni'/f814 magnitude: %f\n"%(names[i],RA[g[gall[grs[garg[validBCGIDs[3]]]]]],Dec[g[gall[grs[garg[validBCGIDs[3]]]]]],iB[g[gall[grs[garg[validBCGIDs[3]]]]]])
