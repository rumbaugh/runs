...
def switchorder(arr):
    if len(arr) != 5: sys.exit("array of length 5 for switch")
    arr[0],arr[1],arr[2],arr[3],arr[4] = arr[1],arr[3],arr[0],arr[4],arr[2]
    return arr
...
cl1604f2 = '/home/rumbaugh/LFC/ACS_merged.F606W+F814W_deep.all.coll.dat'
...
rsfitm = np.array([0.0229,0.0084,0.0419,0.0012,0.063])
rsSTD = np.array([0.0625,0.0735,0.0576,0.0413,0.0907])
rsnstd = np.array([3.,2.,3.,3.,3.])
mx = (43.2104078-43.348505)/(240.9472-241.28263)
rsfitb = switchorder(rsfitb)
rsfitm = switchorder(rsfitm)
rsSTD = switchorder(rsSTD)
rsnstd = switchorder(rsnstd)
...
for i in range(0,5):
    ...
    if i == 3:
        cr2 = read_file(cl1604f2)
        pI = get_colvals(cr2,'col3')
        pR = get_colvals(cr2,'col4')
    ...
    if i == 3:
        sI = get_colvals(crs,'col17')
        sR = get_colvals(crs,'col18')
    ...
    gsqNZ = gsqNZ[0]
    gsqRS = np.where((sR[gsq[gsqNZ]]-sI[gsq[gsqNZ]] > rsfitb[i]-rsfitm[i]*sI[gsq[gsqNZ]]-rsnstd[i]*rsSTD[i]) & (sR[gsq[gsqNZ]]-sI[gsq[gsqNZ]] < rsfitb[i]-rsfitm[i]*sI[gsq[gsqNZ]]+rsnstd[i]*rsSTD[i]))
    gsqRS = gsqRS[0]
    ...
    gt = np.where((pI <= Ilim) & (pI > 0.1) & (pR > 0.1))
    gt = gt[0]
    #gtNZ = np.where((pI[gt] > 0.1) & (pR[gt] > 0.1))
    #gtNZ = gtNZ[0]
    gtRS = np.where((pR[gt]-pI[gt] > rsfitb[i]-rsfitm[i]*pI[gt]-rsnstd[i]*rsSTD[i]) & (pR[gt]-pI[gt] < rsfitb[i]-rsfitm[i]*pI[gt]+rsnstd[i]*rsSTD[i]))
    gtRS = gtRS[0]
    gtNRS = np.where((pR[gt]-pI[gt] < rsfitb[i]-rsfitm[i]*pI[gt]-rsnstd[i]*rsSTD[i]))
    gtNRS = gtNRS[0]
    ...
    print 'Total Blue clusters: %i'%(int(len(gtNRS)*len(gszNRS)*1.0/(len(gsqNRS)+nwl2)))
    print 'Total Red clusters: %i'%(int(len(gtRS)*len(gszRS)*1.0/(len(gsqRS)+nwl2)))
    print 'Blue fraction: %6.3f'%((len(gtNRS)*len(gszNRS)*1.0/(len(gsqNRS)+nwl2))/((len(gtNRS)*len(gszNRS)*1.0/(len(gsqNRS)+nwl2)) +(len(gtRS)*len(gszRS)*1.0/(len(gsqRS)+nwl2))))
