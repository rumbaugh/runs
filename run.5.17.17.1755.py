...
for i in range(0,len(crx['RA'][gsig])):
    ...
    tmpzdist=((1+zx)**2-(1+zs[gnoz])**2)/((1+zx)**2+(1+zs[gnoz])**2)*3.*10.**5/clus_sig[gnoz]
...
for field in targets: 
    ...
    for i in range(0,len(gq)):
        zx=crs['z'][gq[i]]
        tmpzdist=((1+zx)**2-(1+zs[g])**2)/((1+zx)**2+(1+zs[g])**2)*3.*10.**5/clus_sig[g]
...
plt.hist(np.log10(np.abs(PS_AGN)),color='r',bins=[-1.75,-1,np.log10(0.4),np.log10(2),1],normed=True)
plt.hist(np.log10(np.abs(PS_all)),color='b',bins=[-1.75,-1,np.log10(0.4),np.log10(2),1],normed=True,edgecolor='k',facecolor='None',lw=4)#weights=25*np.ones(len(allclusdist))*1./len(allclusdist),)
#plt.scatter(ccd,veldist,color='r',s=16)
...
plt.scatter(allclusdist[g_alllowb],allveldist[g_alllowb]*30./29,color='k',s=18,marker=matplotlib.markers.CARETDOWN,facecolor="None",edgecolor='k')
