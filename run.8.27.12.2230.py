...
visarr2 = np.empty(3,dtype='string')
...
while ((i < 13) & (endloop < 0.5)):
    ...
    if ((i != 13) & (i != 6) & (i != 14) & (i != 16)): 
        ...
        if counter > 2:
            counter = 0
            concat(vis=visarr2,concatvis='/local3/rumbaugh/EVLA/data/11A-138/temp.concat%i.vis'%(counter2+1))
            visarr2 = np.empty(3,dtype='string')
            counter2 += 1
    conflagarr = np.loadtxt('/home/rumbaugh/Continue.txt',dtype='string')  
    if conflagarr == 'False': endloop = 1
    if endloop != 1: i += 1
concat(vis=['/local3/rumbaugh/EVLA/data/11A-138/temp.concat1.vis','/local3/rumbaugh/EVLA/data/11A-138/temp.concat2.vis','/local3/rumbaugh/EVLA/data/11A-138/temp.concat3.vis','/local3/rumbaugh/EVLA/data/11A-138/temp.concat4.vis'],concatvis='/local3/rumbaugh/EVLA/data/11A-138/B1938+666.Late_concat.8.27.12.ms')
