...
FILE=open('/home/rumbaugh/runs/run.12.10.14.1720.sh','w')
FILE.write("#!/bin/bash\n")
    ...
    for groupnum in np.sort(EVLA_obs_dict[EorL].keys()):
        ...
        for SBnum in np.sort(EVLA_obs_dict[EorL][groupnum].keys()):
            ...
            for field,fieldname in zip(lensfields_arr,lensnames):
                ...
                #FILE.write(fit_string)
                if SBnum != 'X':
                    FILE.write("modelfit mfitniter\nselfcal false,false,60\nmodelfit mfitniter\nselfcal false,false, 60\nmodelfit mfitniter\nshift 0.1,-0.3\nwmap  /home/rumbaugh/B1938+666_files/%s.%sSB%i_%s.fixpos.%s.fits\ncmul = imstat(rms)\naddcmp cmul,false,0,0\n"%(source,EorL,groupnum,str(SBnum),date))
                    outfile = '/home/rumbaugh/B1938+666_files/difmap_results/fit_wrms.%s.%sSB%i_%s.fixpos.%s.mod'%(source,EorL,groupnum,str(SBnum),date)
                    FILE.write('wmod %s\n'%outfile)
                FILE.write(end_string)
FILE.close()
